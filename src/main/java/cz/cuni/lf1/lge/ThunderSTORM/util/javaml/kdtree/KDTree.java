...
public class KDTree<T> {
	...
	public List<T> range(double[] lowk, double[] uppk) throws KeySizeException {
		...
		if (lowk.length != uppk.length) {
			...
			KDNode.<T> rsearch(new HPoint(lowk), new HPoint(uppk), m_root, 0, m_K, v);
	...
	/**
	 * Ball query in a kdtree. Returns all points that are closer(based on
	 * euclidean distance) than maxDistance to the queryPoint.
	 * 
	 * @param queryPoint
	 * @param maxDistance
	 * @return
	 ...
	 */
	public List<DistAndValue<T>> ballQuery(double[] queryPoint, double maxDistance)
		throws KeySizeException
	{
		...
		Vector<KDNode<T>> v = new Vector<KDNode<T>>();
		KDNode.<T> rsearch(new HPoint(lower), new HPoint(upper), m_root, 0, m_K, v);
