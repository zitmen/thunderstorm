...
public class EllipticGaussianPSF extends PSFModel {
	...
	public double getValue(double[] params, double x, double y) {
		...
		return params[Params.INTENSITY] * exp(-0.5 * (sqr(dx) / params[Params.SIGMA1] + sqr(dy) /
			params[Params.SIGMA2])) + params[Params.OFFSET];
	}
		...
		return new MultivariateMatrixFunction() {
			...
			public double[][] value(double[] point) throws IllegalArgumentException {
				...
				for (int i = 0; i < xgrid.length; i++) {
					...
					// diff(psf, x0)
					double pom1 = first * cosfi / transformedPoint[Params.SIGMA1] + second * sinfi /
						transformedPoint[Params.SIGMA2];
					retVal[i][Params.X] = transformedPoint[Params.INTENSITY] * pom1 * expVal;
					// diff(psf, y0)
					double pom2 = -first * sinfi / transformedPoint[Params.SIGMA1] + second * cosfi /
						transformedPoint[Params.SIGMA2];
					...
					if (calibration != null) {
						// diff(psf, z0)
						double pom3 = calibration.dwx2(transformedPoint[PSFModel.Params.Z]) / 2.0 * sqr(first /
							transformedPoint[Params.SIGMA1]) + calibration.dwy2(
								transformedPoint[PSFModel.Params.Z]) / 2.0 * sqr(second /
									transformedPoint[Params.SIGMA2]);
						retVal[i][Params.Z] = transformedPoint[Params.INTENSITY] * expVal * pom3;
					}
					else {
						// diff(psf, sigma1)
						retVal[i][Params.SIGMA1] = transformedPoint[Params.INTENSITY] * expVal * sqr(first) /
							sqr(transformedPoint[Params.SIGMA1]);
						// diff(psf, sigma2)
						retVal[i][Params.SIGMA2] = transformedPoint[Params.INTENSITY] * expVal * sqr(second) /
							sqr(transformedPoint[Params.SIGMA2]);
					}
					// diff(psf, off)
		...
		boolean afterFitting)
	{
		if (afterFitting) {
			...
			params[Params.INTENSITY] = params[Params.INTENSITY] * 2 * PI * params[Params.SIGMA1] *
				params[Params.SIGMA2];
		}
		Molecule mol = new Molecule(new Params(new int[] { Params.X, Params.Y, Params.Z, Params.SIGMA1,
			Params.SIGMA2, Params.INTENSITY, Params.OFFSET, Params.BACKGROUND }, params, true));
