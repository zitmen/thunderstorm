...
public class IntegratedSymmetricGaussianPSF extends PSFModel {
	...
	public double getValue(double[] params, double x, double y) {
		...
		// Integrate[ b^2 + J^2/(2*Pi*s^2) *E^(-1/2*((x - x0)^2/(s^2) + (y -
		// y0)^2/(s^2))), {x, ax - 1/2, ax + 1/2}, {y, ay - 1/2, ay + 1/2}]
		...
		final double[] ygrid)
	{
		return new MultivariateMatrixFunction() {
			...
			// Grad[b^2 + (1/4)*J^2*(Erf[(1/2 + ax - x0)/(Sqrt[2]*s)] - Erf[(-(1/2) +
			// ax - x0)/(Sqrt[2]*s)])* (Erf[(1/2 + ay - y0)/(Sqrt[2]*s)] - Erf[(-(1/2)
			// + ay - y0)/(Sqrt[2]*s)]), {b, J, s, x0, y0}]
			public double[][] value(double[] point) throws IllegalArgumentException {
				...
				for (int i = 0; i < edge; i++) {
					...
					for (int j = 0; j < edge; j++) {
						...
						retVal[idx][Params.SIGMA] = 0.25 * transformedPoint[Params.INTENSITY] * (errDiffX *
							expDiffSY + errDiffY * expDiffSXs[j]);
						retVal[idx][Params.X] = 0.25 * transformedPoint[Params.INTENSITY] * errDiffY *
							expDiffXs[j];
...
//                IJ.log("analytic jacobian: " + Arrays.deepToString(retVal));
	 ...
	 * subimage, some values can be reused. But can only be used for a square grid
	 * where xgrid values are same in each column and ygrid values are the same in
	 * each row.
	 *
	 * @param xgrid
	 * @param ygrid
		...
		return new MultivariateVectorFunction() {
			...
			public double[] value(final double[] point) throws IllegalArgumentException {
				...
				for (int i = 0; i < edge; i++) {
					...
					for (int j = 0; j < edge; j++) {
						double errDiffX = errDiffXs[j];
						retVal[idx] = transformedPoint[Params.OFFSET] + 0.25 *
							transformedPoint[Params.INTENSITY] * errDiffX * errDiffY;
						idx++;
					}
				}
				return retVal;
		...
		boolean afterFitting)
	{
		...
		Molecule mol = new Molecule(new Params(new int[] { Params.X, Params.Y, Params.SIGMA,
			Params.INTENSITY, Params.OFFSET, Params.BACKGROUND }, params, true));
	...
	public static double erf(double z) {
		...
		// use Horner's method
		double ans = 1 - t * Math.exp(-z * z - 1.26551223 + t * (1.00002368 + t * (0.37409196 + t *
			(0.09678418 + t * (-0.18628806 + t * (0.27886807 + t * (-1.13520398 + t * (1.48851587 + t *
