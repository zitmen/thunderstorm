...
public abstract class PostProcessingModule {
	...
	protected void saveStateForUndo() {
		...
		if (history.getLastOperation() != null && history.getLastOperation().getName().equals(
			getTabName()))
		{
	...
	class DefaultOperation extends OperationsHistoryPanel.Operation {
		...
		public DefaultOperation() {
			...
			if (!oldRecording) Recorder.saveCommand(); // remove from macro recorder
																									// if not recording!
		...
		protected void clicked() {
			...
			// change thread name temporarily beacuse Macro.getOptions() requires the
			// thread name to start with "Run$_"
