 ...
 */
public final class IJHistogramWindow extends ImageWindow implements Measurements, ActionListener,
	ClipboardOwner, MouseListener, MouseMotionListener, ImageListener, KeyListener, Runnable
{
	...
	/**
	 * Displays a histogram using the specified parameter name, title and
	 * ImageStatistics.
	 */
	...
	public void showHistogram(ImagePlus imp, ImageStatistics stats) {
		...
		boolean fixedRange = type == ImagePlus.GRAY8 || type == ImagePlus.COLOR_256 ||
	...
	void drawHistogram(ImagePlus imp, ImageProcessor ip, boolean fixedRange, double xMin,
		double xMax)
	{
		int x, y;
		long maxCount2 = 0;
	...
	protected void showList() {
		...
		if (cal.calibrated() && !cal.isSigned16Bit()) {
			for (int i = 0; i < stats.nBins; i++) {
				sb.append(i).append("\t").append(ResultsTable.d2s(cal.getCValue(stats.histMin + i *
					stats.binSize), digits)).append("\t").append(histogram[i]).append("\n");
			}
			TextWindow tw = new TextWindow(getTitle(), "level\t" + vheading + "\tcount", sb.toString(),
				200, 400);
		}
		else {
			for (int i = 0; i < stats.nBins; i++) {
				sb.append(ResultsTable.d2s(cal.getCValue(stats.histMin + i * stats.binSize), digits))
					.append("\t").append(histogram[i]).append("\n");
			}
	...
	protected void copyToClipboard() {
		...
		for (int i = 0; i < stats.nBins; i++) {
			pw.print(ResultsTable.d2s(cal.getCValue(stats.histMin + i * stats.binSize), digits) + "\t" +
				histogram[i] + "\n");
		}
		...
		while (true) {
			...
			synchronized (this) {
				if (doUpdate) {
					...
					catch (InterruptedException e) { // interrupted tells the thread to
																						// exit
						return;
					}
				}
	...
	private void applyRoiAsFilter() {
		...
		double leftVal = cal.getCValue(stats.histMin + indexLeft * stats.binSize); // param
																																								// >
																																								// val
		double rightVal = cal.getCValue(stats.histMin + indexRight * stats.binSize); // param
																																									// <
																																									// val

		IJResultsTable.getResultsTable().tableWindow.getFilter().addNewFilter(paramName, leftVal,
			rightVal);
	}
}
