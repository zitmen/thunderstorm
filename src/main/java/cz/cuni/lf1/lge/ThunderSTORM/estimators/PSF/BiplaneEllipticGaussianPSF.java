...
public class BiplaneEllipticGaussianPSF extends PSFModel {
	...
	public BiplaneEllipticGaussianPSF(DoubleDefocusCalibration<DaostormCalibration> calibration,
		boolean numericalDerivatives)
	{
		...
		boolean afterFitting)
	{
		...
		Molecule mol = new Molecule(new Params(new int[] { Params.X, Params.Y, Params.Z, Params.SIGMA1,
			Params.SIGMA2, Params.SIGMA3, Params.SIGMA4, Params.INTENSITY, Params.OFFSET1, Params.OFFSET2,
			Params.BACKGROUND }, params, true));
	...
	public double[] getInitialParams(SubImage plane1, SubImage plane2) {
		...
		guess[Params.Z] = 0;
		guess[Params.INTENSITY] = (plane1.getMax() - plane1.getMin()) + (plane2.getMax() - plane2
			.getMin());
	...
	public MultivariateVectorFunction getValueFunction(final double[] xgrid1, final double[] ygrid1,
		final double[] xgrid2, final double[] ygrid2)
	{
		return new MultivariateVectorFunction() {
			...
			public double[] value(double[] params) throws IllegalArgumentException {
				...
				for (int i = 0; i < xgrid1.length; i++, index++) {
					double dx = ((xgrid1[i] - tparams[Params.X]) * cosfi1 - (ygrid1[i] - tparams[Params.Y]) *
						sinfi1);
					double dy = ((xgrid1[i] - tparams[Params.X]) * sinfi1 + (ygrid1[i] - tparams[Params.Y]) *
						cosfi1);
					retVal[index] = exp(-0.5 * (sqr(dx / params[Params.SIGMA1]) + sqr(dy /
						params[Params.SIGMA2]))) * (tparams[Params.INTENSITY] / 2.0) / (2 * Math.PI *
							params[Params.SIGMA1] * params[Params.SIGMA2]) + tparams[Params.OFFSET1];
				}
				for (int i = 0; i < xgrid2.length; i++, index++) {
					double dx = ((xgrid2[i] - tparams[Params.X]) * cosfi2 - (ygrid2[i] - tparams[Params.Y]) *
						sinfi2);
					double dy = ((xgrid2[i] - tparams[Params.X]) * sinfi2 + (ygrid2[i] - tparams[Params.Y]) *
						cosfi2);
					retVal[index] = exp(-0.5 * (sqr(dx / params[Params.SIGMA3]) + sqr(dy /
						params[Params.SIGMA4]))) * (tparams[Params.INTENSITY] / 2.0) / (2 * Math.PI *
							params[Params.SIGMA3] * params[Params.SIGMA4]) + tparams[Params.OFFSET2];
				}
				return retVal;
			}
		};
	...
	public MultivariateMatrixFunction getJacobianFunction(final double[] xgrid1,
		final double[] ygrid1, final double[] xgrid2, final double[] ygrid2)
	{
		return useNumericalDerivatives ? getNumericJacobianFunction(xgrid1, ygrid1, xgrid2, ygrid2)
			: getAnalyticJacobianFunction(xgrid1, ygrid1, xgrid2, ygrid2);
	}

	private MultivariateMatrixFunction getAnalyticJacobianFunction(final double[] xgrid1,
		final double[] ygrid1, final double[] xgrid2, final double[] ygrid2)
	{
		return new MultivariateMatrixFunction() {
			...
			public double[][] value(double[] point) throws IllegalArgumentException {
				...
				for (int i = 0; i < xgrid1.length; i++, index++) {
					...
					double expVal = exp(-0.5 * (sqr(first) / transformedPoint[Params.SIGMA1] + sqr(second) /
						transformedPoint[Params.SIGMA2]));
					double oneDivPISS2 = 1 / (Math.PI * point[Params.SIGMA1] * point[Params.SIGMA2]);
					// diff(psf, x0)
					double pom1 = first * cosfi1 / transformedPoint[Params.SIGMA1] + second * sinfi1 /
						transformedPoint[Params.SIGMA2];
					retVal[index][Params.X] = oneDivPISS2 * 0.5 * transformedPoint[Params.INTENSITY] * pom1 *
						expVal;
					// diff(psf, y0)
					double pom2 = first * sinfi1 / transformedPoint[Params.SIGMA1] + second * cosfi1 /
						transformedPoint[Params.SIGMA2];
					retVal[index][Params.Y] = oneDivPISS2 * 0.5 * transformedPoint[Params.INTENSITY] * pom2 *
						expVal;
					// diff(psf, z0)
					double pom4 = (transformedPoint[Params.Z] - calibration.cal1.c1) / sqr(
						calibration.cal1.d1 * (1 + sqr((transformedPoint[Params.Z] - calibration.cal1.c1) /
							calibration.cal1.d1)));
					double pom5 = (transformedPoint[Params.Z] - calibration.cal1.c2) / sqr(
						calibration.cal1.d2 * (1 + sqr((transformedPoint[Params.Z] - calibration.cal1.c2) /
							calibration.cal1.d2)));
					double pom3 = sqr(first) * pom4 + sqr(second) * pom5;
					retVal[index][Params.Z] = oneDivPISS2 * 0.5 * transformedPoint[Params.INTENSITY] *
						expVal * pom3 - oneDivPISS2 * 0.5 * transformedPoint[Params.INTENSITY] * expVal * pom4 -
				...
				for (int i = 0; i < xgrid2.length; i++, index++) {
					...
					double first = cosfiXd - sinfiYd, second = sinfiXd + cosfiYd;
					double expVal = exp(-0.5 * (sqr(first) / transformedPoint[Params.SIGMA3] + sqr(second) /
						transformedPoint[Params.SIGMA4]));
					double oneDivPISS2 = 1 / (Math.PI * point[Params.SIGMA3] * point[Params.SIGMA4]);
					// diff(psf, x0)
					double pom1 = first * cosfi2 / transformedPoint[Params.SIGMA3] + second * sinfi2 /
						transformedPoint[Params.SIGMA4];
					retVal[index][Params.X] = oneDivPISS2 * 0.5 * transformedPoint[Params.INTENSITY] * pom1 *
						expVal;
					// diff(psf, y0)
					double pom2 = first * sinfi2 / transformedPoint[Params.SIGMA3] + second * cosfi2 /
						transformedPoint[Params.SIGMA4];
					retVal[index][Params.Y] = oneDivPISS2 * 0.5 * transformedPoint[Params.INTENSITY] * pom2 *
						expVal;
					// diff(psf, z0)
					double pom4 = (transformedPoint[Params.Z] - calibration.cal2.c1) / sqr(
						calibration.cal2.d1 * (1 + sqr((transformedPoint[Params.Z] - calibration.cal2.c1) /
							calibration.cal2.d1)));
					double pom5 = (transformedPoint[Params.Z] - calibration.cal2.c2) / sqr(
						calibration.cal2.d2 * (1 + sqr((transformedPoint[Params.Z] - calibration.cal2.c2) /
							calibration.cal2.d2)));
					double pom3 = sqr(first) * pom4 + sqr(second) * pom5;
					retVal[index][Params.Z] = oneDivPISS2 * 0.5 * transformedPoint[Params.INTENSITY] *
						expVal * pom3 - oneDivPISS2 * 0.5 * transformedPoint[Params.INTENSITY] * expVal * pom4 -
	...
	private MultivariateMatrixFunction getNumericJacobianFunction(final double[] xgrid1,
		final double[] ygrid1, final double[] xgrid2, final double[] ygrid2)
	{
		final MultivariateVectorFunction valueFunction = getValueFunction(xgrid1, ygrid1, xgrid2,
			ygrid2);
		return new MultivariateMatrixFunction() {
	...
	public MultivariateFunction getLikelihoodFunction(double[] xgrid1, double[] ygrid1,
		final double[] values1, double[] xgrid2, double[] ygrid2, final double[] values2)
	{
		final MultivariateVectorFunction valueFunction = getValueFunction(xgrid1, ygrid1, xgrid2,
			ygrid2);
		return new MultivariateFunction() {
			...
			public double value(double[] point) {
				...
				for (int i = 0; i < values1.length; i++, index++) {
					logLikelihood += values1[i] * Math.max(-1e6, log(expectedValues[index])) -
						expectedValues[index];
				}
				for (int i = 0; i < values2.length; i++, index++) {
					logLikelihood += values2[i] * Math.max(-1e6, log(expectedValues[index])) -
						expectedValues[index];
				}
	...
	public double getChiSquared(double[] xgrid1, double[] ygrid1, double[] values1, double[] xgrid2,
		double[] ygrid2, double[] values2, double[] params, boolean weighted)
	{
