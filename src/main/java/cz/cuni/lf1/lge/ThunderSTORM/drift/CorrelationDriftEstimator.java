...
public class CorrelationDriftEstimator {
	 ...
	 */
	public static CrossCorrelationDriftResults estimateDriftFromCoords(double[] x, double[] y,
		double[] frame, final int steps, final double magnification, final double smoothingBandwidth,
		int roiWidth, int roiHeight, boolean saveCorrelationImages)
	{
		...
		RenderingMethod renderer = new ASHRendering.Builder().roi(0, imageWidth, 0, imageHeight)
			.resolution(1 / magnification).shifts(2).build();
		RenderingMethod lowResRenderer = new ASHRendering.Builder().roi(0, imageWidth, 0, imageHeight)
			.resolution(1).shifts(2).build();

		FloatProcessor firstImage = (FloatProcessor) renderer.getRenderedImage(bins.xBinnedByFrame[0],
			bins.yBinnedByFrame[0], null, null, null).getProcessor().convertToFloat();
		int paddedSize = MathProxy.nextPowerOf2(MathProxy.max(firstImage.getWidth(), firstImage
			.getHeight()));
		FHT firstImageFFT = createPaddedFFTImage(firstImage, paddedSize);

		FloatProcessor lowResFirstImage = (FloatProcessor) lowResRenderer.getRenderedImage(
			bins.xBinnedByFrame[0], bins.yBinnedByFrame[0], null, null, null).getProcessor()
			.convertToFloat();
		int lowResPaddedSize = MathProxy.nextPowerOf2(MathProxy.max(lowResFirstImage.getWidth(),
			lowResFirstImage.getHeight()));
		...
		for (int i = 1; i < steps; i++) {
			...
			FloatProcessor nextImage = (FloatProcessor) renderer.getRenderedImage(bins.xBinnedByFrame[i],
				bins.yBinnedByFrame[i], null, null, null).getProcessor().convertToFloat();
			...
			FloatProcessor lowResNextImage = (FloatProcessor) lowResRenderer.getRenderedImage(
				bins.xBinnedByFrame[i], bins.yBinnedByFrame[i], null, null, null).getProcessor()
				.convertToFloat();
			FHT lowResImageFFT = createPaddedFFTImage(lowResNextImage, lowResPaddedSize);
			FHT lowResCrossCorrelationImage = computeCrossCorrelationImage(lowResFirstImageFFT,
				lowResImageFFT);
			...
			multiplyImageByGaussianMask(new Point2D.Double(-driftXofImage[i - 1] / magnification +
				lowResPaddedSize / 2, -driftYofImage[i - 1] / magnification + lowResPaddedSize / 2),
				lowResPaddedSize, lowResCrossCorrelationImage);
			Point2D.Double lowResMaximumCoords = CorrelationDriftEstimator.findMaxima(
				lowResCrossCorrelationImage);
			lowResMaximumCoords = CorrelationDriftEstimator.findMaximaWithSubpixelPrecision(
				lowResMaximumCoords, 11, lowResCrossCorrelationImage);

			// translate maxima coords from low res image to high res image
			Point2D.Double highResMaximumCoords = new Point2D.Double(crossCorrelationImage.getWidth() /
				2 + magnification * (lowResMaximumCoords.x - (lowResCrossCorrelationImage.getWidth() / 2)),
				crossCorrelationImage.getHeight() / 2 + magnification * (lowResMaximumCoords.y -
					(lowResCrossCorrelationImage.getHeight() / 2)));
			// find maxima in high res image
			highResMaximumCoords = CorrelationDriftEstimator.findMaximaWithSubpixelPrecision(
				highResMaximumCoords, 11, crossCorrelationImage);
		...
		// interpolate the drift using loess interpolator, or linear interpolation
		// if not enough data for loess
		...
		if (steps < 4) {
			...
			xFunction = addLinearExtrapolationToBorders(interpolator.interpolate(bins.binCenters,
				driftXofImage), bins.minFrame, bins.maxFrame);
			yFunction = addLinearExtrapolationToBorders(interpolator.interpolate(bins.binCenters,
				driftYofImage), bins.minFrame, bins.maxFrame);
		}
		else {
		...
		IJ.showStatus("");
		IJ.showProgress(1.0);
		return new CrossCorrelationDriftResults(correlationImages, xFunction, yFunction,
			bins.binCenters, driftXofImage, driftYofImage, 1 / magnification, bins.minFrame,
			bins.maxFrame, MoleculeDescriptor.Units.PIXEL);
	}
	...
	private static void multiplyImageByGaussianMask(Point2D.Double gaussianCenter,
		double gaussianSigma, FloatProcessor image)
	{
		for (int y = 0; y < image.getHeight(); y++) {
			for (int x = 0; x < image.getWidth(); x++) {
				double maskValue = MathProxy.exp(-(MathProxy.sqr(x - gaussianCenter.x) + MathProxy.sqr(y -
					gaussianCenter.y)) / (2 * gaussianSigma * gaussianSigma));
	...
	private static class BinningResults {
		...
		public BinningResults(double[][] xBinnedByFrame, double[][] yBinnedByFrame, double[] binCenters,
	...
	private static BinningResults binResultByFrame(double[] x, double[] y, double[] frame,
		int binCount)
	{
		...
		return new BinningResults(xBinnedByFrame, yBinnedByFrame, binCenters, (int) minFrame,
			(int) maxFrame);
	}
	...
	static Point2D.Double findMaxima(FloatProcessor crossCorrelationImage) {
		...
		return new Point2D.Double(maxIndex % crossCorrelationImage.getWidth(), maxIndex /
			crossCorrelationImage.getWidth());
	}

	static Point2D.Double findMaximaWithSubpixelPrecision(Point2D.Double maximumCoords, int roiSize,
		FHT crossCorrelationImage)
	{
		...
		if (isCloseToBorder((int) maximumCoords.x, (int) maximumCoords.y, (roiSize - 1) / 2,
			crossCorrelationImage))
		{
		...
		return new Point2D.Double((int) maximumCoords.x + psf.getX(), (int) maximumCoords.y + psf
			.getY());
	}

	//
	public static PolynomialSplineFunction addLinearExtrapolationToBorders(
		PolynomialSplineFunction spline, int minFrame, int maxFrame)
	{
		...
		if (addToBeginning) {
			...
			PolynomialFunction beginningFunction = new PolynomialFunction(new double[] {
				valueAtFirstKnot - (knots[0] - minFrame) * derivativeAtFirstKnot, derivativeAtFirstKnot });
		...
		if (addToEnd) {
			...
			// add function
			double derivativeAtLastKnot = polynomials[polynomials.length - 1].polynomialDerivative()
				.value(knots[knots.length - 1] - knots[knots.length - 2]);
			double valueAtLastKnot = spline.value(knots[knots.length - 1]);
			PolynomialFunction endFunction = new PolynomialFunction(new double[] { valueAtLastKnot,
				derivativeAtLastKnot });
