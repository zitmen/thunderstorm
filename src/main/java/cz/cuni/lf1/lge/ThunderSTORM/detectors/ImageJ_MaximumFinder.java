// Note: this is a copy of ImageJ's MaximumFinder class, but all the manipulations with statusBar or progressBar were disabled
...
/**
 * This ImageJ plug-in filter finds the maxima (or minima) of an image. It can
 * create a mask where the local maxima of the current image are marked (255;
 * unmarked pixels 0). The plug-in can also create watershed-segmented
 * particles: Assume a landscape of inverted heights, i.e., maxima of the image
 * are now water sinks. For each point in the image, the sink that the water
 * goes to determines which particle it belongs to. When finding maxima (not
 * minima), pixels with a level below the lower threshold can be left
 * unprocessed. Except for segmentation, this plugin works with ROIs, including
 * non-rectangular ROIs. Since this plug-in creates a separate output image it
 * processes only single images or slices, no stacks. Notes: - When using one
 * instance of ImageJ_MaximumFinder for more than one image in parallel threads,
 * all must images have the same width and height. version 09-Nov-2006 Michael
 * Schmid version 21-Nov-2006 Wayne Rasband. Adds "Display Point Selection"
 * option and "Count" output type. version 28-May-2007 Michael Schmid. Preview
 * added, bugfix: minima of calibrated images, uses Arrays.sort version
 * 07-Aug-2007 Fixed a bug that could delete particles when doing watershed
 * segmentation of an EDM. version 21-Apr-2007 Adapted for float instead of
 * 16-bit EDM; correct progress bar on multiple calls version 05-May-2009 Works
 * for images>32768 pixels in width or height version 01-Nov-2009 Bugfix: extra
 * lines in segmented output eliminated; watershed is also faster now Maximum
 * points encoded in long array for sorting instead of separete objects that
 * need gc New output type 'List' version 22-May-2011 Bugfix: Maximum search in
 * EDM and float images with large dynamic range could omit maxima
 */

class ImageJ_MaximumFinder implements ExtendedPlugInFilter, DialogListener {

	// filter params
	/**
	 * maximum height difference between points that are not counted as separate
	 * maxima
	 */
	...
	/**
	 * what type of output to create was chosen in the dialog (see constants
	 * above)
	 */
	...
	private int flags = DOES_ALL | NO_CHANGES | NO_UNDO;// the flags (see
																											// interfaces PlugInFilter
																											// & ExtendedPlugInFilter)
	private boolean thresholded; // whether the input image has a threshold
	private boolean roiSaved; // whether the filter has changed the roi and saved
														// the original roi
	private boolean previewing; // true while dialog is displayed (processing for
															// preview)
	private Vector checkboxes; // a reference to the Checkboxes of the dialog
	private boolean thresholdWarningShown = false; // whether the warning "can't
																									// find minima with
																									// thresholding" has been
																									// shown
	private Label messageArea; // reference to the textmessage area for displaying
															// the number of maxima
	private double progressDone; // for progress bar, fraction of work done so far
	private int nPasses = 0; // for progress bar, how many images to process
														// (sequentially or parallel threads)
	// the following are class variables for having shorter argument lists
	private int width, height; // image dimensions
	private int intEncodeXMask; // needed for encoding x & y in a single int
															// (watershed): mask for x
	private int intEncodeYMask; // needed for encoding x & y in a single int
															// (watershed): mask for y
	private int intEncodeShift; // needed for encoding x & y in a single int
															// (watershed): shift of y
	/**
	 * directions to 8 neighboring pixels, clockwise: 0=North (-y), 1=NE, 2=East
	 * (+x), ... 7=NW
	 */
	private int[] dirOffset; // pixel offsets of neighbor pixels for direct
														// addressing
	private Polygon points; // maxima found by findMaxima() when outputType is
													// POINT_SELECTION
	 ...
	 */
	final static byte MAXIMUM = (byte) 1; // marks local maxima (irrespective of
																				// noise tolerance)
	...
	final static byte MAX_AREA = (byte) 8; // marks areas near a maximum, within
																					// the tolerance
	final static byte EQUAL = (byte) 16; // marks contigous maximum points of
																				// equal level
	final static byte MAX_POINT = (byte) 32; // marks a single point standing for
																						// a maximum
	final static byte ELIMINATED = (byte) 64; // marks maxima that have been
																						// eliminated before watershed
	 ...
	 * @return Code describing supported formats etc. (see
	 *         ij.plugin.filter.PlugInFilter & ExtendedPlugInFilter)
	 */
	...
	public int showDialog(ImagePlus imp, String command, PlugInFilterRunner pfr) {
		...
		ip.resetBinaryThreshold(); // remove invisible threshold set by MakeBinary
																// and Convert to Mask
		...
		if (!dialogItemChanged(gd, null)) // read parameters
			return DONE;
		IJ.register(this.getClass()); // protect static class variables (parameters)
	...
	public boolean dialogItemChanged(GenericDialog gd, AWTEvent e) {
		...
		if (thresholded) useMinThreshold = gd.getNextBoolean();
		else useMinThreshold = false;
		lightBackground = gd.getNextBoolean();
		boolean invertedLut = imp.isInvertedLut();
		if (useMinThreshold && ((invertedLut && !lightBackground) || (!invertedLut &&
			lightBackground)))
		{
			if (!thresholdWarningShown) if (!IJ.showMessageWithCancel("Find Maxima",
				"\"Above Lower Threshold\" option cannot be used\n" +
					"when finding minima (image with light background\n" +
					"or image with dark background and inverting LUT).") && !previewing) return false; // if
																																															// faulty
																																															// input
																																															// is
																																															// not
			...
			((Checkbox) (checkboxes.elementAt(1))).setState(false); // reset "Above
																															// Lower
																															// Threshold"
																															// checkbox
		}
		if (!gd.getPreviewCheckbox().getState()) messageArea.setText(""); // no "nnn
																																			// Maxima"
																																			// message
																																			// when
																																			// not
	 ...
	 * only). Don't call or set nPasses to zero if no progress bar is desired.
	 */
	...
	public void run(ImageProcessor ip) {
		...
		if ((invertedLut && !lightBackground) || (!invertedLut && lightBackground)) {
			threshold = ImageProcessor.NO_THRESHOLD; // don't care about threshold
																								// when finding minima
		...
		outIp = findMaxima(ip, tolerance, threshold, outputType, excludeOnEdges, false); // process
																																											// the
																																											// image
		if (outIp == null) return; // cancelled by user or previewing or no output
																// image
		if (!Prefs.blackBackground) // normally, output has an inverted LUT,
																// "active" pixels black (255) - like a mask
			outIp.invertLut();
		String resultName;
		if (outputType == SEGMENTED) // Segmentation required
			resultName = " Segmented";
		else resultName = " Maxima";
	...
	/**
	 * Finds the image maxima and returns them as a Polygon. There is an example
	 * at http://imagej.nih.gov/ij/macros/js/FindMaxima.js.
	 * 
	 * @param ip The input image
	 * @param tolerance Height tolerance: maxima are accepted only if protruding
	 *          more than this value from the ridge to a higher maximum
	...
	public Polygon getMaxima(ImageProcessor ip, double tolerance, boolean excludeOnEdges) {
		findMaxima(ip, tolerance, ImageProcessor.NO_THRESHOLD, ImageJ_MaximumFinder.POINT_SELECTION,
	 ...
	 * @param tolerance Height tolerance: maxima are accepted only if protruding
	 *          more than this value from the ridge to a higher maximum
	 * @param threshold minimum height of a maximum (uncalibrated); for no minimum
	 *          height set it to ImageProcessor.NO_THRESHOLD
	 * @param outputType What to mark in output image: SINGLE_POINTS, IN_TOLERANCE
	 *          or SEGMENTED. No output image is created for output types
	 ...
	 *         points are set to 255 (Background 0). Pixels outside of the roi of
	 *         the input ip are not set. Returns null if outputType does not
	 *         require an output or if cancelled by escape
	 */
		...
		int outputType, boolean excludeOnEdges, boolean isEDM)
	{
		...
		if (threshold != ImageProcessor.NO_THRESHOLD && ip.getCalibrationTable() != null &&
			threshold > 0 && threshold < ip.getCalibrationTable().length) threshold = ip
				.getCalibrationTable()[(int) threshold]; // convert threshold to
																									// calibrated
		ByteProcessor typeP = new ByteProcessor(width, height); // will be a notepad
																														// for pixel types
		byte[] types = (byte[]) typeP.getPixels();
		float globalMin = Float.MAX_VALUE;
		float globalMax = -Float.MAX_VALUE;
		for (int y = roi.y; y < roi.y + roi.height; y++) { // find local
																												// minimum/maximum now
			for (int x = roi.x; x < roi.x + roi.width; x++) { // ImageStatistics won't
																												// work if we have no
																												// ImagePlus
		...
		if (threshold != ImageProcessor.NO_THRESHOLD) threshold -= (globalMax - globalMin) * 1e-6;// avoid
																																															// rounding
																																															// errors
		// for segmentation, exclusion of edge maxima cannot be done now but has to
		// be done after segmentation:
		boolean excludeEdgesNow = excludeOnEdges && outputType != SEGMENTED;

		if (Thread.currentThread().isInterrupted()) return null;
		// IJ.showStatus("Getting sorted maxima...");
		long[] maxPoints = getSortedMaxPoints(ip, typeP, excludeEdgesNow, isEDM, globalMin, globalMax,
			threshold);
		if (Thread.currentThread().isInterrupted()) return null;
		// IJ.showStatus("Analyzing maxima...");
		float maxSortingError = 0;
		if (ip instanceof FloatProcessor) // sorted sequence may be inaccurate by
																			// this value
		...
		if (outputType == SEGMENTED) {
			// Segmentation required, convert to 8bit (also for 8-bit images, since
			// the calibration
			...
			cleanupMaxima(outIp, typeP, maxPoints); // eliminate all the small maxima
																							// (i.e. those outside MAX_AREA)
			// if (IJ.debugMode) new ImagePlus("pixel types postcleanup",
			// typeP).show();
			// if (IJ.debugMode) new ImagePlus("pre-watershed",
			// outIp.duplicate()).show();
			if (!watershedSegment(outIp)) // do watershed segmentation
				return null; // if user-cancelled, return
			if (!isEDM) cleanupExtraLines(outIp); // eliminate lines due to local
																						// minima (none in EDM)
		...
		if (roi != null) {
			for (int y = 0, i = 0; y < outIp.getHeight(); y++) { // delete everything
																														// outside roi
				for (int x = 0; x < outIp.getWidth(); x++, i++) {
					if (x < roi.x || x >= roi.x + roi.width || y < roi.y || y >= roi.y + roi.height)
						outPixels[i] = (byte) 0;
					else if (mask != null && (mask[x - roi.x + roi.width * (y - roi.y)] == 0)) outPixels[i] =
						(byte) 0;
				}
			}
		}
	...
	/**
	 * Find all local maxima (irrespective whether they finally qualify as maxima
	 * or not)
	 * 
	 * @param ip The image to be analyzed
	 * @param typeP A byte image, same size as ip, where the maximum points are
	 *          marked as MAXIMUM (do not use it as output: for rois, the points
	 *          are shifted w.r.t. the input image)
	 ...
	 *         integer), the value is encoded in the upper 32 bits and the pixel
	 *         offset in the lower 32 bit Note: Do not use the positions of the
	 *         points marked as MAXIMUM in typeP, they are invalid for images with
	 *         a roi.
	 */
		...
		boolean isEDM, float globalMin, float globalMax, double threshold)
	{
		...
		for (int y = roi.y; y < roi.y + roi.height; y++) { // find local maxima now
			...
			for (int x = roi.x, i = x + y * width; x < roi.x + roi.width; x++, i++) { // for
																																								// better
																																								// performance
																																								// with
																																								// rois,
																																								...
																																								// roi
				float v = ip.getPixelValue(x, y);
				float vTrue = isEDM ? trueEdmHeight(x, y, ip) : v; // for EDMs, use
																														// interpolated
																														// ridge height
				...
				boolean isInner = (y != 0 && y != height - 1) && (x != 0 && x != width - 1); // not
																																											// necessary,
																																											// but
																																											// faster
																																											// than
				...
				for (int d = 0; d < 8; d++) { // compare with the 8 neighbor pixels
					if (isInner || isWithin(x, y, d)) {
						...
						float vNeighborTrue = isEDM ? trueEdmHeight(x + DIR_X_OFFSET[d], y + DIR_Y_OFFSET[d],
							ip) : vNeighbor;
		...
		float vFactor = (float) (2e9 / (globalMax - globalMin)); // for converting
																															// float values
																															// into a 32-bit
																															// int
		long[] maxPoints = new long[nMax]; // value (int) is in the upper 32 bit,
																				// pixel offset in the lower
		int iMax = 0;
		for (int y = roi.y; y < roi.y + roi.height; y++) // enter all maxima into an
																											// array
			for (int x = roi.x, p = x + y * width; x < roi.x + roi.width; x++, p++)
				if (types[p] == MAXIMUM) {
					float fValue = isEDM ? trueEdmHeight(x, y, ip) : ip.getPixelValue(x, y);
					int iValue = (int) ((fValue - globalMin) * vFactor); // 32-bit int,
																																// linear
																																// function of
																																// float value
	 ...
	 *          MAX_POINT, etc.
	 * @param maxPoints input: a list of all local maxima, sorted by height. Lower
	 *          32 bits are pixel offset
	 ...
	 *          for maxima having values not deviating from each other by more
	 *          than this (this could be a result of precision loss when sorting
	 *          ints instead of floats, or because sorting does not take the
	 *          height correction in 'trueEdmHeight' into account
	 * @param outputType
	 */
	void analyzeAndMarkMaxima(ImageProcessor ip, ByteProcessor typeP, long[] maxPoints,
		boolean excludeEdgesNow, boolean isEDM, float globalMin, double tolerance, int outputType,
		float maxSortingError)
	{
		byte[] types = (byte[]) typeP.getPixels();
		int nMax = maxPoints.length;
		int[] pList = new int[width * height]; // here we enter points starting from
																						// a maximum
		Vector xyVector = null;
		Roi roi = null;
		boolean displayOrCount = outputType == POINT_SELECTION || outputType == LIST ||
			outputType == COUNT;
		if (displayOrCount) xyVector = new Vector();
		if (imp != null) roi = imp.getRoi();

		for (int iMax = nMax - 1; iMax >= 0; iMax--) { // process all maxima now,
																										// starting from the highest
			if (iMax % 100 == 0 && Thread.currentThread().isInterrupted()) return;
			int offset0 = (int) maxPoints[iMax]; // type cast gets 32 lower bits,
																						// where pixel index is encoded
			// int offset0 = maxPoints[iMax].offset;
			if ((types[offset0] & PROCESSED) != 0) // this maximum has been reached
																							// from another one, skip it
				continue;
			...
			do { // repeat if we have encountered a sortingError
				pList[0] = offset0;
				types[offset0] |= (EQUAL | LISTED); // mark first point as equal height
																						// (to itself) and listed
				int listLen = 1; // number of elements in the list
				int listI = 0; // index of current element in the list
				boolean isEdgeMaximum = (x0 == 0 || x0 == width - 1 || y0 == 0 || y0 == height - 1);
				sortingError = false; // if sorting was inaccurate: a higher maximum was
															// not handled so far
				boolean maxPossible = true; // it may be a true maximum
				double xEqual = x0; // for creating a single point: determine average
														// over the
				...
				do { // while neigbor list is not fully processed (to listLen)
					...
					int x = offset % width;
					int y = offset / width;
					// if(x==18&&y==20)IJ.write("x0,y0="+x0+","+y0+"@18,20;v0="+v0+"
					...
					boolean isInner = (y != 0 && y != height - 1) && (x != 0 && x != width - 1); // not
																																												...
																																												// than
																																												// isWithin
					for (int d = 0; d < 8; d++) { // analyze all neighbors (in 8
																				// directions) at the same level
						int offset2 = offset + dirOffset[d];
						if ((isInner || isWithin(x, y, d)) && (types[offset2] & LISTED) == 0) {
							if ((types[offset2] & PROCESSED) != 0) {
								maxPossible = false; // we have reached a point processed
																			// previously, thus it is no maximum now
								// if(x0<25&&y0<20)IJ.write("x0,y0="+x0+","+y0+":stop at
								// processed neighbor from x,y="+x+","+y+", dir="+d);
								break;
							}
							...
							if (v2 > v0 + maxSortingError) {
								maxPossible = false; // we have reached a higher point, thus it
																			// is no maximum
								// if(x0<25&&y0<20)IJ.write("x0,y0="+x0+","+y0+":stop at higher
								// neighbor from x,y="+x+","+y+",
								// dir="+d+",value,value2,v2-v="+v0+","+v2+","+(v2-v0));
								break;
							}
							else if (v2 >= v0 - (float) tolerance) {
								if (v2 > v0) { // maybe this point should have been treated
																// earlier
									sortingError = true;
									offset0 = offset2;
									v0 = v2;
								...
								if (v2 == v0) { // prepare finding center of equal points (in
				...
				if (sortingError) { // if x0,y0 was not the true maximum but we have
														// reached a higher one
					...
					for (listI = 0; listI < listLen; listI++) {
						...
						if (maxPossible) {
							...
							if ((types[offset] & EQUAL) != 0) {
								double dist2 = (xEqual - x) * (double) (xEqual - x) + (yEqual - y) *
									(double) (yEqual - y);
								if (dist2 < minDist2) {
									minDist2 = dist2; // this could be the best "single maximum"
																		// point
									nearestI = listI;
								}
							}
			...
			while (sortingError); // redo if we have encountered a higher maximum:
														// handle it now.
		} // for all maxima iMax
			...
			if (outputType == POINT_SELECTION && npoints > 0) {
				...
				if ((measurements & Measurements.LABELS) != 0) {
					...
					if (imp.getStackSize() > 1) {
						...
						if (label != null && !label.equals("")) s += colon + label;
						else s += colon + currentSlice;
					}
	 ...
	 * threshold 0) and mark maximum areas as 255. For use as input for watershed
	 * segmentation
	 * 
	 ...
	 */
	ByteProcessor make8bit(ImageProcessor ip, ByteProcessor typeP, boolean isEDM, float globalMin,
		float globalMax, double threshold)
	{
		byte[] types = (byte[]) typeP.getPixels();
		double minValue;
		if (isEDM) {
		...
		// IJ.write("min,max="+minValue+","+globalMax+"; offset,
		// 1/factor="+offset+", "+(1./factor));
		if (isEDM && factor > 1) factor = 1; // with EDM, no better resolution
		...
		for (int y = 0, i = 0; y < height; y++) {
			for (int x = 0; x < width; x++, i++) {
				...
				if (threshold != ImageProcessor.NO_THRESHOLD && rawValue < threshold) pixels[i] = (byte) 0;
				else if ((types[i] & MAX_AREA) != 0) pixels[i] = (byte) 255; // prepare
																																			// watershed
																																			// by
																																			// setting
																																			// "true"
	 ...
	 * Distance Map. This is needed since the point sampled is not necessarily at
	 * the highest position. For simplicity, we don't care about the Sqrt(5)
	...
	float trueEdmHeight(int x, int y, ImageProcessor ip) {
		...
		if (x == 0 || y == 0 || x == xmax || y == ymax || v == 0) {
			...
			float trueH = v + 0.5f * SQRT2; // true height can never by higher than
																			// this
			...
			for (int d = 0; d < 4; d++) { // for all directions halfway around:
				...
				h += (d % 2 == 0) ? 1 : SQRT2; // in diagonal directions, distance is
																				// sqrt2
				if (trueH > h) trueH = h;
			}
	 ...
	 * maximum, explore the surrounding down to successively lower levels until a
	 * marked maximum is touched (or the plateau of a previously eliminated
	 * maximum leads to a marked maximum). Then set all the points above this
	 * value to this value
	 * 
	 * @param outIp the image containing the pixel values
	 * @param typeP the types of the pixels are marked here
	 * @param maxPoints array containing the coordinates of all maxima that might
	 *          be relevant
	 */
	void cleanupMaxima(ByteProcessor outIp, ByteProcessor typeP, long[] maxPoints) {
		...
		for (int iMax = nMax - 1; iMax >= 0; iMax--) {
			int offset0 = (int) maxPoints[iMax]; // type cast gets lower 32 bits where
																						// pixel offset is encoded
			...
			// if (xList[0]==122) IJ.write("max#"+iMax+" at
			// x,y="+xList[0]+","+yList[0]+"; level="+level);
			...
			while (!saddleFound && loLevel > 0) {
				...
				do { // for all pixels listed so far
					...
					for (int d = 0; d < 8; d++) { // analyze all neighbors (in 8
						...
						int offset2 = offset + dirOffset[d];
						if ((isInner || isWithin(x, y, d)) && (types[offset2] & LISTED) == 0) {
							if ((types[offset2] & MAX_AREA) != 0 || (((types[offset2] & ELIMINATED) != 0) &&
								(pixels[offset2] & 255) >= loLevel))
							{
								saddleFound = true; // we have reached a point touching a "true"
																		// maximum...
								// if (xList[0]==122) IJ.write("saddle found at
								// level="+loLevel+"; x,y="+xList[listI]+","+yList[listI]+",
								// dir="+d);
			...
			for (listI = 0; listI < listLen; listI++) // reset attribute since we may
																								// come to this place again
				types[pList[listI]] &= ~LISTED;
			for (listI = 0; listI < lastLen; listI++) { // for all points higher than
																									// the level of the saddle
																									// point
				int offset = pList[listI];
				pixels[offset] = (byte) loLevel; // set pixel value to the level of the
																					// saddle point
				types[offset] |= ELIMINATED; // mark as processed: there can't be a
																			// local maximum in this area
			}
		} // for all maxima iMax
	} // void cleanupMaxima
	 ...
	 * patches, single dots and lines ending somewhere within a segmented particle
			...
			for (int x = 0; x < width; x++, i++) {
				...
				if (v != (byte) 255 && v != 0) {
					...
					if (nRadii == 0) // single point or foreground patch?
						pixels[i] = (byte) 255;
					else if (nRadii == 1) removeLineFrom(pixels, x, y);
				} // if v<255 && v>0
			} // for x
		} // for y
	 ...
	 * ("non-white") are considered foreground. Edge pixels are considered
	 * foreground.
	 * 
	 * @param ip
	 ...
	 *         point is embedded in either foreground or background
	 */
	int nRadii(byte[] pixels, int x, int y) {
		...
		boolean isInner = (y != 0 && y != height - 1) && (x != 0 && x != width - 1); // not
																																									...
																																									// than
																																									// isWithin
		for (int d = 0; d < 8; d++) { // walk around the point and note every
																	// no-line->line transition
			boolean pixelSet = prevPixelSet;
			if (isInner || isWithin(x, y, d)) {
				boolean isSet = (pixels[offset + dirOffset[d]] != (byte) 255);
				if ((d & 1) == 0) pixelSet = isSet; // non-diagonal directions: always
																						// regarded
	...
	/**
	 * after watershed, set all pixels in the background and segmentation lines to
	 * 0
	 */
	 ...
	 * 
	 * @param typeP Here the pixel types of the original image are noted, pixels
	 *          with bit MAX_AREA at the edge are considered indicators of an edge
	 *          maximum.
	 * @param ip the image resulting from watershed segmentaiton (foreground
	 *          pixels, i.e. particles, are 255, background 0)
	 */
	void deleteEdgeParticles(ByteProcessor ip, ByteProcessor typeP) {
		...
		for (int x = 0; x < width; x++) {
			int y = 0;
			if ((types[x + y * width] & MAX_AREA) != 0 && pixels[x + y * width] != 0) deleteParticle(x, y,
				ip, wand);
			y = height - 1;
			if ((types[x + y * width] & MAX_AREA) != 0 && pixels[x + y * width] != 0) deleteParticle(x, y,
				ip, wand);
	...
	/**
	 * delete a particle (set from value 255 to current fill value). Position x,y
	 * must be within the particle
	 */
	 ...
	 * maxima other than the marked ones. Local minima will lead to artifacts that
	 * can be removed later. On output, all particles will be set to 255,
	 * segmentation lines remain at their old value.
	 * 
	 * @param ip The byteProcessor containing the image, with size given by the
	 *          class variables width and height
	 * @return false if canceled by the user (note: can be cancelled only if
	 *         called by "run" with a known ImagePlus)
	 */
	private boolean watershedSegment(ByteProcessor ip) {
		...
		byte[] pixels = (byte[]) ip.getPixels();
		// Create an array with the coordinates of all points between value 1 and
		// 254
		...
		int[] coordinates = new int[arraySize]; // from pixel coordinates, low bits
																						// x, high bits y
		int highestValue = 0;
		int maxBinSize = 0;
		int offset = 0;
		...
		// Create an array of the points (pixel offsets) that we set to 255 in one
		// pass.
		...
		// At each level, dilate the particle (set pixels to 255), constrained to
		// pixels
		...
		for (int level = highestValue; level >= 1; level--) {
			int remaining = histogram[level]; // number of points in the level that
			...
			while (remaining > 0 && idle < 8) {
				...
				do { // expand each level in 8 directions
					int n = processLevel(directionSequence[dIndex % 8], ip, table, levelStart[level],
						remaining, coordinates, setPointList);
			...
			if (remaining > 0 && level > 1) { // any pixels that we have not reached?
				...
				// in principle we should add all unprocessed pixels of this level to
				// the
				// tasklist of the next level. This would make it very slow for some
				// images,
				// however. Thus we only add the pixels if they are at the border (of
				// the
				// image or a thresholded area) and correct unprocessed pixels at the
				// very
				...
				if (nextLevel > 0) {
					...
					for (int i = 0, p = levelStart[level]; i < remaining; i++, p++) {
						...
						if (x == 0 || y == 0 || x == width - 1 || y == height - 1) addToNext = true; // image
																																													// border
					...
					// IJ.log("level="+level+": add
					// "+(newNextLevelEnd-levelStart[nextLevel+1])+" points to
					// "+nextLevel);
					// tasklist for the next level to process becomes longer by this:
					histogram[nextLevel] = newNextLevelEnd - levelStart[nextLevel];
				}
			}
			if (debug && (level > 170 || level > 100 && level < 110 || level < 10)) movie.addSlice(
				"level " + level, ip.duplicate());
		}
	...
	/**
	 * dilate the UEP on one level by one pixel in the direction specified by
	 * step, i.e., set pixels to 255
	 * 
	 ...
	 * @param pixelPointers[] list of pixel coordinates (x+y*width) sorted by
	 *          level (in sequence of y, x within each level)
	 * @param xCoordinates list of x Coorinates for the current level only (no
	 *          offset levelStart)
	 * @return number of pixels that have been changed
	 */
	private int processLevel(int pass, ImageProcessor ip, int[] fateTable, int levelStart,
		int levelNPoints, int[] coordinates, int[] setPointList)
	{
		int xmax = width - 1;
		int ymax = height - 1;
		byte[] pixels = (byte[]) ip.getPixels();
		...
		for (int i = 0, p = levelStart; i < levelNPoints; i++, p++) {
			...
			int mask = 1 << pass;
			if ((fateTable[index] & mask) == mask) setPointList[nChanged++] = offset; // remember
																																								// to
																																								// set
																																								// pixel
																																								// to
																																								// 255
			else coordinates[levelStart + (nUnchanged++)] = xy; // keep this pixel for
																													// future passes
	...
	private int[] makeFateTable() {
		...
		for (int item = 0; item < 256; item++) { // dissect into pixels
			...
			for (int i = 0, mask = 1; i < 8; i++) { // we dilate in the direction
																							// opposite to the direction of
																							// the existing neighbors
				if (isSet[(i + 4) % 8]) table[item] |= mask;
				mask *= 2;
			}
			for (int i = 0; i < 8; i += 2) // if side pixels are set, for counting
																			// transitions it is as good as if the
			...
			if (transitions >= 4) { // if neighbors contain more than one region,
															// dilation ito this pixel is forbidden
				table[item] = 0;
			}
			else {}
		}
	 ...
	 * order: 0=(x,y-1), 1=(x+1,y-1), ... 7=(x-1,y) Also creates further class
	 * variables: width, height, and the following three values needed for storing
	 * coordinates in single ints for watershed: intEncodeXMask, intEncodeYMask
	 * and intEncodeShift. E.g., for width between 129 and 256, xMask=0xff and
	 * yMask = 0xffffff00 are bitwise masks for x and y, respectively, and shift=8
	 * is the bit shift to get y from the y-masked value Returns as class
	 * variables: the arrays of the offsets to the 8 neighboring pixels and the
	 * array maskAndShift for watershed
	 */
	void makeDirectionOffsets(ImageProcessor ip) {
		...
		// shift="+shift);
		dirOffset = new int[] { -width, -width + 1, +1, +width + 1, +width, +width - 1, -1, -width -
			1 };
		// dirOffset is created last, so check for it being null before
		// makeDirectionOffsets
		// (in case we have multiple threads using the same ImageJ_MaximumFinder)
	}

	/**
	 * returns whether the neighbor in a given direction is within the image NOTE:
	 * it is assumed that the pixel x,y itself is within the image! Uses class
	 * variables width, height: dimensions of the image
	 * 
