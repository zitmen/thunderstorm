...
public class DaostormCalibration extends DefocusCalibration {
	...
	public DaostormCalibration(double angle, Homography.TransformationMatrix biplaneTransformation,
		double w01, double a1, double b1, double c1, double d1, double w02, double a2, double b2,
		double c2, double d2)
	{
		super(DefocusFunctionSqrt.name, angle, biplaneTransformation, w01, a1, b1, c1, d1, w02, a2, b2,
			c2, d2);
		s1Par = null;
		s2Par = null;
	}
	...
	{
		super(DefocusFunctionSqrt.name, angle, biplaneTransformation, sigma1Params.getW0(), sigma1Params
	...
	public double evalDefocus(double z, double w0, double a, double b, double c, double d) {
		return (w0 / 2.0 * sqrt(1 + sqr((z - c) / d) + a * pow((z - c) / d, 3) + b * pow((z - c) / d,
			4)));
	}

	private double dw(double z, double w0, double a, double b, double c, double d) {
		double zsubz0 = z - c;
		return (w0 * (2 * zsubz0 / sqr(d) + 3 * a * sqr(zsubz0) / pow(d, 3) + 4 * b * pow(zsubz0, 3) /
			pow(d, 4))) / (2 * sqrt(1 + sqr(zsubz0) / sqr(d) + a * pow(zsubz0, 3) / pow(d, 3) + b * pow(
				zsubz0, 4) / pow(d, 4)));
	}
