...
import static cz.cuni.lf1.lge.ThunderSTORM.estimators.PSF.PSFModel.Params.LABEL_SIGMA3;
import static cz.cuni.lf1.lge.ThunderSTORM.estimators.PSF.PSFModel.Params.LABEL_SIGMA4;
import static cz.cuni.lf1.lge.ThunderSTORM.estimators.PSF.PSFModel.Params.LABEL_Z;
import static cz.cuni.lf1.lge.ThunderSTORM.estimators.PSF.PSFModel.Params.LABEL_Z_REL;
...
public class MoleculeDescriptor implements Cloneable {
	...
	private HashMap<String, IndexAndColumn> paramNames; // duplicate of arrays
																											// `names` and `indices`,
	...
	public Vector<String> names; // names of the parameters in Molecules.values -
																// the order correspond to the `indices`
	...
	// note: in all the constructors, if the order of indices is simply
	// {0,1,2...,names.length-1},
	// then you can call for example MoleculeDescriptor(names, null) and the
	// constructor fills
	...
	public MoleculeDescriptor clone() {
		...
		for (int i = 0, im = names.size(); i < im; i++) {
			if (names.elementAt(i) != null) {
				clonedNames[i] = new String(names.elementAt(i).toCharArray()); // don't
																																				// know
																																				// how
																																				// else
																																				// to
	...
	public static Pair<String, Units> parseColumnLabel(String label) {
		...
		if ((start < 0) || (end < 0) || (start > end)) {
			...
			return new Pair<String, Units>(label.substring(0, start).trim(), Units.fromString(label
	...
	public static class Fitting {
		...
		/**
		 * Returns lateral uncertainty in nanometers. Thompson, et al. 2002;
		 * corrected by Mortensen, et al. 2010 (16/9 instead of 1 to not
		 * underestimate) compensation for EM gain worked out by Quan, et al. 2010
		 */
		public static double uncertaintyXY(Molecule molecule) throws UncertaintyNotApplicableException {
			...
			if (molecule.hasParam(LABEL_SIGMA)) { // 2D (symmetric Gaussian)
				psfSigma2 = molecule.getParam(LABEL_SIGMA, Units.NANOMETER) * molecule.getParam(LABEL_SIGMA,
					Units.NANOMETER);
			}
			else if (molecule.hasParam(LABEL_SIGMA1) && molecule.hasParam(LABEL_SIGMA2)) { // 3D
																																											// astigmatism
																																											// (elliptic
																																											// Gaussian)
				psfSigma2 = molecule.getParam(LABEL_SIGMA1, Units.NANOMETER) * molecule.getParam(
					LABEL_SIGMA2, Units.NANOMETER);
			}
			else {
			...
			double psfPhotons = molecule.getParam(LABEL_INTENSITY, Units.PHOTON) * CameraSetupPlugIn
				.getQuantumEfficiency();
			double bkgStd = molecule.getParam(LABEL_BACKGROUND, Units.PHOTON) * CameraSetupPlugIn
				.getQuantumEfficiency() + readout;
			double tau = 0.0;

			String fittingMethod = null;
			MeasurementProtocol protocol = IJResultsTable.getResultsTable().getMeasurementProtocol();
			if (protocol.analysisEstimator instanceof EllipticGaussianEstimatorUI) { // 3D?
																																								// elliptic
																																								// Gauss
																																								// (astigmatism)
				fittingMethod = ((EllipticGaussianEstimatorUI) protocol.analysisEstimator).getMethod();
				DaostormCalibration cal = ((EllipticGaussianEstimatorUI) protocol.analysisEstimator)
					.getDaoCalibration();
				double l2 = abs(cal.getC1() * cal.getC2());
				double d2 = abs(cal.getD1() * cal.getD2());
				tau = 2.0 * PI * bkgStd * bkgStd * (psfSigma2 * (1.0 + l2 / d2) + pixelSize * pixelSize /
					12.0) / (psfPhotons * pixelSize * pixelSize);
			}
			else if (protocol.analysisEstimator instanceof SymmetricGaussianEstimatorUI) { // 2D?
																																											// Gauss
																																											// or
																																											// IntGauss
				fittingMethod = ((SymmetricGaussianEstimatorUI) protocol.analysisEstimator).getMethod();
				tau = 2.0 * PI * bkgStd * bkgStd * (psfSigma2 + pixelSize * pixelSize / 12.0) /
					(psfPhotons * pixelSize * pixelSize);
			}

			if (fittingMethod != null) {
				if (fittingMethod.equals(SymmetricGaussianEstimatorUI.MLE) || fittingMethod.equals(
					SymmetricGaussianEstimatorUI.WLSQ))
				{
					// Note: here we don't distinguish between MLE and WLSQ, however,
					// there is a difference!
					// For details, see supplementary note for Mortensen 2010, Eq. (46),
					// which shows an extra offset-dependent term!
					return sqrt((gain * psfSigma2 + pixelSize * pixelSize / 12.0) / psfPhotons * (1.0 + 4.0 *
						tau + sqrt(2.0 * tau / (1 + 4.0 * tau))));
				}
				else if (fittingMethod.equals(SymmetricGaussianEstimatorUI.LSQ)) {
					return sqrt((gain * psfSigma2 + pixelSize * pixelSize / 12.0) / psfPhotons * (16.0 / 9.0 +
						4.0 * tau));
				}
			}
			throw new UncertaintyNotApplicableException(
				"Unsupported fitting method! Was the measurement protocol loaded properly?");
		}

		/**
		 * Returns axial uncertainty in nanometers. Here we always assume MLE fit
		 * --> the math has been worked out by Rieger, et al. 2014. When we
		 * distinguish between (W)LSQ and MLE, we could work out the math for LSQ in
		 * a similar fashion as Thompson, et al. 2002 and/or Mortensen, et al. 2010.
		 */
		public static double uncertaintyZ(Molecule molecule) throws UncertaintyNotApplicableException {
			MeasurementProtocol protocol = IJResultsTable.getResultsTable().getMeasurementProtocol();
			if (!(protocol.analysisEstimator instanceof EllipticGaussianEstimatorUI) || !(molecule
				.hasParam(LABEL_SIGMA1) && molecule.hasParam(LABEL_SIGMA2)))
			{
				throw new UncertaintyNotApplicableException(
					"Axial uncertainty cannot be calculated for 2D estimate (missing sigma1, sigma2)!");
			}
			...
			double zCoord = molecule.hasParam(LABEL_Z_REL) ? molecule.getParam(LABEL_Z_REL,
				Units.NANOMETER) : molecule.getParam(LABEL_Z, Units.NANOMETER);

			DaostormCalibration cal = ((EllipticGaussianEstimatorUI) protocol.analysisEstimator)
				.getDaoCalibration();
			double l2 = abs(cal.getC1() * cal.getC2());
			double d2 = abs(cal.getD1() * cal.getD2());
			double tau = 2.0 * PI * bkgStd * bkgStd * (psfSigma1 * psfSigma2 * (1.0 + l2 / d2) +
				pixelSize * pixelSize / 12.0) / (psfPhotons * pixelSize * pixelSize);
			double zLimit = sqrt(l2 + d2); // singularity in CRLB - do not evaluate at
																			// positions beyond
			if (abs(zCoord) >= zLimit) return Double.POSITIVE_INFINITY;
			//
			double compensation = (sqrt(gain * psfSigma1 * psfSigma1 + pixelSize * pixelSize / 12.0) /
				psfSigma1 + sqrt(gain * psfSigma2 * psfSigma2 + pixelSize * pixelSize / 12.0) / psfSigma2) /
			...
			if (fittingMethod != null) {
					...
					SymmetricGaussianEstimatorUI.WLSQ))
				{
					...
					// For details, see supplementary note for Mortensen 2010, Eq. (46),
					// which shows an extra offset-dependent term!
					stdSigma = sqrt(1 + 8.0 * tau + sqrt(9.0 * tau / (1.0 + 4.0 * tau))) * compensation /
						sqrt(psfPhotons);
					...
					throw new UncertaintyNotApplicableException(
						"Unsupported (unknown) fitting method! Was the measurement protocol loaded properly?");
				}
			}
			else {
				throw new UncertaintyNotApplicableException(
					"Unsupported (empty) fitting method! Was the measurement protocol loaded properly?");
			}
			//
			double Fsq = 4.0 * l2 * zCoord * zCoord / sqr(l2 + d2 + zCoord * zCoord);
			double stdF = sqrt(1.0 - Fsq) * stdSigma;
			double stdZ = stdF * sqr(l2 + d2 + zCoord * zCoord) / (2.0 * sqrt(l2) * (l2 + d2 - zCoord *
				zCoord));
			return stdZ;
		}
	...
	public static enum Units {
			...
			MICROMETER("um"), NANOMETER("nm"), PIXEL("px"), MICROMETER_SQUARED("um^2"), NANOMETER_SQUARED(
				"nm^2"), PIXEL_SQUARED("px^2"), DIGITAL("ADU"), PHOTON("photon"), DEGREE("deg"), RADIAN(
					"rad"), UNITLESS("");
		...
		public double convertTo(Units target, double value) {
			...
			throw new UnsupportedOperationException("Conversion from " + toString() + " to " + target
		...
		public static Vector<Units> getCompatibleUnits(Units selected) {
			if ((groups == null) || (groupMap == null)) {
				groups = new Vector[5];
				groups[0] = new Vector<Units>(Arrays.asList(new Units[] { Units.PIXEL, Units.NANOMETER,
					Units.MICROMETER }));
				groups[1] = new Vector<Units>(Arrays.asList(new Units[] { Units.PIXEL_SQUARED,
					Units.NANOMETER_SQUARED, Units.MICROMETER_SQUARED }));
		...
		public static Units getAnalogUnits(Units digitalUnit) {
			switch (digitalUnit) {
				case PIXEL:
				case MICROMETER: // this is of course analog unit, but we need all units
													// to be the same
					return NANOMETER;
				case PIXEL_SQUARED:
				case MICROMETER_SQUARED: // this is of course analog unit, but we need
																	// all units to be the same
	...
	public static enum MergingOperations {
		...
		public static void merge(Molecule molecule, List<Molecule> detections, String paramName)
			throws Exception
		{
			if (allParams == null) {
				init();
			...
			if (allParams.containsKey(paramName)) {
				switch (allParams.get(paramName)) {
					case MIN:
						molecule.setParam(paramName, VectorMath.min(Molecule.extractParamToArray(detections,
							paramName)));
						break;
					case MAX:
						molecule.setParam(paramName, VectorMath.max(Molecule.extractParamToArray(detections,
							paramName)));
						break;
					case SUM:
						molecule.setParam(paramName, VectorMath.sum(Molecule.extractParamToArray(detections,
							paramName)));
						break;
					case MEAN:
						molecule.setParam(paramName, VectorMath.mean(Molecule.extractParamToArray(detections,
							paramName)));
						break;
					case COUNT:
