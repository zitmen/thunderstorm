...
/**
 * A squared polynomial function y = w0*sqrt(1 + ((z-c)/d)^2 + a*((z-c)/d)^3 +
 * b*((z-c)/d)^4) Note: to avoid problems with over-fitting, the third degree of
 * the polynomial is set to zero and the fourth degree is forced non-negative
 * TODO: this can be solved using regularization put on parameters `a` and `b`,
 * but at the moment I am not sure how to do it in a simple way using the Apache
...
public class DefocusFunctionSqrt extends DefocusFunction {
	...
	public DefocusFunctionSqrt(double w0, double a, double b, double c, double d,
		boolean scaledToNm)
	{
	...
	public ParametricUnivariateFunction getFittingFunction() {
		return new ParametricUnivariateFunction() {
			...
			public double[] gradient(double x, double... params) {
				...
				// Partial derivatives of: w0*sqrt(1 + ((z-c)/d)^2 + a*((z-c)/d)^3 +
				// b*((z-c)/d)^4)
				double dd = sqrt(1 + sqr(xsubx0 / trans[4]) + trans[2] * pow(xsubx0 / trans[4], 3) +
					trans[3] * pow(xsubx0 / trans[4], 4));
				gradients[0] = 0.50 * dd;
				gradients[1] = 0.25 * trans[0] * (-2 * xsubx0 / sqr(trans[4]) - 3 * trans[2] * sqr(xsubx0) /
					pow(trans[4], 3) - 4 * trans[3] * pow(xsubx0, 3) / pow(trans[4], 4)) / dd;
				gradients[2] = 0.25 * trans[0] * pow(xsubx0, 3) / pow(trans[4], 3) / dd;
				gradients[3] = 0.25 * trans[0] * pow(xsubx0, 4) / pow(trans[4], 4) / dd;
				gradients[4] = 0.25 * trans[0] * (-2 * sqr(xsubx0) / pow(trans[4], 3) - 3 * trans[2] * pow(
					xsubx0, 3) / pow(trans[4], 4) - 4 * trans[3] * pow(xsubx0, 4) / pow(trans[4], 5)) / dd;
				return gradients;
			}
		};
	}
	...
	@Override
	public DefocusFunction getNewInstance(double w0, double a, double b, double c, double d,
		boolean scaledToNm)
	{
	...
	@Override
	public DefocusCalibration getCalibration(double angle,
		Homography.TransformationMatrix biplaneTransformation, DefocusFunction polynomS1Final,
		DefocusFunction polynomS2Final)
	{
	...
	public String toString() {
		return String.format(Locale.ENGLISH,
			"%e*sqrt(1 + ((z%+g)/%e)^2 %+e*((z%+g)/%e)^3 %+e*((z%+g)/%e)^4)", w0, -c, d, a, -c, d, b, -c,
			d);
	}
}
