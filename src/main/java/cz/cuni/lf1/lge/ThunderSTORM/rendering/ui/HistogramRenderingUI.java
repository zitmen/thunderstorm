...
public class HistogramRenderingUI extends AbstractRenderingUI {
	...
	private void initPars() {
		...
		dx = parameters.createDoubleField("dx", DoubleValidatorFactory.positiveNonZero(), 20,
			avgGTZeroCondition);
		dz = parameters.createDoubleField("dz", DoubleValidatorFactory.positiveNonZero(), 100,
			threeDAndAvgGTZeroCondition);
	...
	public IncrementalRenderingMethod getMethod() {
		if (threeD.getValue()) {
			...
			return new HistogramRendering.Builder().roi(left, left + sizeX, top, top + sizeY).resolution(
				1 / magnification.getValue()).average(avg.getValue()).defaultDX(dx.getValue() /
					CameraSetupPlugIn.getPixelSize()).forceDefaultDX(forceDx.getValue()).defaultDZ(dz
						.getValue()).forceDefaultDZ(forceDz.getValue()).colorize(colorize.getValue())
				.colorizationLUT(lutPicker.getLut(colorizationLut.getValue())).zRange(zRange.from,
					zRange.to, zRange.step).build();
		}
		else {
			return new HistogramRendering.Builder().roi(left, left + sizeX, top, top + sizeY).resolution(
				1 / magnification.getValue()).average(avg.getValue()).defaultDX(dx.getValue() /
					CameraSetupPlugIn.getPixelSize()).forceDefaultDX(forceDx.getValue()).build();
		}
	}
}
