...
public class GenericTableModel extends AbstractTableModel implements Cloneable {
	...
	public void sortTableByColumn(String colname) {
		...
		ArrayIndexComparator cmp = new ArrayIndexComparator(getColumnAsDoubles(colname,
			Units.UNITLESS));
	...
	// this is in fact label for column heading, which the TableView asks for this
	// class
	@Override
	...
	public synchronized int addRow(Molecule mol) {
		if (rows.isEmpty()) { // if the table is empty, use descriptor from the
													// molecule instance
	...
	public void calculateUncertaintyXY()
		throws MoleculeDescriptor.Fitting.UncertaintyNotApplicableException
	{
		// Note: even though that the lateral uncertainty can be calculated in
		// pixels,
