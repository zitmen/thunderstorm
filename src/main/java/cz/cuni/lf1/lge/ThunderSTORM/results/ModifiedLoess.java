...
/**
 * Implements the <a href="http://en.wikipedia.org/wiki/Local_regression"> Local
 * Regression Algorithm</a> (also Loess, Lowess) for interpolation of real
 ...
 * @since 2.0
 */
public class ModifiedLoess implements UnivariateInterpolator, Serializable {
	...
	/**
	 * The bandwidth parameter: when computing the loess fit at a particular
	 * point, this fraction of source points closest to the current point is taken
	 * into account for computing a least-squares regression.
	 * <p/>
	 * A sensible value is usually 0.25 to 0.5.
	 */
	private final double bandwidth;
	/**
	 * The number of robustness iterations parameter: this many robustness
	 * iterations are done.
	 * <p/>
	 * A sensible value is usually 0 (just the initial fit without any robustness
	 * iterations) to 4.
	 */
	private final int robustnessIters;
	/**
	 * If the median residual at a certain robustness iteration is less than this
	 * amount, no more iterations are done.
	 */
	private final double accuracy;

	/**
	 * Constructs a new {@link LoessInterpolator} with a bandwidth of
	 * {@link #DEFAULT_BANDWIDTH}, {@link #DEFAULT_ROBUSTNESS_ITERS} robustness
	 * iterations and an accuracy of {#link #DEFAULT_ACCURACY}. See
	 * {@link #LoessInterpolator(double, int, double)} for an explanation of the
	 * parameters.
	 */
	public ModifiedLoess() {
	...
	/**
	 * Construct a new {@link LoessInterpolator} with given bandwidth and number
	 * of robustness iterations.
	 * <p>
	 * Calling this constructor is equivalent to calling {link
	 * {@link #LoessInterpolator(double, int, double) LoessInterpolator(bandwidth,
	 * robustnessIters, LoessInterpolator.DEFAULT_ACCURACY)}
	 * </p>
	 *
	 * @param bandwidth when computing the loess fit at a particular point, this
	 *          fraction of source points closest to the current point is taken
	 *          into account for computing a least-squares regression.</br>
	 ...
	 */
	public ModifiedLoess(double bandwidth, int robustnessIters) {
		this(bandwidth, robustnessIters, DEFAULT_ACCURACY);
	}

	/**
	 * Construct a new {@link LoessInterpolator} with given bandwidth, number of
	 * robustness iterations and accuracy.
	 *
	 * @param bandwidth when computing the loess fit at a particular point, this
	 *          fraction of source points closest to the current point is taken
	 *          into account for computing a least-squares regression.</br>
	 ...
	 *          robustness iterations) to 4, the default value is
	 *          {@link #DEFAULT_ROBUSTNESS_ITERS}.
	 * @param accuracy If the median residual at a certain robustness iteration is
	...
	/**
	 * Compute an interpolating function by performing a loess fit on the data at
	 * the original abscissae and then building a cubic spline with a
	 * {@link org.apache.commons.math3.analysis.interpolation.SplineInterpolator}
	 * on the resulting fit.
	 *
	 * @param xval the arguments for the interpolation points
	 * @param yval the values for the interpolation points
	 * @return A cubic spline built upon a loess fit to the data at the original
	 *         abscissae
	 ...
	 * @throws NotFiniteNumberException if any of the arguments and values are not
	 *           finite real numbers.
	...
	/**
	 * Given an index interval into xval that embraces a certain number of points
	 * closest to {@code xval[i-1]}, update the interval so that it embraces the
	 * same number of points closest to {@code xval[i]}, ignoring zero weights.
	 *
	...
	private static void updateBandwidthInterval(final double[] xval, final double[] weights,
		final int i, final int[] bandwidthInterval)
	{
	...
	/**
	 * Compute the <a href=
	 * "http://en.wikipedia.org/wiki/Local_regression#Weight_function">tricube</a>
	 * weight function
	 *
	 * @param x Argument.
	 * @return <code>(1 - |x|<sup>3</sup>)<sup>3</sup></code> for |x| &lt; 1, 0
	 *         otherwise.
	 */
	 ...
	 * @throws org.apache.commons.math3.exception.NotFiniteNumberException if one
	 *           of the values is not a finite real number.
	 */
