...
public class AnalysisOptionsDialog extends JDialog implements ActionListener {
	...
	private Semaphore semaphore; // ensures waiting for a dialog without the
																// dialog being modal!
	 ...
	 */
	public AnalysisOptionsDialog(final ImagePlus imp, String title, List<IFilterUI> filters,
		List<IDetectorUI> detectors, List<IEstimatorUI> estimators, List<IRendererUI> renderers)
	{
		//
	...
	/**
	 * Action handler. There are three possible actions. Canceling the analysis,
	 * confirming the settings of analysis, and preview the results of analysis on
	 * a single frame selected in active {@code ImagePlus} window.
	 *
	 * @param e event object holding the action details. It gets processed as
	 *          follows:
	 *          <ul>
	 *          <li>actionCommand == "Cancel": cancel the analysis</li>
	 *          <li>actionCommand == "Ok": confirm the settings and run the
	 *          analysis</li>
	 *          <li>actionCommand == "Preview": preview the results of analysis
	 *          with the current selected settings on a single frame</li>
	 *          </ul>
	 */
	@Override
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("Cancel")) {
			...
			try {
				...
				saveSelectedModuleIndexesToPrefs(activeFilterIndex, activeDetectorIndex,
					activeEstimatorIndex, activeRendererIndex);
			}
			catch (Exception ex) {
			...
			try {
				...
				Thresholder.setActiveFilter(activeFilterIndex); // !! must be called
																												// before any threshold
																												// is evaluated !!
				Thresholder.parseThreshold(allDetectors.get(activeDetectorIndex)
					.getThreadLocalImplementation().getThresholdFormula());
			}
			catch (Exception ex) {
			...
			previewFuture = previewThreadRunner.submit(new Runnable() {
				...
				public void run() {
					try {
						...
						FloatProcessor fp = subtract((FloatProcessor) processor.crop().convertToFloat(),
						...
						FloatProcessor filtered = allFilters.get(activeFilterIndex)
							.getThreadLocalImplementation().filterImage(fp);
						new ImagePlus("Filtered frame " + Integer.toString(imp.getSlice()), filtered).show();
						GUI.checkIJEscapePressed();
						IDetector detector = allDetectors.get(activeDetectorIndex)
							.getThreadLocalImplementation();
						List<Point> detections = Point.applyRoiMask(imp.getRoi(), detector
							.detectMoleculeCandidates(filtered));
						ij.measure.ResultsTable tbl = ij.measure.ResultsTable.getResultsTable();
						tbl.reset();
						tbl.incrementCounter();
						tbl.addValue("Threshold value for frame " + Integer.toString(imp.getSlice()), detector
							.getThresholdValue());
						tbl.show("Results");
						GUI.checkIJEscapePressed();
						List<Molecule> results = allEstimators.get(activeEstimatorIndex)
							.getThreadLocalImplementation().estimateParameters(fp, detections);
						GUI.checkIJEscapePressed();
						//
						ImagePlus impPreview = new ImagePlus("Detections in frame " + Integer.toString(imp
							.getSlice()), processor);
						RenderingOverlay.showPointsInImage(impPreview, Molecule.extractParamToArray(results,
							PSFModel.Params.LABEL_X), Molecule.extractParamToArray(results,
								PSFModel.Params.LABEL_Y), Color.red, RenderingOverlay.MARKER_CROSS);
						impPreview.show();
					}
			...
			throw new UnsupportedOperationException("Command '" + e.getActionCommand() +
				"' is not supported!");
		}
	}
	 ...
	 * call!</strong> Meaning that when creating a non-modal dialog it is created
	 * and runs in its own thread and does not block the creator thread. Call this
	 * method, however, calls {@code semaphore.acquire}, which is a blocking call
	 * and waits until the semaphore is released (if it wasn't already) which is
	 * done after closing the dialog. Clearly, if this wasn't a blocking call,
	 * there wouldn't be a way to know how was the dialog closed, because it
	 * wouldn't need to be closed at the time of calling this method.
	 *
	...
	static void saveSelectedModuleIndexesToPrefs(int filterIndex, int detectorIndex,
		int estimatorIndex, int rendererIndex)
	{
		if (filterIndex >= 0) {
