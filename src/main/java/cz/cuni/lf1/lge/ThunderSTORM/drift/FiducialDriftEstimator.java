...
import cz.cuni.lf1.lge.ThunderSTORM.results.FrameSequence;
...
public class FiducialDriftEstimator {
	...
	public DriftResults estimateDrift(List<Molecule> molecules, double distanceThr,
		double onTimeRatio, double smoothingBandwidth)
	{
		...
		for (Molecule mol : fiducialMarkers) {
			...
			double[] frame = extractParamAsArray(detections, detections.get(0).descriptor.getParamIndex(
				MoleculeDescriptor.LABEL_FRAME));
			System.arraycopy(frame, 0, combinedFrames, lastIndex, frame.length);
			lastIndex += frame.length;
		}
		// find offsets for each fiducial marker (to get relative drift out of
		// absolute coordinates)
		IJ.showStatus("Finding marker offsets (x)...");
		double[] markerOffsetsInX = findFiducialsOffsets(fiducialMarkers, combinedFrames,
			PSFModel.Params.LABEL_X);
		IJ.showProgress(0.875);
		GUI.checkIJEscapePressed();
		IJ.showStatus("Finding marker offsets (y)...");
		double[] markerOffsetsInY = findFiducialsOffsets(fiducialMarkers, combinedFrames,
		...
		for (int i = 0; i < fiducialMarkers.size(); i++) {
			...
			double[] x = extractParamAsArray(detections, detections.get(0).descriptor.getParamIndex(
				PSFModel.Params.LABEL_X));
			double[] y = extractParamAsArray(detections, detections.get(0).descriptor.getParamIndex(
				PSFModel.Params.LABEL_Y));
		...
		PolynomialSplineFunction xFunction = CorrelationDriftEstimator.addLinearExtrapolationToBorders(
			interpolator.interpolate(combinedFrames, combinedX), minFrame, maxFrame);
		PolynomialSplineFunction yFunction = CorrelationDriftEstimator.addLinearExtrapolationToBorders(
			interpolator.interpolate(combinedFrames, combinedY), minFrame, maxFrame);
		IJ.showProgress(1);
		// same units as input
		MoleculeDescriptor.Units units = molecules.get(0).getParamUnits(PSFModel.Params.LABEL_X);
		return new DriftResults(xFunction, yFunction, combinedFrames, combinedX, combinedY, minFrame,
			maxFrame, units);
	}
	...
	private List<Molecule> groupMolecules(List<Molecule> molecules, double distanceThr) {
		...
		grouping.matchMolecules(MathProxy.sqr(distanceThr), new FrameSequence.RelativeToDetectionCount(
			2), new FrameSequence.LastFewDetectionsMean(5), 0, tracker);
		List<Molecule> groupedMolecules = grouping.getAllMolecules();
		return groupedMolecules;
	}

	public double[] findFiducialsOffsets(List<Molecule> fiducials, double[] combinedFrames,
		String param)
	{
		// first, restructure the required data in a data structure that can be
		// efficiently used in the optimization process

		// a helper class that holds a detection coordinate and an index of fiducial
		// marker the detection belongs to
		...
		// create a map from frame to a list of fiducial marker detections in that
		// frame
		...
		for (Iterator<Map.Entry<Double, List<ValAndMarkerIndex>>> it = values.entrySet().iterator(); it
			.hasNext();)
		{
		...
		final List<List<ValAndMarkerIndex>> detectionsInFrames = new ArrayList<List<ValAndMarkerIndex>>(
			values.values());

		NelderMead nm = new NelderMead();
		// cost function:
		// for each frame where multiple drift values are present
		// cost += square of difference between each drift value and mean drift
