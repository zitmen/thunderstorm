...
public class MultipleLocationsBiplaneFitting implements IBiplaneEstimator {
	...
	public MultipleLocationsBiplaneFitting(int fittingRadius, double distThrPx,
		Homography.TransformationMatrix homography, IOneLocationBiplaneFitter fitter)
	{
		...
		subxgrid1 = new double[bigSubImageSize * bigSubImageSize]; // to prevent
																																// multiple
																																// allocations
		subxgrid2 = new double[bigSubImageSize * bigSubImageSize]; // to prevent
																																// multiple
																																// allocations
		subygrid1 = new double[bigSubImageSize * bigSubImageSize]; // to prevent
																																// multiple
																																// allocations
		subygrid2 = new double[bigSubImageSize * bigSubImageSize]; // to prevent
																																// multiple
																																// allocations
		subimageData1 = new double[bigSubImageSize * bigSubImageSize]; // to prevent
																																		// multiple
																																		// allocations
		subimageData2 = new double[bigSubImageSize * bigSubImageSize]; // to prevent
																																		// multiple
																																		// allocations
	...
	private PSFSeparator.Position transformPos(Homography.TransformationMatrix transform, double x,
		double y)
	{
		mPos.setX(x);
		mPos.setY(y);
		return Homography.transformPositions(transform, mPositions, plane1.getWidth(), plane1
			.getHeight()).get(0);
	}

	public boolean extractSubGrid(double[] /*[out]*/ subxgrid, double[] /*[out]*/ subygrid,
		double[] /*[out]*/ data, double[] xgrid, double[] ygrid, FloatProcessor image, double x,
		double y)
	{
	...
	public void run() throws StoppedByUserException {
		...
		for (Pair<Point, Point> location : locations) {
			...
			PSFSeparator.Position pos = transformPos(homographyInverse, location.second.getX()
				.doubleValue() + 0.5, location.second.getY().doubleValue() + 0.5);
			...
			{
				try {
					SubImage subImage1 = new SubImage(bigSubImageSize, bigSubImageSize, subxgrid1, subygrid1,
						subimageData1, posX, posY);

					SubImage subImage2 = new SubImage(bigSubImageSize, bigSubImageSize, subxgrid2, subygrid2,
						subimageData2, posX, posY);
					...
					// maximum number of iterations has been exceeded (it is set very
					// high, so it usually means trouble)
					IJ.log(
						"Warning: the fitter couldn't converge (max. count of iterations was exceeded)! The molecule candidate has been thrown away.");
				}
				catch (ConvergenceException ex) {
					// exception:
					// "org.apache.commons.math3.exception.ConvergenceException: illegal
					// state:
					...
					// -> another possible reason is that camera offset is set too high,
					// which in combination with WLSQ
					// fitting may lead to division by zero, since intensities are used
					// for weighting!
					IJ.log(
						"Warning: the fitter couldn't converge (probably NaN or Inf occurred in calculations; if you use WLSQ fitting, check if camera offset isn't too high correctly; if not try MLE or LSQ fitting)! The molecule candidate has been thrown away.");
				}
				catch (Exception ex) {
	...
	private boolean checkIsInSubimage(double xinit, double yinit, double x, double y) {
		return !(Math.abs(x - xinit) > ((double) subimageSize + 0.5) || Math.abs(y -
			yinit) > ((double) subimageSize + 0.5));
	}
		...
		List<Point> detections1, List<Point> detections2) throws StoppedByUserException
	{
		this.plane1 = plane1;
		this.plane2 = plane2;
		this.locations = Homography.mergePositions(plane1.getWidth(), plane1.getHeight(), homography,
			detections1, detections2, distThrPx * distThrPx);
		initializeGrid();
