...
public class DataGenerator {
	...
	public FloatProcessor generateBackground(int width, int height, Drift drift, Range bkg) {
		// padd the background image; crop the center of the image later, after the
		// drift is applied
		FloatProcessor img = new FloatProcessor(width + 2 * (int) ceil(drift.dist), height + 2 *
			(int) ceil(drift.dist));
	...
	public Vector<EmitterModel> generateMolecules(int width, int height, FloatProcessor mask,
		double density, Range intensity_photons, IPsfUI psf)
	{
		...
		double gPpx = Units.NANOMETER_SQUARED.convertTo(Units.MICROMETER_SQUARED, sqr(CameraSetupPlugIn
			.getPixelSize()) * width * height / (mask.getWidth() * mask.getHeight())) * density, p_px, p,
				fwhm0;
		double zFrom = psf.getZRange().from, zTo = psf.getZRange().to;
		for (int x = 0; x < mask.getWidth(); x++) {
			for (int y = 0; y < mask.getHeight(); y++) {
				p_px = gPpx * mask.getf(x, y); // expected number of molecules inside a
																				// pixel
				int nMols = p_px > 0 ? (int) rand.nextPoisson(p_px) : 0; // actual
																																	// number of
																																	// molecules
																																	// inside a
																																	// pixel
				for (int i = 0; i < nMols; i++) {
					...
					params[PSFModel.Params.X] = (x + 0.5 + getNextUniform(-0.5, +0.5)) * width / mask
						.getWidth();
					params[PSFModel.Params.Y] = (y + 0.5 + getNextUniform(-0.5, +0.5)) * height / mask
						.getHeight();
					...
					params[PSFModel.Params.INTENSITY] = getNextUniform(intensity_photons.from,
						intensity_photons.to);
					...
					// set a common MoleculeDescriptor for all molecules in a frame to
					// save memory
					if (descriptor != null) {
	...
	public Vector<EmitterModel> generateSingleFixedMolecule(int width, int height, double xOffset,
		double yOffset, Range intensity_photons, IPsfUI psf)
	{
		double[] params = new double[PSFModel.Params.PARAMS_LENGTH];
		Vector<EmitterModel> molist = new Vector<EmitterModel>();
	...
	/**
	 * Replaces each pixel value with a sample from a poisson distribution with
	 * mean value equal to the pixel original value.
	 */
	...
	/**
	 * Replaces each pixel value with a sample from a Gamma distribution with
	 * shape equal to the original pixel value and scale equal to the gain
	 * parameter.
	 */
	...
	public ShortProcessor renderFrame(int width, int height, int frame_no, Drift drift,
		Vector<EmitterModel> molecules, FloatProcessor backgroundMeanIntensity)
	{
		...
		for (EmitterModel mol : molecules) {
			...
			if (mol.isOutOfRoi(frame.getRoi())) { // does the molecule get out of ROI
																						// due to the drift?
				deleteLater.add(mol);
			}
			else {
		...
		// simulates poisson distributed photon arrival (and EM gain using Gamma
		// distribution if it is enabled)
