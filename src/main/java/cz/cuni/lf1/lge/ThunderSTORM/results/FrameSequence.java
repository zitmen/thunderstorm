...
public class FrameSequence {
	 ...
	 * just 1 frame. Maximum number of off frames between two detections to be
	 * still considered as one molecule can be specifed (offFramesThr). Also
	 * maximum number of frames per one molecule can be specified
	 * (framesPerMolecule). The method works in 3D - calculating weighted squared
	 * euclidean distance. The weight of z in the distance is specified in
	 * zCoordWeight parameter. X and Y weights are not adjustable. Note: this
	 * method makes changes into `detections`! TODO: refactor - methods
	 * match/merge molecules are very similar
	 */
	public void mergeMolecules(double dist2_thr, int off_thr,
		ActiveMoleculePositionStrategy positionStrategy, int framesPerMolecule, double zCoordWeight,
		...
		for (int frame : frames) {
			...
			try {
				...
				for (Molecule mol : activeMolecules) {
					if ((framesPerMolecule > 0) && ((frame - getFirstAddedChildMolecule(mol).getParam(
						MoleculeDescriptor.LABEL_FRAME)) >= framesPerMolecule))
					{
						deactivate.add(mol); // the molecule was active too many frames so
																	// no more detections can be added
					}
					else if (((frame - 1) - getLastAddedChildMolecule(mol).getParam(
						MoleculeDescriptor.LABEL_FRAME)) > off_thr)
					{
						deactivate.add(mol); // the molecule was inactive too long so no
																	// more detections can be added
					}
				}
				...
				for (Molecule mol : activeMolecules) {
					try {
						...
						// key in the tree is the coords of the molecule from last frame,
						// but the object stored is the parent molecule
						tree.insert(new double[] { selectedMol.getX(), selectedMol.getY(), zCoordWeight *
							selectedMol.getZ() }, mol);
					}
					catch (KeyDuplicateException ex) {
						IJ.handleException(ex); // almost never happens...if it does,
																		// something is wrong with fitting/detection
					}
				}
				...
				for (Molecule mol : fr2mol) {
					if (!emptyTree) {
						nn_mol = tree.nearest(new double[] { mol.getX(), mol.getY(), zCoordWeight * mol
							.getZ() });
						Molecule selectedMol = positionStrategy.getActiveMoleculePosition(nn_mol);
						if (squareDistWeightedZ(mol, selectedMol, zCoordWeight) < dist2_thr) { // in
																																										// radius?
	 ...
	 * considered as one molecule can be specifed(offFramesThr). The method works
	 * in 3D - calculating weighted squared euclidean distance. The weight of z in
	 * the distance is specified in zCoordWeight parameter. X and Y weights are
	 * not adjustable. Note: this method makes changes into `detections`! TODO:
	 * refactor - methods match/merge molecules are very similar
	 */
	public void matchMolecules(double dist2_thr, OffFramesThresholdStrategy off_thr,
		ActiveMoleculePositionStrategy positionStrategy, double zCoordWeight,
		ProgressTracker progressTracker)
	{
		molecules.clear();
		List<Molecule> activeMolecules = new ArrayList<Molecule>();
			...
			try {
				...
				for (Molecule mol : activeMolecules) {
					try {
						...
						// key in the tree is the coords of the molecule from last frame,
						// but the object stored is the parent molecule
						tree.insert(new double[] { lastMol.getX(), lastMol.getY(), zCoordWeight * lastMol
							.getZ() }, mol);
					}
					catch (KeyDuplicateException ex) {
						IJ.handleException(ex); // almost never happens...if it does,
																		// somethin is wrong with fitting/detection
					}
				}
				boolean emptyTree = activeMolecules.isEmpty();
				Molecule nn_mol;
				...
				// remove from activeMolecules those, that were off for more than
				// offFramesThr
				activeMoleculesTemp.clear();
				for (Molecule mol : activeMolecules) {
					if (frame - getLastAddedChildMolecule(mol).getParam(
						MoleculeDescriptor.LABEL_FRAME) <= off_thr.getMaxOffFrames(mol))
					{
	...
	/**
	 * return the molecule that was last added to the input molecule or the input
	 * molecule if it is a single molecule
	 */
	...
	/**
	 * return the molecule that was first added to the input molecule or the input
	 * molecule if it is a single molecule
	 */
	...
	private static double squareDistWeightedZ(Molecule m1, Molecule m2, double zWeight) {
		return MathProxy.sqr(m1.getX() - m2.getX()) + MathProxy.sqr(m1.getY() - m2.getY()) + MathProxy
			.sqr(zWeight) * MathProxy.sqr(m1.getZ() - m2.getZ());
	}
	...
	public static class LastFewDetectionsMean implements ActiveMoleculePositionStrategy {
		...
		int few;
		private static final MoleculeDescriptor simpleMolDesc = new MoleculeDescriptor(new String[] {
			PSFModel.Params.LABEL_X, PSFModel.Params.LABEL_Y, PSFModel.Params.LABEL_Z });
