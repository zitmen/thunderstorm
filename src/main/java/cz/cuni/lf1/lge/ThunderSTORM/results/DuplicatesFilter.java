...
public class DuplicatesFilter extends PostProcessingModule {
	...
	protected JPanel createUIPanel() {
		...
		JLabel label = new JLabel(
			"Remove molecules that converged to the same position. Distance threshold [nm]:");
		filterPanel.add(label, new GridBagHelper.Builder().gridxy(0, 0).gridheight(1).fill(
			GridBagConstraints.BOTH).weightx(1).weighty(1).build());
		filterPanel.add(Help.createHelpButton(getClass()), new GridBagHelper.Builder().gridxy(1, 0)
			.anchor(GridBagConstraints.EAST).build());
		filterPanel.add(distTextField, new GridBagHelper.Builder().gridxy(0, 1).fill(
	...
	void applyToModel(GenericTableModel model, String formula) {
		...
		if (!model.columnExists(MoleculeDescriptor.Fitting.LABEL_UNCERTAINTY_XY)) {
			throw new RuntimeException(String.format(
				"Fitting uncertainty not found in Results table. Looking for: %s. Found: %s.",
				MoleculeDescriptor.Fitting.LABEL_UNCERTAINTY_XY, model.getColumnNames()));
		}
		...
		if (retval.isVector()) {
			if (((Double[]) retval.get()).length != model.getRowCount()) {
				throw new FormulaParserException(
					"Semantic error: result of formula must be either a scalar value, or a numeric vector of the same length as the number of rows in the table!");
			}
			else {
	...
	static class FrameSequence {
		 ...
		 * uncertainty with which their positions were estimated. The method works
		 * just in 2D, since the Thompson formula applies only for lateral
		 * coordinates. Thus calculating distance {
		 *
		...
		public boolean[] filterDuplicateMolecules() {
			...
			for (int fi = 0; fi < frno.length; fi++) {
				...
				for (int i = 0, count = frmol.size(); i < count; i++) {
					...
					int id = (int) mol.getParam(MoleculeDescriptor.LABEL_ID); // zero-based
																																		// indexing
