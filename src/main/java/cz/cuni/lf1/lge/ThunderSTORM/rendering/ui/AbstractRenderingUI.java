...
public abstract class AbstractRenderingUI extends IRendererUI {
	...
	public AbstractRenderingUI() {
		...
		magnification = parameters.createDoubleField("magnification", DoubleValidatorFactory
			.positiveNonZero(), 5);
		...
		zRange = parameters.createStringField("zrange", new Validator<String>() {
			...
			public void validate(String input) throws ValidatorException {
				try {
					...
					if (r.from > r.to) {
						throw new RuntimeException("Z range \"from\" value (" + r.from +
							") must be smaller than \"to\" value (" + r.to + ").");
					}
					if (nSlices < 1) {
