...
public class ImportExportPlugIn implements PlugIn {
	...
	private void setupMeasurementProtocol(boolean detectMeasurementProtocolFile) {
		...
		if (detectMeasurementProtocolFile) {
			// Automatically load protocol, if available -> this allows for
			// (re)calculation of
			// fitting uncertainties and other values that depend on the data about
			// analysis.
			...
			if (protoFile.exists() && protoFile.isFile()) {
				...
				MeasurementProtocol protocol = MeasurementProtocol.importFromFile(getProtocolFilePath(
					path));
				...
				if (!protocol.cameraSettings.isEmpty()) {
					CameraSetupPlugIn.params.setNoGuiParametersAllowed(true); // allow
																																		// setting
																																		// parameters
																																		// even
																																		// before UI
					...
					if (protocol.cameraSettings.containsKey(CameraSetupPlugIn.EM_GAIN_ENABLED)) {
						CameraSetupPlugIn.setIsEmGain((Boolean) protocol.cameraSettings.get(
							CameraSetupPlugIn.EM_GAIN_ENABLED));
					}
					if (protocol.cameraSettings.containsKey(CameraSetupPlugIn.EM_GAIN)) {
						CameraSetupPlugIn.setGain((Double) protocol.cameraSettings.get(
							CameraSetupPlugIn.EM_GAIN));
					}
					if (protocol.cameraSettings.containsKey(CameraSetupPlugIn.BASELINE_OFFSET)) {
						CameraSetupPlugIn.setOffset((Double) protocol.cameraSettings.get(
							CameraSetupPlugIn.BASELINE_OFFSET));
					}
					if (protocol.cameraSettings.containsKey(CameraSetupPlugIn.READOUT_NOISE)) {
						CameraSetupPlugIn.setReadoutNoise((Double) protocol.cameraSettings.get(
							CameraSetupPlugIn.READOUT_NOISE));
					}
					if (protocol.cameraSettings.containsKey(CameraSetupPlugIn.PHOTONS_TO_ADU)) {
						CameraSetupPlugIn.setPhotons2ADU((Double) protocol.cameraSettings.get(
							CameraSetupPlugIn.PHOTONS_TO_ADU));
					}
					if (protocol.cameraSettings.containsKey(CameraSetupPlugIn.PIXEL_SIZE)) {
						CameraSetupPlugIn.setPixelSize((Double) protocol.cameraSettings.get(
							CameraSetupPlugIn.PIXEL_SIZE));
					}
					if (protocol.cameraSettings.containsKey(CameraSetupPlugIn.QUANTUM_EFFICIENCY)) {
						CameraSetupPlugIn.setQuantumEfficiency((Double) protocol.cameraSettings.get(
							CameraSetupPlugIn.QUANTUM_EFFICIENCY));
					}
					CameraSetupPlugIn.params.setNoGuiParametersAllowed(false); // back to
																																			// the
																																			// default
																																			// setting
				}
				...
				IJ.log(
					"Measurement protocol was not found (we will not be able to recalculate fitting uncertainties if requested).");
			}
		}
	}
	...
	private void callExporter(IImportExport exporter, GenericTable table, String fpath,
		int floatPrecision, List<String> columns)
	{
	...
	private void callImporter(IImportExport importer, GenericTable table, String fpath,
		int startingFrame)
	{
	...
	abstract class IODialog extends DialogStub {
		...
		public IODialog(ParameterTracker params, ParameterTracker fileParams, Window owner,
			String title)
		{
	...
	class ImportDialog extends IODialog {
		...
		public ImportDialog(Window owner, boolean groundTruth) {
			super(new ParameterTracker("thunderstorm.io"), new ParameterTracker("thunderstorm.io." +
				(groundTruth ? "gt" : "res")), owner, "Import" + (groundTruth ? " ground-truth" : ""));
			assert moduleNames != null && moduleNames.length > 0;
			fileFormat = fileParams.createStringField("fileFormat", StringValidatorFactory.isMember(
				moduleNames), moduleNames[0]);
			filePath = fileParams.createStringField("filePath", StringValidatorFactory.fileExists(), "");
			startingFrame = params.createIntField("startingFrame", IntegerValidatorFactory
				.positiveNonZero(), 1);
			append = params.createBooleanField("append", null, false);
			this.groundTruth = groundTruth;
			if (!groundTruth) {
				detectMeasurementProtocol = fileParams.createBooleanField("detectMeasurementProtocol", null,
					true);
				showPreview = params.createBooleanField("livePreview", null, true);
				rawImageStack = params.createStringField("rawImageStack", StringValidatorFactory.openImages(
					true), "");
			}
		}
		...
		protected void layoutComponents() {
			...
			filePathTextField.getDocument().addDocumentListener(createDocListener(filePathTextField,
				fileFormatCBox));
			fileFormatCBox.addItemListener(createItemListener(filePathTextField, fileFormatCBox));
			JButton browseButton = createBrowseButton(filePathTextField, true,
				new FileNameExtensionFilter(createFilterString(moduleExtensions), moduleExtensions));
	...
	class ExportDialog extends IODialog {
		...
		public ExportDialog(Window owner, boolean groundTruth, String[] columnHeaders) {
			super(new ParameterTracker("thunderstorm.io"), new ParameterTracker("thunderstorm.io." +
				(groundTruth ? "gt" : "res")), owner, "Export" + (groundTruth ? " ground-truth" : ""));
			assert moduleNames != null && moduleNames.length > 0;
			...
			floatPrecision = fileParams.createIntField("floatPrecision", IntegerValidatorFactory
				.positive(), 5);
		...
		protected void layoutComponents() {
			...
			if (!groundTruth) {
				...
				protocolPanel.add(Box.createHorizontalStrut(filePathTextField.getPreferredSize().width / 2),
					GridBagHelper.leftCol());
				protocolPanel.add(Box.createHorizontalStrut(filePathTextField.getPreferredSize().width),
					GridBagHelper.rightCol());
			...
			columnsPanel.add(Box.createHorizontalStrut(filePathTextField.getPreferredSize().width),
				GridBagHelper.rightCol());
			for (int i = 0; i < columnHeaders.length; i++) {
	...
	private ItemListener createItemListener(final JTextField filePathTextField,
		final JComboBox<String> fileFormatCBox)
	{
			...
			public void itemStateChanged(ItemEvent e) {
				...
				if (!fp.isEmpty()) {
					if (fp.endsWith("\\") || fp.endsWith("/")) {
						filePathTextField.setText(fp + "results." + moduleExtensions[fileFormatCBox
							.getSelectedIndex()]);
					}
					else {
						SwingUtilities.invokeLater(new Runnable() {
							...
							public void run() {
								...
								if (dotpos > 0) {
									filePathTextField.setText(fp.substring(0, dotpos + 1) +
										moduleExtensions[fileFormatCBox.getSelectedIndex()]);
								}
							}
						});
					}
	...
	private DocumentListener createDocListener(final JTextField filePathTextField,
		final JComboBox<String> fileFormatCBox)
	{
