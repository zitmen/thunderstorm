...
public abstract class AbstractRendering implements RenderingMethod, IncrementalRenderingMethod {
	...
	protected static abstract class AbstractBuilder<BuilderType extends AbstractBuilder, BuiltType extends AbstractRendering> {
		...
		private double zFrom = Double.NEGATIVE_INFINITY, zTo = Double.POSITIVE_INFINITY, zStep =
		 ...
		 * pixel of the original image. <br>
		 * If the molecule locations are in nm, value of 20 signifies that one pixel
		 * in the superresolution image will be 20nm big <br>
		 * If the molecule locations are in pixels, a value of 0.2 signifies that
		 * the pixel size of the superresoultion image will be 0.2 times the pixel
		 * size of the original image.(5x smaller)
		 */
		public BuilderType resolution(double nmPerPixel) {
			if (nmPerPixel <= 0) {
				throw new IllegalArgumentException("Resolution must be positive. Passed value = " +
					nmPerPixel);
			}
		...
		public BuilderType imageSize(int x, int y) {
			if (x <= 0 || y <= 0) {
				throw new IllegalArgumentException("Image size must be positive. Passed values = " + x +
					" " + y);
			}
			this.imSizeX = x;
			this.imSizeY = y;
		...
		public BuilderType defaultDX(double defaultDX) {
			if (defaultDX <= 0) {
				throw new IllegalArgumentException("Default dx must be positive. Passed value = " +
					defaultDX);
			}
		...
		public BuilderType defaultDZ(double defaultDZ) {
			if (defaultDZ <= 0) {
				throw new IllegalArgumentException("Default dz must be positive. Passed value = " +
					defaultDZ);
			}
		...
		public BuilderType zRange(double from, double to, double step) {
			if (to <= from) {
				throw new IllegalArgumentException("Z range \"from\" value (" + from +
					") must be smaller than \"to\" value (" + to + ").");
			}
			if (step <= 0) {
				throw new IllegalArgumentException(
					"Z range \"step\" value must be positive. Passed value = " + step);
			}
			this.zFrom = from;
			this.zStep = step;
		...
		protected void validate() {
			if (!roiWasSet && !sizeWasSet) {
				throw new IllegalArgumentException(
					"Image size must be resolved while building. Set at least image size or roi.");
			}
			if (!roiWasSet && sizeWasSet) {
				if (!resolutionWasSet) {
				...
				if (resolutionWasSet) {
					...
					if (newImSizeX != imSizeX || newImSizeY != imSizeY) {
						throw new IllegalArgumentException(
							"Invalid combination of image size, roi and resolution. Set only two of them.");
					}
				}
				else {
			...
			if (pixelcount > Integer.MAX_VALUE) {
				throw new IllegalArgumentException("Tried to create too big image (" + imSizeX + " x " +
					imSizeY + "). Check that parameters are correct and use appropriate units.");
			}
		}
		...
		if (image.isComposite()) {
			...
			if (colorizationLut == null) { // fallback if no LUTs are installed
				for (int i = 0; i < channeLuts.length; i++) {
					// Colormap for slices: (has constant grayscale intensity, unlike jet
					// and similar)
					// r: /
					// __/
					// g: /\
