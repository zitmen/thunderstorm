...
public class ResultsFilter extends PostProcessingModule {
	...
	protected JPanel createUIPanel() {
		...
		filterPanel.add(new JLabel("Filter: "), new GridBagHelper.Builder().gridxy(0, 0).anchor(
			GridBagConstraints.WEST).build());
		GridBagConstraints gbc = new GridBagConstraints();
		gbc.fill = GridBagConstraints.HORIZONTAL;
		gbc.weightx = 1;
		filterPanel.add(filterTextField, new GridBagHelper.Builder().gridxy(0, 1).fill(
			GridBagConstraints.HORIZONTAL).weightx(1).build());
		filterPanel.add(Help.createHelpButton(getClass()), new GridBagHelper.Builder().gridxy(2, 0)
	...
	private Rectangle2D.Double convertRectangleUnits(Rectangle2D rect, Units ux, Units uy,
		double magnification)
	{
		Rectangle2D.Double ret = new Rectangle2D.Double(Units.PIXEL.convertTo(ux, rect.getMinX() /
			magnification), Units.PIXEL.convertTo(uy, rect.getMinY() / magnification), 0, 0);

		ret.add(Units.PIXEL.convertTo(ux, rect.getMaxX() / magnification), Units.PIXEL.convertTo(uy,
			rect.getMaxY() / magnification));
		return ret;
	}
	...
	void addNewFilter(String paramName, double greaterThan, double lessThan) {
		...
		sb.append("(");
		sb.append(paramName).append(" > ").append(BigDecimal.valueOf(greaterThan).round(new MathContext(
			6)).toString());
		sb.append(" & ");
		sb.append(paramName).append(" < ").append(BigDecimal.valueOf(lessThan).round(new MathContext(6))
			.toString());
		sb.append(")");
	...
	public static String createRectanglesFilter(Rectangle2D[] rectangles) {
		...
		for (int i = 0; i < rectangles.length; i++) {
			sb.append("(");
			sb.append("x > ").append(BigDecimal.valueOf(rectangles[i].getMinX()).round(new MathContext(6))
				.toString());
			sb.append(" & x < ").append(BigDecimal.valueOf(rectangles[i].getMaxX()).round(new MathContext(
				6)).toString());
			sb.append(" & y > ").append(BigDecimal.valueOf(rectangles[i].getMinY()).round(new MathContext(
				6)).toString());
			sb.append(" & y < ").append(BigDecimal.valueOf(rectangles[i].getMaxY()).round(new MathContext(
				6)).toString());
			sb.append(")");
		...
		if (text.isEmpty()) {
			...
			if (!retval.isVector()) {
				throw new FormulaParserException(
					"Semantic error: result of filtering formula must be a vector of boolean values!");
			}
