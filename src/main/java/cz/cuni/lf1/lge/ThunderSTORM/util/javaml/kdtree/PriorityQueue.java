...
class PriorityQueue implements java.io.Serializable {
	 ...
	 * implemented in such a way that objects are added using an <code>add</code>
	 * function. The <code>add</code> function takes two parameters an object and
	 * a long.
	 * <p>
	 * The object represents an item in the queue, the long indicates its priority
	 * in the queue. The remove function in this class returns the object first in
	 * the queue and that object is removed from the queue permanently.
	 * 
	 ...
	 * queue and to leave in the order of priority i.e the highest priority get's
	 * to leave first.
	 */
	...
	/**
	 * This is an initializer for the object. It basically initializes an array of
	 * long called value to represent the prioritys of the objects, it also
	 * creates an array of objects to be used in parallel with the array of longs,
	 * to represent the objects entered, these can be used to sequence the data.
	 * 
	 * @param size the initial capacity of the queue, it can be resized
	 */
	...
	private void bubbleDown(int pos) {
		...
		for (; pos * 2 <= count; pos = child) {
			...
			if (child != count)
				...
				if (value[child] < value[child + 1]) child++; /* choose the biggest child */
			/*
	 ...
	 * rightful place higher in the queue, but only if it's priority allows it to
	 * do so, similar to bubbleDown only in the other direction this swaps out its
	 * parents.
	 * 
