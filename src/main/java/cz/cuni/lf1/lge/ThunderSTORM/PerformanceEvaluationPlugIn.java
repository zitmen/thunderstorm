...
public class PerformanceEvaluationPlugIn implements PlugIn {
	...
	public void run(String command) {
		...
		if (!IJResultsTable.isResultsWindow() || !IJGroundTruthTable.isGroundTruthWindow()) {
			IJ.error("Requires `" + IJResultsTable.IDENTIFIER + "` and `" +
				IJGroundTruthTable.IDENTIFIER + "` windows open!");
			return;
		}
		//
		try {
			...
			runEvaluation(dialog.getFrameByFrame(), dialog.getEvaluationSpace().equals("xyz"), sqr(dialog
				.getToleranceRadius()), distUnits);
		}
		catch (Exception ex) {
	...
	private void prepareResultsTable(Units units) {
		try {
			...
			descriptor.addParam(MoleculeDescriptor.LABEL_DISTANCE_TO_GROUND_TRUTH_XY, lastIndex + 1,
				units);
			descriptor.addParam(MoleculeDescriptor.LABEL_DISTANCE_TO_GROUND_TRUTH_Z, lastIndex + 2,
				units);
			descriptor.addParam(MoleculeDescriptor.LABEL_DISTANCE_TO_GROUND_TRUTH_XYZ, lastIndex + 3,
				units);
		}
		catch (Exception ex) {
			//
	...
	private void runEvaluation(boolean frameByFrame, boolean threeD, double dist2Tol,
		Units distUnits)
	{
		//
		...
		try {
			...
			IJ.showMessage(
				"Column `frame` does not exist! Either fill the column, or don't use frame-by-frame evaluation.");
			return;
		}
	...
	private int getFrameCount() {
		return (int) max(VectorMath.max(IJResultsTable.getResultsTable().getColumnAsDoubles(
			MoleculeDescriptor.LABEL_FRAME)), VectorMath.max(IJGroundTruthTable.getGroundTruthTable()
	...
	final class MoleculeMatcherWorker implements Runnable {
		...
		public MoleculeMatcherWorker(int frameStart, int frameStop, boolean threeD, double dist2Thr,
			Units distUnits)
		{
			this.frames = new TreeSet<Integer>();
			this.detections = fillWithData(frameStart, frameStop, IJResultsTable.getResultsTable());
			this.groundTruth = fillWithData(frameStart, frameStop, IJGroundTruthTable
				.getGroundTruthTable());
		...
		private Map<Integer, List<Molecule>> fillWithData(int frameStart, int frameStop,
			GenericTable table)
		{
	...
	class PerformanceEvaluationDialog extends DialogStub {
		...
		public PerformanceEvaluationDialog() {
			super(new ParameterTracker("thunderstorm.evaluation"), IJ.getInstance(),
				"ThunderSTORM: Performance evaluation");
			toleranceRadius = params.createDoubleField("toleranceRadius", DoubleValidatorFactory
				.positiveNonZero(), 50.0);
