...
abstract public class DLMImportExport implements IImportExport {
	...
	public void importFromFile(String fp, GenericTable table, int startingFrame) throws IOException {
		...
		// im using file size and counting bytes read to track progress (without
		// having to know line count in advance)
		Scanner sc = null;
		CountingInputStream countingInputStream = null;
		try {
			countingInputStream = new CountingInputStream(new BufferedInputStream(new FileInputStream(
				file)));
			...
			while (sc.hasNextLine()) {
				...
				if (line.length == 1 && line[0].isEmpty()) continue; // just an empty
																															// line...do not
																															// report
				if (line.length != dataColCount) { // but report when there is a
																						// corrupted row
	...
	private String[] splitLine(String line) {
		...
		for (int i = 0; i < arr.length; i++) {
			arr[i] = arr[i].trim();
			if (arr[i].startsWith("\"") && arr[i].endsWith("\"") || arr[i].startsWith("'") && arr[i]
				.endsWith("'") || arr[i].startsWith("`") && arr[i].endsWith("`"))
			{
