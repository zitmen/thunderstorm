...
public class MFA_MLEFitter extends MFA_AbstractFitter {
	...
	double pValueThr;
	boolean sameI;
	final static int MODEL_SELECTION_ITERATIONS = 5000; // full fitting takes
																											// ~50000 iterations (-->
																											// this is 10%)
	...
	public Molecule fit(SubImage subimage) {
		...
		if (maxN > 1) {
			...
			for (int n = 1; n <= maxN; n++) {
				...
				logLik = model.getLikelihoodFunction(subimage.xgrid, subimage.ygrid, subimage.values).value(
					fittedParams);
				if (n > 1) {
					pValue = 1.0 - new ChiSquaredDistribution(model.getDoF() - modelBest.getDoF())
						.cumulativeProbability(-2 * (logLikPrevBest - logLik));
		...
		MLEFitter fitter = new MLEFitter(modelBest, MLEFitter.MAX_ITERATIONS -
			MODEL_SELECTION_ITERATIONS, Params.BACKGROUND);
