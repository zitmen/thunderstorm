...
/**
 * Representation of PSFModel model. <strong>Note:</strong> in a future release
 * the PSFModel will be more abstract to allow easily work with any possible
 * PSFModel out there, but right we use strictly the symmetric 2D Gaussian
 * model. <strong>This class and its children need to be refactored!</strong>
 */
public abstract class PSFModel implements IModule {

	/**
	 * This class allows to maintain the variables inside of the PSF, keeps their
	 * order in check just by changing the ids, and makes the conversion between
	 * names and indices into the `params` array. If a PSF doesn't use any of the
	 * parameters, it can be simply ignored. Allocating few extra bytes doesn't
	 * hurt the speed much and it simplifies the code a bit.
	 */
	...
	public MultivariateMatrixFunction getNumericJacobianFunction(final double[] xgrid,
		final double[] ygrid)
	{
	...
	public MultivariateFunction getLikelihoodFunction(final double[] xgrid, final double[] ygrid,
		final double[] imageValues)
	{
...
//        IJ.log("point: " + Arrays.toString(point));
	...
	public double getChiSquared(final double[] xgrid, final double[] ygrid,
		final double[] imageValues, double[] point, boolean weighted)
	{
	...
	public abstract Molecule newInstanceFromParams(double[] params,
		MoleculeDescriptor.Units subImageUnits, boolean afterFitting);
	// --> afterFitting: there might be some normalizations before/after fitting;
	// this method is also used in rendering, where such normalization isn't
	// needed
}
