...
import cz.cuni.lf1.lge.ThunderSTORM.estimators.IOneLocationBiplaneFitter;
import cz.cuni.lf1.lge.ThunderSTORM.estimators.LSQFitter;
import cz.cuni.lf1.lge.ThunderSTORM.estimators.MLEFitter;
import cz.cuni.lf1.lge.ThunderSTORM.estimators.MultipleLocationsBiplaneFitting;
...
public class EllipticBiplaneGaussianEstimatorUI extends IBiplaneEstimatorUI {
	...
	public EllipticBiplaneGaussianEstimatorUI() {
		...
		METHOD = parameters.createStringField("method", StringValidatorFactory.isMember(new String[] {
			MLE, LSQ, WLSQ }), MLE);
		NUMERICAL_DERIVATIVES = parameters.createBooleanField("numericalDerivatives", null, false);
		CALIBRATION_PATH = parameters.createStringField("calibrationpath", StringValidatorFactory
			.fileExists(), "");
		// it's better to make the following distance larger and let the matcher do
		// it's job; otherwise,
		// a localization uncertainty would be the way to go, however, it would
		// require an additional round of fitting
		MATCHING_DISTANCE_THRESHOLD = parameters.createDoubleField("distThrPx", DoubleValidatorFactory
			.positive(), 3);
	}
	...
	public JPanel getOptionsPanel() {
		...
		final JTextField calibrationFileTextField = new JTextField(Prefs.get(
			"thunderstorm.estimators.calibrationpath", ""));
		...
		JButton findCalibrationButton = DialogStub.createBrowseButton(calibrationFileTextField, true,
			new FileNameExtensionFilter("Yaml file", "yaml"));
	...
	public IBiplaneEstimator getImplementation() {
		...
		return new MultipleLocationsBiplaneFitting(fittingRadius, distThrPx, calibration.homography,
			fitter);
	}
	...
	private static DoubleDefocusCalibration<DaostormCalibration> loadCalibration(
		String calibrationFilePath)
	{
		...
		try {
			...
			for (Object obj : new Yaml(new Homography.TransformationMatrix.YamlConstructor()).loadAll(
				fr))
			{
				if (obj instanceof Homography.TransformationMatrix) {
					...
					throw new RuntimeException(
						"Unknown calibration class incompatible with biplane analysis!");
				}
			}
			if (homography == null) {
			...
			if (cal1 == null) {
				throw new RuntimeException(
					"Defocusing curve estimate is missing from the calibration data!");
			}
			if (cal2 == null) {
				throw new RuntimeException(
					"Biplane calibration requires two defocusing curves, but only one was found!");
			}
			cal1.homography = homography;
			cal2.homography = homography;
			return new DoubleDefocusCalibration<DaostormCalibration>(DefocusFunctionSqrt.name, homography,
				cal1.getDaoCalibration(), cal2.getDaoCalibration());
		}
