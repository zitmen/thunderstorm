...
/**
 * This implementation is directly re-written from Matlab without any
 * optimizations!
 */
public class Homography {

	public static TransformationMatrix estimateTransform(
		CalibrationConfig.RansacConfig ransacTranslationAndFlipConfig,
		CalibrationConfig.RansacConfig ransacHomographyConfig, int w1, int h1, List<Position> fits1,
		int w2, int h2, List<Position> fits2)
	{
		List<Position> pt1 = moveToCenterXY(fits1, w1, h1);
		List<Position> pt2 = moveToCenterXY(fits2, w2, h2);
		return findHomography(ransacHomographyConfig, findTranslationAndFlip(
			ransacTranslationAndFlipConfig, pt1, pt2), pt1, pt2);
	}

	/**
	 * 1) Based on the transformation, pair up the fits[1|2] together 2) For each
	 * fit in matched subset of fits1 select only the subset of Molecules
	 * (z-slice) present both in fits[1|2] 3) For each pair of Molecules assign
	 * sigma1 and sigma2 for the moleule from fits1 4) Return the Map of Positions
	 * containing the Molecules with sigmas properly set --> these are used for
	 * estimation of calibration curves
	 */
	public static Map<Position, Position> mergePositions(TransformationMatrix transform,
		IBinaryTransform<Position> mapping, double dist2thr, int w1, int h1, List<Position> fits1,
		int w2, int h2, List<Position> fits2)
	{
		// 1)
		...
		for (Map.Entry<Position, Position> pair : pairs.entrySet()) {
			...
			// -> [optional] recalculate the position as it has been transformed by
			// the transform
	...
	/**
	 * This method is simplier compared to the other one in the fact that
	 * fits[1|2] are supposed to be in the same frame, but different planes. The
	 * only purpose is to match the pairs. TODO: this is not very effective
	 * approach as the number of allocations is high!
	 */
	public static List<Pair<Point, Point>> mergePositions(int width, int height,
		TransformationMatrix transform, List<Point> fits1, List<Point> fits2, double dist2px)
	{
		MoleculeDescriptor descriptor = new MoleculeDescriptor(new String[] {
			MoleculeDescriptor.LABEL_ID, PSFModel.Params.LABEL_X, PSFModel.Params.LABEL_Y },
			new MoleculeDescriptor.Units[] { MoleculeDescriptor.Units.UNITLESS,
				MoleculeDescriptor.Units.PIXEL, MoleculeDescriptor.Units.PIXEL });
		// wrap to molecules
		...
		for (int i = 0; i < fits1.size(); i++) {
			Point p = fits1.get(i);
			m1.add(new Molecule(descriptor, new double[] { i, p.getX().doubleValue(), p.getY()
				.doubleValue() }));
		}
		for (int i = 0; i < fits2.size(); i++) {
			Point p = fits2.get(i);
			m2.add(new Molecule(descriptor, new double[] { i, p.getX().doubleValue(), p.getY()
				.doubleValue() }));
		}
		...
		for (Map.Entry<Molecule, Molecule> pair : map.entrySet()) {
			pairs.add(new Pair<Point, Point>(fits1.get((int) pair.getKey().getParamAt(idIndex)), fits2
				.get((int) pair.getValue().getParamAt(idIndex))));
		}
		return pairs;
	}
	...
	public static List<PSFSeparator.Position> transformPositions(
		TransformationMatrix transformationMatrix, List<PSFSeparator.Position> positions, int width,
		int height)
	{
		return moveToBoundaryXY(applyH(transformationMatrix, moveToCenterXY(positions, width, height)),
			width, height);
	}
	...
	public static class TransformationMatrix {
		...
		public <T extends IMatchable<T>> T transform(T p) {
			RealMatrix pMat = new Array2DRowRealMatrix(new double[][] { { p.getX() }, { p.getY() }, {
				1.0 } }); // 3x1 matrix
		...
		public static TransformationMatrix createIdentity() {
			...
			tm.matrix = new Array2DRowRealMatrix(new double[][] { { 1, 0, 0 }, { 0, 1, 0 }, { 0, 0,
		...
		public static TransformationMatrix getNormalization(List<Position> positions) {
			...
			return TransformationMatrix.createFrom(new Array2DRowRealMatrix(new double[][] { { 1.0 /
				scale, 0.0, -centroid.centroidX / scale }, { 0.0, 1.0 / scale, -centroid.centroidY /
					scale }, { 0.0, 0.0, 1.0 } }));
		}

		public static TransformationMatrix getDenormalizedHomography(TransformationMatrix homography,
			TransformationMatrix norm1, TransformationMatrix norm2)
		{
	...
	private static <T extends IMatchable<T>> List<T> moveToCenterXY(Iterable<T> posIn, int width,
		int height)
	{
	...
	// Find the translation between two planes using a set of corresponding points
	// (RANSAC method is used).
	private static TransformationMatrix findTranslationAndFlip(CalibrationConfig.RansacConfig config,
		List<Position> p1, List<Position> p2)
	{
		...
		IRansacFunctions functions = new IRansacFunctions() {
			...
			@Override
			public List<Pair<Double, Integer>> distance(TransformationMatrix t, List<Position> p1,
				List<Position> p2)
			{
	...
	// RANSAC method is used.
	private static TransformationMatrix findHomography(CalibrationConfig.RansacConfig config,
		final TransformationMatrix initialGuess, List<Position> p1, List<Position> p2)
	{
		...
		Pair<List<Position>, List<Position>> pairs = initialPairing(config.thPairDist, initialGuess, p1,
			p2);
		...
		TransformationMatrix homography = ransac(applyH(norm1, pairs.first), applyH(norm2,
			pairs.second), config, functions);
	...
	// Project pts1 to P1 using H, then calcultate the distances between P1 and
	// P2; I is index into P2/pts2
	private static List<Pair<Double, Integer>> calcDist(TransformationMatrix t, List<Position> p1,
		List<Position> p2)
	{
	...
	private static TransformationMatrix solveTranslationAndFlip(List<Position> p1,
		List<Position> p2)
	{
		if (p1.size() != p2.size()) {
	...
	/**
	 * to enhance the results, we pair together all points with mutual distance
	 * less than `thPairDist`, instead of just points with minimum distance; this
	 * brings a certain redundancy to the data thus we need more iterations of
	 * ransac but the result is obviously much better since there might be areas
	 * with dense distribution of molecules in the data and it could cause many
	 * mismatched pairs
	 */
	private static Pair<List<Position>, List<Position>> initialPairing(double thPairDist,
		TransformationMatrix H, List<Position> pts1, List<Position> pts2)
	{
	...
	private static TransformationMatrix solveHomography(List<Position> p1, List<Position> p2) {
		...
		if (p1.size() < 4) {
			throw new IllegalArgumentException(
				"Homography can't be solved for less than 4 points of interest!");
		}
	...
	private static boolean validateH(double thDist, TransformationMatrix initial,
	...
	private interface IRansacFunctions {
		...
		List<Pair<Double, Integer>> distance(TransformationMatrix t, List<Position> p1,
			List<Position> p2); // returns <dist, idx>

		boolean isResultValid(TransformationMatrix t);
	}

	/**
	 * Use RANdom SAmple Consensus to find a fit from X to Y. X is M*n matrix
	 * including n points with dim M, Y is N*n; The fit, f, and the indices of
	 * inliers, are returned. RANSACCOEF is a struct with following fields:
	 * minPtNum,iterNum,thDist,thInlrRatio MINPTNUM is the minimum number of
	 * points with whom can we find a fit. For line fitting, it's 2. For
	 * homography, it's 4. ITERNUM is the number of iteration, THDIST is the
	 * inlier distance threshold and ROUND(THINLRRATIO*n) is the inlier number
	 * threshold. FUNCFINDF is a func handle, f1 = funcFindF(x1,y1) x1 is M*n1 and
	 * y1 is N*n1, n1 >= ransacCoef.minPtNum f1 can be of any type. FUNCDIST is a
	 * func handle, d = funcDist(f,x1,y1) It uses f returned by FUNCFINDF, and
	 * return the distance between f and the points, d is 1*n1. For line fitting,
	 * it should calculate the dist between the line and the points [x1;y1]; for
	 * homography, it should project x1 to y2 then calculate the dist between y1
	 * and y2.
	 */
	private static TransformationMatrix ransac(List<Position> p1, List<Position> p2,
		CalibrationConfig.RansacConfig conf, IRansacFunctions functions)
	{
		int ptNum1 = p1.size();
		int ptNum2 = p2.size();
		...
		if (conf.pairs && ptNum1 != ptNum2) {
			throw new IllegalArgumentException(
				"ransac: `p1` and `p2` must have the same number of items!");
		}
		...
		for (int p = 0; p < conf.iterNum; p++) {
			...
			TransformationMatrix f1 = functions.findTransform(select(p1, sampleIdx1), select(p2,
				sampleIdx2));
			...
			fLib.add(functions.findTransform(select(p1, inlier1), select(p2, selectIndices(distIdx,
				inlier1))));
		}
	...
	/**
	 * Select the coef with the most inliers; if there is more of them with the
	 * same number of inliers, then choose the one with the minimum distance.
	 */
	private static int findBest(List<Integer> inlrNum, List<Double> inlrDist) {
		if (inlrNum.size() != inlrDist.size()) {
			throw new IllegalArgumentException(
				"`inlrNum` and `inlrDist` must have the same number of elements!");
		}
	...
	private static int[] randIndex(int maxIndex, int len) {
		if (len > maxIndex + 1) {
			throw new IllegalArgumentException(
				"randIndex: `len` can't be greater than number of indices when selecting non-repeatedly!");
		}
