...
public class CalibrationConfig {
	...
	public static class RansacConfig {
		...
		public int minPtNum;
		public int iterNum;
		public double thDist; // precision [px]; usualy the precision is much higher
													// (~1e-6 px)
		public double thInlr; // [%]
		public boolean pairs;
		public double thPairDist; // mutual distance of paired molecules [px]
															// (should be same as `coef.thDist` set in
															// `findTranslationAndFlip`)
		public double thAllowedTransformChange; // point [1,1] after applying T
																						// can't move further than 100px,
																						// otherwise something is wrong,
																						// since this step should be just
																						// for fine-tuning! (this value is
																						// large on purpose, so the test
																						// does not reject a valid
																						// transformation)

		public static RansacConfig createTranslationAndFlipConfig(int iterNum, double thDist,
			double thInlr)
		{
			return new RansacConfig(2, iterNum, thDist, thInlr, false, 0.0, Double.POSITIVE_INFINITY);
		}

		public static RansacConfig createHomographyConfig(int iterNum, double thDist, double thInlr,
			double thPairDist, double thAllowedTransformChange)
		{
			return new RansacConfig(4, iterNum, thDist, thInlr, true, thPairDist,
				thAllowedTransformChange);
		}

		private RansacConfig(int minPtNum, int iterNum, double thDist, double thInlr, boolean pairs,
			double thPairDist, double thAllowedTransformChange)
		{
