...
/**
 * Gaussian filter is a convolution filter with its kernel filled with values of
 * normalized 2D Gaussian function written as
 * {@mathjax \frac{1}{\sqrt{2\pi\sigma^2}} e^{-\frac{x^2}{2 \sigma^2}}}. This
 * kernel is symmetric and it is also separable, thus the filter uses the
 * separable kernel feature.
 */
public final class GaussianFilter implements IFilter {
	 ...
	 * of the 2D Gaussian function with a specified {@mathjax \sigma}
	 * ({@code sigma}).
	 *
	...
	public GaussianFilter(int size, double sigma) {
		this.filter = new cz.cuni.lf1.thunderstorm.algorithms.filters.GaussianFilter(size, sigma,
