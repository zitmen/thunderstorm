	...
	public boolean isVariable(String obj, String var) {
		if (isThresholding()) {
			...
			throw new UnsupportedOperationException(
				"Unsupported type of parser! Supported types are: thresholding and results filtering.");
		}
	}
	...
	public RetVal getVariable(String obj, String var, Units units) {
		if (isThresholding()) {
			if (obj == null) { // active filter this filter is already active,
				// hence there is no need to redo the filtering step, since it has been
				// already done
			...
			if (units == null) {
				...
				catch (Exception ex) { // if the conversion is not possible, then ignore
																// it
					return new RetVal(IJResultsTable.getResultsTable().getColumnAsDoubleObjects(var));
				}
			}
