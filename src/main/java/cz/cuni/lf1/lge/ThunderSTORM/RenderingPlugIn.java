...
public class RenderingPlugIn implements PlugIn {
	...
	private enum AutoSize {
			MANUAL("manual"), RESULTS("results"), IMAGE("image");
	...
	public void run(String string) {
		...
		if (!table.columnExists(LABEL_X) || !table.columnExists(LABEL_Y)) {
			IJ.error(String.format(
				"X and Y columns not found in Results table. Looking for: %s and %s. Found: %s.", LABEL_X,
				LABEL_Y, table.getColumnNames()));
			return;
		}
		...
		if (table.columnExists(MoleculeDescriptor.Fitting.LABEL_UNCERTAINTY_XY)) {
			dx = table.getColumnAsDoubles(MoleculeDescriptor.Fitting.LABEL_UNCERTAINTY_XY,
		...
		if (table.columnExists(MoleculeDescriptor.Fitting.LABEL_UNCERTAINTY_Z)) {
			dz = table.getColumnAsDoubles(MoleculeDescriptor.Fitting.LABEL_UNCERTAINTY_Z,
				MoleculeDescriptor.Units.NANOMETER);
		}
		...
		if (MacroParser.isRanFromMacro()) {
			...
			if (autosize.equals(AutoSize.IMAGE.getValue()) && (IJResultsTable.IDENTIFIER.equals(table
				.getTableIdentifier()) && (im = ((IJResultsTable) table).getAnalyzedImage()) != null))
			{
				left = 0;
				top = 0;
				sizeX = im.getWidth();
				...
				Rectangle rect = resizeByResults(xpos, ypos, new EmptyRendererUI().magnification
					.getValue());
				left = rect.getX();
				top = rect.getY();
				sizeX = rect.getWidth();
			...
			if (IJResultsTable.IDENTIFIER.equals(table.getTableIdentifier()) && (im =
				((IJResultsTable) table).getAnalyzedImage()) != null)
			{
				guessedLeft = 0;
				guessedTop = 0;
			...
			RenderingDialog dialog = new RenderingDialog(preview, knownRenderers, guessedLeft, guessedTop,
				guessedWidth, guessedHeight);
		...
		if (setAsPreview) {
			RenderingQueue queue = new RenderingQueue(method, new RenderingQueue.DefaultRepaintTask(method
				.getRenderedImage()), selectedRendererUI.getRepaintFrequency());
...
class RenderingDialog extends JDialog {
	...
	public RenderingDialog(boolean preview, List<IRendererUI> knownRenderers, double left, double top,
	...
	private void layoutComponents() {
		...
		sizeResultsButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Rectangle rect = RenderingPlugIn.resizeByResults(rt.getColumnAsDoubles(LABEL_X, PIXEL), rt
					.getColumnAsDoubles(LABEL_Y, PIXEL), new EmptyRendererUI().magnification.getValue());
				left = rect.getX();
				top = rect.getY();
				sizeX = rect.getWidth();
				sizeY = rect.getHeight();
				...
				if (rt.columnExists(LABEL_Z)) {
					...
					renderingMethods.getActiveComboBoxItem().setZRange(VectorMath.min(zpos), VectorMath.max(
						zpos));
				}
				else {
		...
		okButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					Prefs.set("thunderstorm.rendering.index", activeRendererIndex = renderingMethods
						.getActiveComboBoxItemIndex());
