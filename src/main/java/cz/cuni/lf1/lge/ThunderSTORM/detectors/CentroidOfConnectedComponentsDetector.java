...
public final class CentroidOfConnectedComponentsDetector extends IDetectorUI implements IDetector {
	 ...
	 */
	public CentroidOfConnectedComponentsDetector(String threshold, boolean useWatershed)
	 ...
	 * threshold to zero, splitting close peaks by watershed and finding centroids
	 * of connected components. In more detail this is how it is done:
	 * <ol>
	 ...
	 * image is zero. Leave the grayscale value otherwise.</li>
	 * <li>perform a watershed transform (this is the trick for recognition of
	 * more connected molecules</li>
	 * <li>AND the thresholded image with watershed image</li>
	 * <li>then we think of the resulting image as an undirected graph with
	 * 8-connectivity and find all connected components with the same id</li>
	 * <li>finally, positions of molecules are calculated as centroids of
	 * components with the same id</li>
	 * </ol>
	 *
	...
	public List<Point> detectMoleculeCandidates(FloatProcessor image) throws FormulaParserException {
		...
		thresholdValue = localThresholdValue; // publish the calculated
																					// threshold,(not thread safe but only
																					// used for preview logging)
		...
		if (useWatershed) {
			...
			ByteProcessor watershedImage = watershedImpl.findMaxima(maskedImage, 0,
				ImageProcessor.NO_THRESHOLD, MaximumFinder.SEGMENTED, false, false);
			FloatProcessor thresholdImageANDWatershedImage = applyMask(thresholdedImage,
				(FloatProcessor) watershedImage.convertToFloat());
		...
		for (Graph.ConnectedComponent c : Graph.getConnectedComponents((ImageProcessor) maskedImage,
			Graph.CONNECTIVITY_8))
		{
			Point pt = c.centroid();
			pt.val = new Double(image.getf((int) Math.round(pt.x.doubleValue()), (int) Math.round(pt.y
				.doubleValue())));
			detections.add(pt);
		}
		return detections;
