...
public class MFA_LSQFitter extends MFA_AbstractFitter {
	...
	double pValueThr;
	boolean sameI;
	final static int MODEL_SELECTION_ITERATIONS = 50; // full fitting takes ~20
																										// iterations; this is here
																										// to limit max. number of
																										// iterations, which is set
																										// to 1000

	public MFA_LSQFitter(PSFModel basePsfModel, double defaultSigma, int maxN, double pValueThr,
		boolean sameI, Range expI)
	{
		...
		if (expectedIntensity != null) {
			this.expectedIntensity.convert(MoleculeDescriptor.Units.PHOTON,
				MoleculeDescriptor.Units.DIGITAL);
		}
	}
	...
	public Molecule fit(SubImage subimage) {
		...
		if (maxN > 1) {
			...
			for (int n = 1; n <= maxN; n++) {
				...
				if (n > 1) {
					try {
						pValue = 1.0 - new FDistribution(model.getDoF() - modelBest.getDoF(),
							subimage.values.length - model.getDoF()).cumulativeProbability(((chi2Best - chi2) /
								(model.getDoF() - modelBest.getDoF())) / (chi2 / (subimage.values.length - model
									.getDoF())));
						if (!Double.isNaN(pValue) && (pValue < pValueThr)) {// &&
																																// !isOutOfRegion(mol,
																																// ((double)subimage.size)
																																// / 2.0)
							modelBest = model;
							chi2Best = chi2;
						}
						...
						int maxMol = (subimage.values.length - 2) / (int) (basePsfModel.getDoF() - 2); // both
																																														// intensity
																																														// and
																																														// offset
																																														// are
																																														...
																																														// single
																																														// parameter
						throw new StoppedDueToErrorException("F-distribution `F(" + (int) (model.getDoF() -
							modelBest.getDoF()) + "," + (int) (subimage.values.length - model.getDoF()) +
							")` could not be created! " +
							"There is too many molecules (degrees of freedom) in the model!\n The maximum number of " +
							"molecules with the current settings (PSF model and fitting radius) is " + maxMol +
							".", ex);
					}
				}
				else {
		...
		return eliminateBadFits(mol, ((double) subimage.size_x) / 2.0 - defaultSigma / 2.0,
			((double) subimage.size_y) / 2.0 - defaultSigma / 2.0);
	}
}
