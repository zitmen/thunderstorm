...
public final class DifferenceOfGaussiansFilter implements IFilter {
    ...
    public DifferenceOfGaussiansFilter(int size, double sigma_g1, double sigma_g2) {
        ...
        dog = new cz.cuni.lf1.thunderstorm.algorithms.filters.DifferenceOfGaussiansFilter(size, sigma_g1, sigma_g2, DuplicatePadding::new);
    }
    ...
    public FloatProcessor filterImage(@NotNull FloatProcessor image) {
        ...
        input = image;
        result_g1 = GrayScaleImageImpl.convertToFloatProcessor(g1.filter(new GrayScaleImageImpl(image)));
        result_g2 = GrayScaleImageImpl.convertToFloatProcessor(g2.filter(new GrayScaleImageImpl(image)));
