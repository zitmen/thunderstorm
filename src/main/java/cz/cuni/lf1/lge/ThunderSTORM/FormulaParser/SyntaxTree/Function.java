...
public class Function extends Node {
	...
	private static final HashSet<String> builtInFunctions = new HashSet<String>(Arrays.asList(
		new String[] { "var", "std", "mean", "median", "max", "min", "sum", "abs" }));

	@Override
	public void semanticScan() throws FormulaParserException {
		if (!builtInFunctions.contains(name.toLowerCase())) throw new FormulaParserException(
			"Semantic error! Function '" + name + "' does not exist!");
		arg.semanticScan();
	}
	...
	public RetVal eval(Object param) {
		...
		// the following will never happen due to the semanticCheck in the
		// constructor
	...
	protected double max(Object param) {
		...
		if (val.isMatrix()) { // FloatProcessor
			return (FloatStatistics.getStatistics((FloatProcessor) val.get(), Measurements.MIN_MAX,
				null)).max;
		}
	...
	protected double std(Object param) {
		RetVal val = arg.eval(param);
		if (val.isMatrix()) { // FloatProcessor
			return (FloatStatistics.getStatistics((FloatProcessor) val.get(), Measurements.STD_DEV,
				null)).stdDev;
		}
		else if (val.isVector()) {
	...
	protected double mean(Object param) {
		RetVal val = arg.eval(param);
		if (val.isMatrix()) { // FloatProcessor
			return (FloatStatistics.getStatistics((FloatProcessor) val.get(), Measurements.MEAN,
				null)).mean;
		}
		else if (val.isVector()) {
	...
	protected double median(Object param) {
		RetVal val = arg.eval(param);
		if (val.isMatrix()) { // FloatProcessor
			return (FloatStatistics.getStatistics((FloatProcessor) val.get(), Measurements.MEDIAN,
				null)).median;
		}
		else if (val.isVector()) {
