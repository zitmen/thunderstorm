...
public class ResultsGrouping extends PostProcessingModule {
	...
	public ResultsGrouping() {
		...
		framesPerMolecule = params.createIntField("framesPerMolecule", IntegerValidatorFactory
			.positive(), 0);
		zCoordWeightParam = params.createDoubleField("zCoordWeight", DoubleValidatorFactory.positive(),
			0.1);
	}
	...
	protected JPanel createUIPanel() {
		...
		grouping.add(groupThrLabel, new GridBagHelper.Builder().gridxy(0, 0).weightx(0.2).anchor(
			GridBagConstraints.EAST).build());
		grouping.add(distanceTextField, new GridBagHelper.Builder().gridxy(1, 0).weightx(0.3).anchor(
			GridBagConstraints.WEST).build());
		grouping.add(framesPerMoleculeLabel, new GridBagHelper.Builder().gridxy(2, 0).weightx(0.2)
			.anchor(GridBagConstraints.EAST).build());
		grouping.add(framesPerMoleculeTextField, new GridBagHelper.Builder().gridxy(3, 0).weightx(0.3)
			.anchor(GridBagConstraints.WEST).build());
		grouping.add(Help.createHelpButton(getClass()), new GridBagHelper.Builder().gridxy(4, 0).anchor(
			GridBagConstraints.EAST).build());
		grouping.add(offFramesLabel, new GridBagHelper.Builder().gridxy(0, 1).weightx(0.5).anchor(
			GridBagConstraints.EAST).build());
		grouping.add(offFramesTextField, new GridBagHelper.Builder().gridxy(1, 1).weightx(0.5).anchor(
			GridBagConstraints.WEST).build());
	...
	protected void runImpl() {
		...
		new WorkerThread<List<Molecule>>() {
			...
			protected void finishJob(List<Molecule> mergedMolecules) {
				...
				table.setStatus(Integer.toString(merged) + " molecules were merged into " + Integer
					.toString(into) + " molecules");
				table.showPreview();
			}
	...
	public static List<Molecule> getMergedMolecules(GenericTableModel model, double dist,
		int offFrames, int framesPerMol, double zWeight)
	{
		if (!model.columnExists(PSFModel.Params.LABEL_X) || !model.columnExists(
			PSFModel.Params.LABEL_Y))
		{
			throw new RuntimeException(String.format(
				"X and Y columns not found in Results table. Looking for: %s and %s. Found: %s.",
				PSFModel.Params.LABEL_X, PSFModel.Params.LABEL_Y, model.getColumnNames()));
		}
		//
		...
		frames.mergeMolecules(sqr(dist), offFrames, new FrameSequence.LastDetection(), framesPerMol,
			zWeight, null);
		//
