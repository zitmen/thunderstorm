...
public class ResultsDriftCorrection extends PostProcessingModule {
	...
	public ResultsDriftCorrection() {
		...
		ParameterTracker.Condition pathCondition = new Condition() {
			...
			@Override
			public boolean isSatisfied() {
				return actionParam.getValue().equals(actions[2]) || saveParam.getValue(); // load
																																									// action
																																									// or
																																									// enabled
																																									// save
		...
		actionParam = params.createStringField("method", StringValidatorFactory.isMember(actions),
			actions[0]);
		// cross correlation params
		binsParam = params.createIntField("steps", IntegerValidatorFactory.rangeInclusive(2,
			Integer.MAX_VALUE), 5, crossCorrCondition);
		magnificationParam = params.createDoubleField("magnification", DoubleValidatorFactory
			.positiveNonZero(), 5, crossCorrCondition);
		ccSmoothingBandwidthParam = params.createDoubleField("ccSmoothingBandwidth",
			DoubleValidatorFactory.rangeInclusive(0, 1), 0.25, crossCorrCondition);
		showCorrelationImagesParam = params.createBooleanField("showCorrelations", null, false,
			crossCorrCondition);
		// fiducials params
		distanceThresholdParam = params.createDoubleField("distanceThr", DoubleValidatorFactory
			.positiveNonZero(), 40, fiducialCondition);
		onTimeRatioParam = params.createDoubleField("onTimeRatio", DoubleValidatorFactory
			.rangeInclusive(0, 1), 0.1, fiducialCondition);
		smoothingBandwidthParam = params.createDoubleField("smoothingBandwidth", DoubleValidatorFactory
			.rangeInclusive(0, 1), 0.25, fiducialCondition);
		// load save params
	...
	protected JPanel createUIPanel() {
		...
		ccPanel.add(new JLabel("Trajectory smoothing factor:", SwingConstants.TRAILING), GridBagHelper
			.leftCol());
		...
		JButton browseButton = DialogStub.createBrowseButton(savePathTextField, true,
			new FileNameExtensionFilter("JSON text file", "json"));
		...
		// Table ui panel
		GridBagHelper.Builder glueConstr = new GridBagHelper.Builder().fill(
			GridBagConstraints.HORIZONTAL).weightx(1);
		...
		ccOptionsBalloon = new BalloonTip(ccOptionsButton, ccPanel, new RoundedBalloonStyle(5, 5,
			fiducialPanel.getBackground(), Color.BLACK), false);
		fiducialOptionsBalloon = new BalloonTip(fiducialOptionsButton, fiducialPanel,
			new RoundedBalloonStyle(5, 5, fiducialPanel.getBackground(), Color.BLACK), false);
		ccOptionsBalloon.setVisible(false);
		fiducialOptionsBalloon.setVisible(false);
		...
		uiPanel.add(Help.createHelpButton(getClass()), new GridBagHelper.Builder().gridxy(9, 0).anchor(
	...
	public void runImpl() {
		try {
			...
			if (!model.columnExists(PSFModel.Params.LABEL_X) || !model.columnExists(
				PSFModel.Params.LABEL_Y))
			{
				throw new RuntimeException("Could not find " + PSFModel.Params.LABEL_X + " and " +
					PSFModel.Params.LABEL_Y + " columns.");
			}
			if (!model.columnExists(MoleculeDescriptor.LABEL_FRAME)) {
				throw new RuntimeException("Could not find \"" + MoleculeDescriptor.LABEL_FRAME +
					"\" column.");
			}
		}
		...
		new WorkerThread<DriftResults>() {
			...
			public DriftResults doJob() {
				...
				if (action.equals(actions[0])) {
					...
					double[] x = model.getColumnAsDoubles(PSFModel.Params.LABEL_X,
						MoleculeDescriptor.Units.PIXEL);
					...
					results = CorrelationDriftEstimator.estimateDriftFromCoords(x, y, frame, binsParam
						.getValue(), magnificationParam.getValue(), ccSmoothingBandwidthParam.getValue(), -1,
						-1, showCorrelationImagesParam.getValue());
				}
				else if (action.equals(actions[1])) {
					// fiducials
					List<Molecule> molecules = getClonedMoleculeList();

					results = new FiducialDriftEstimator().estimateDrift(molecules, distanceThresholdParam
						.getValue(), onTimeRatioParam.getValue(), smoothingBandwidthParam.getValue());
				}
			...
			public void finishJob(DriftResults driftCorrection) {
				...
				if ((driftCorrection instanceof CrossCorrelationDriftResults) && showCorrelationImagesParam
					.getValue())
				{
	...
	private DriftResults loadResultsFromFile(String path) throws IOException {
		...
		try {
			...
			Gson gson = new GsonBuilder().setPrettyPrinting().registerTypeAdapter(
				UnivariateFunction.class, new InstanceCreator<PolynomialSplineFunction>()
				{

					@Override
					public PolynomialSplineFunction createInstance(Type type) {
						return new PolynomialSplineFunction(new double[] { 1, 2 }, new PolynomialFunction[] {
	...
	private void showAnotherLocationDialog(IOException ex, final DriftResults results) {
		...
		dialog.add(new JLabel("Could not save drift results. " + ex.getMessage(),
	...
	public static void showDriftPlot(DriftResults driftCorrection) {
		...
		Plot plot = new Plot("Drift", "frame", "drift [" + driftCorrection.getUnits() + "]",
			(float[]) null, null);
		if (driftCorrection.getDriftDataX().length > 50) {
			plot.setFrameSize(1280, 720);
		}
		plot.setLimits(minFrame, driftCorrection.getMaxFrame(), Math.min(VectorMath.min(driftCorrection
			.getDriftDataX()), VectorMath.min(driftCorrection.getDriftDataY())), Math.max(VectorMath.max(
				driftCorrection.getDriftDataX()), VectorMath.max(driftCorrection.getDriftDataY())));
		plot.setColor(new Color(255, 128, 128));
		plot.addPoints(driftCorrection.getDriftDataFrame(), driftCorrection.getDriftDataX(),
			Plot.CROSS);
		plot.draw();
		plot.setColor(new Color(128, 255, 128));
		plot.addPoints(driftCorrection.getDriftDataFrame(), driftCorrection.getDriftDataY(),
			Plot.CROSS);
	...
	static void showCorrelations(CrossCorrelationDriftResults driftCorrection) {
		...
		for (int i = 1; i < binDriftsX.length; i++) {
			RenderingOverlay.showPointsInImage(imp, new double[] { -binDriftsX[i] / driftCorrection
				.getScaleFactor() + imp.getWidth() / 2 + 0.5 }, new double[] { -binDriftsY[i] /
					driftCorrection.getScaleFactor() + imp.getHeight() / 2 + 0.5 }, i, Color.red,
