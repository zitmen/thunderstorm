...
public class Point<T extends Number> {
	...
	public Point<Integer> roundToInteger() {
		...
		if (val == null) {
			...
			return new Point<Integer>(round(this.x.floatValue()), round(this.y.floatValue()), round(
				this.val.floatValue()));
		}
	}
	...
	public Point<Long> roundToLong() {
		...
		if (val == null) {
			...
			return new Point<Long>(round(this.x.doubleValue()), round(this.y.doubleValue()), round(
	...
	/**
	 * Returns the point with its X,Y coordinates and intensity as Double (double
	 * precision).
	 *
	 ...
	 * and nanometers. The data type T of Point&lt;T&gt; remains the same as it
	 * was before calling this method.
	 * <ol>
	 ...
	 *          <li>the result is converted back to type T (there may be a round
	 *          off or overflow error)</li>
	 *          </ol>
	...
	/**
	 * Comparator class for sorting the Point&lt;T&gt; instances. This comparator
	 * uses only X,Y coordinations, <strong>not</strong> the intensity. The
	 * left-most point will be ordered as first. If two or more points have the
	 * same X coordinate the point with the lowest value of Y coordinate will be
	 * ordered as first.
	 */
	...
	public static List<Point> applyRoiMask(Roi roi, List<Point> detections) {
		if (roi == null) {
			...
			for (int i = 0; i < detections.size();) {
				...
				if (!roi.contains(pt.x.intValue() + roi.getBounds().x, pt.y.intValue() + roi
					.getBounds().y))
				{
					detections.remove(i);
				}
