...
public class RenderingOverlay {
	 ...
	 * coordinates on to an input image. If the input {@code ImagePlus} instance
	 * contains a stack, the marker will be put into one global Overlay, which is
	 * shown for each slice of the stack.
	 *
	 ...
	 * @param markerType one of the predefined marker-types ({@code MARKER_CIRCLE}
	 *          or {@code MARKER_CROSS})
	 */
	public static void showPointsInImage(ImagePlus imp, double[] xCoord, double[] yCoord, Color c,
		int markerType)
	{
	 ...
	 * @param xCoord array of X coordinates
	 * @param yCoord array of Y coordinates
	 * @param slice slice number (indexing starts from 1). If the slice number is
	 *          0, the markers will be put into one global Overlay, which is shown
	 *          for each slice of the stack.
	 * @param c color of markers
	 * @param markerType one of the predefined marker-types ({@code MARKER_CIRCLE}
	 *          or {@code MARKER_CROSS})
	 */
	public static void showPointsInImage(ImagePlus imp, double[] xCoord, double[] yCoord, int slice,
		Color c, int markerType)
	{
		assert (xCoord.length == yCoord.length);
	...
	public static Overlay addPointsToOverlay(double[] xCoord, double[] yCoord, Overlay overlay,
	 ...
	 * @param markerType
	 */
	public static void showPointsInImage(IJResultsTable rt, ImagePlus imp, Rectangle roi, Color c,
		int markerType)
	{
		if (rt.isEmpty()) {
			return;
	 ...
	 * @param markerType
	 */
	public static void showPointsInImage(Molecule[] mols, ImagePlus imp, Rectangle roi, Color c,
		int markerType)
	{
		Overlay overlay = imp.getOverlay();
		if (overlay == null) {
		...
		Arrays.sort(mols, new Comparator<Molecule>() {
			...
			public int compare(Molecule o1, Molecule o2) {
				return Double.compare(o1.getParam(MoleculeDescriptor.LABEL_FRAME), o2.getParam(
	...
	static class MultiplePointsRoi extends Roi {
		...
		public void draw(Graphics g) {
			for (int i = 0; i < xs.length; i++) {
				// x,y added to coords because of the WTF way of flattening overlay in
				// newer versions of ImageJ(1.45s works fine without it)
				// x,y is set to zeros in constructors, but imageJ sets it to (width,
				// height) while flattening the overlay to signal that the roi should
				// not be painted (the roi is painted outside the image)
