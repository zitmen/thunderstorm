...
/**
 * ThunderSTORM Analysis plugin. Open the options dialog, process an image stack
 * to recieve a list of localized molecules which will get displayed in the
 * {@code ResultsTable} and previed in a new {@code ImageStack} with detections
...
public final class AnalysisPlugIn implements ExtendedPlugInFilter {
	...
	private final int pluginFlags = DOES_8G | DOES_16 | DOES_32 | NO_CHANGES | NO_UNDO | DOES_STACKS |
		PARALLELIZE_STACKS | FINAL_PROCESSING | SUPPORTS_MASKING;
	...
	/**
	 * Returns flags specifying capabilities of the plugin. This method is called
	 * before an actual analysis and returns flags supported by the plugin. The
	 * method is also called after the processing is finished to fill the
	 * {@code ResultsTable} and to visualize the detections directly in image
	 * stack (a new copy of image stack is created). <strong>The
	 * {@code ResultsTable} is always guaranteed to contain columns <i>frame, x,
	 * y</i>!</strong> The other parameters are optional and can change for
	 * different PSFs.
	 *
	...
	public int setup(String command, ImagePlus imp) {
		...
		if (command.equals("final")) {
			...
			processedImage = imp;
			return pluginFlags; // Grayscale only, no changes to the image and
													// therefore no undo
		}
	}
	...
	public int showDialog(final ImagePlus imp, final String command, PlugInFilterRunner pfr) {
		...
		try {
			...
			if (MacroParser.isRanFromMacro()) {
				...
				MacroParser parser = new MacroParser(false, allFilters, allEstimators, allDetectors,
					allRenderers);
				...
				renderingQueue = new RenderingQueue(method, new RenderingQueue.DefaultRepaintTask(
					renderedImage), rendererPanel.getRepaintFrequency());

				measurementProtocol = new MeasurementProtocol(imp, allFilters.get(selectedFilter),
					allDetectors.get(selectedDetector), allEstimators.get(selectedEstimator));
			}
			else {
				...
				try {
					SwingUtilities.invokeAndWait(new Runnable() {
						...
						@Override
						public void run() {
							dialog = new AnalysisOptionsDialog(imp, command, allFilters, allDetectors,
								allEstimators, allRenderers);
				...
				renderedImage = (method != null) ? method.getRenderedImage() : null;
				renderingQueue = new RenderingQueue(method, new RenderingQueue.DefaultRepaintTask(
					renderedImage), renderer.getRepaintFrequency());
				...
				measurementProtocol = new MeasurementProtocol(imp, dialog.getFilter(), dialog.getDetector(),
					dialog.getEstimator());
			}
		}
		catch (Throwable ex) {
		...
		try {
			...
			Thresholder.setActiveFilter(selectedFilter); // !! must be called before
																										// any threshold is
																										// evaluated !!
			Thresholder.parseThreshold(allDetectors.get(selectedDetector).getThreadLocalImplementation()
				.getThresholdFormula());
		}
		catch (Exception ex) {
	 ...
	 * stack we want to process. Allocation of resources to store the results is
	 * done here.
	 *
	...
	/**
	 * Run the plugin. This method is ran in parallel, thus counting the results
	 * must be done atomicaly.
	 *
	 * @param ip input image
	 */
	@Override
	public void run(ImageProcessor ip) {
		assert (selectedFilter >= 0 && selectedFilter < allFilters
			.size()) : "Index out of bounds: selectedFilter!";
		assert (selectedDetector >= 0 && selectedDetector < allDetectors
			.size()) : "Index out of bounds: selectedDetector!";
		assert (selectedEstimator >= 0 && selectedEstimator < allEstimators
			.size()) : "Index out of bounds: selectedEstimator!";
		assert (selectedRenderer >= 0 && selectedRenderer < allRenderers
			.size()) : "Index out of bounds: selectedRenderer!";
		assert (renderingQueue != null) : "Renderer was not selected!";
		//
		ip.setRoi(roi.getBounds());
		FloatProcessor fp = subtract((FloatProcessor) ip.crop().convertToFloat(),
			(float) CameraSetupPlugIn.getOffset());
		...
		try {
			...
			FloatProcessor filtered = allFilters.get(selectedFilter).getThreadLocalImplementation()
				.filterImage(fp);
			...
			List<Molecule> fits = allEstimators.get(selectedEstimator).getThreadLocalImplementation()
				.estimateParameters(fp, Point.applyRoiMask(roi, detections));
	...
	synchronized public void storeFits(Iterable<Molecule> fits, int frame) {
		...
		for (Molecule psf : fits) {
			psf.insertParamAt(0, MoleculeDescriptor.LABEL_FRAME, MoleculeDescriptor.Units.UNITLESS,
				(double) frame);
			rt.addRow(psf);
		}
	}
	...
	synchronized public void showResults() {
		...
		if (processedImage != null) {
			...
			RenderingOverlay.showPointsInImage(rt, processedImage, roi.getBounds(), Color.red,
