...
/**
 * This class wraps mathematical functions used in ThunderSTORM. The reason for
 * this is that the common java.lang.Math class is missing some of the methods
 * we need to use. Also the methods in the class are a bit slow for our needs.
 * So it is more convenient to have this class (Adapter pattern) to not need to
 * commit changes inside the application code and instead of that use methods in
 * this class to globaly change the methods we want to use. Our decision was to
 * use highly optimized FastMath class from Apache Commons Math3 library, which
 * performs much better in our scenario (many calls to {@code exp}) than the
 * general Java Math class.
 */
