...
public class RadialSymmetryFitter implements IOneLocationFitter {
	...
	public Molecule fit(SubImage img) {
		...
		return new Molecule(new Params(new int[] { PSFModel.Params.X, PSFModel.Params.Y }, coordinates,
			false));
	}
	 ...
	 *          will be computed, true for main diagonal and false for
	 *          antidiagonal
	 * @return
	 */
	...
	private float[] calculateSlope(float[] dIdu, float[] dIdv) {
		...
		for (int i = 0; i < m.length; i++) {
			...
			val = Float.isInfinite(val) ? Float.MAX_VALUE / 1e5f : val; // replace inf
																																	// by some big
																																	// value - Not
																																	// max_value
																																	// because it
	...
	private float[] calculateWeights(float[] dIdu, float[] dIdv, float[] xMesh, float[] yMesh) {
		...
		for (int i = 0; i < weights.length; i++) {
			float gradientMagnitude = dIdu[i] * dIdu[i] + dIdv[i] * dIdv[i];
			double distanceToCentroid = Math.sqrt((xMesh[i] - xCentroid) * (xMesh[i] - xCentroid) +
				(yMesh[i] - yCentroid) * (yMesh[i] - yCentroid));
	 ...
	 * origin (xc, yc) such that lines y = mx+b have the minimal total distance^2
	 * to the origin:
	 */
