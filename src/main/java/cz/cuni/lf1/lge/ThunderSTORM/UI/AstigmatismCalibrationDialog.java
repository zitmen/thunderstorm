...
public class AstigmatismCalibrationDialog extends DialogStub implements ActionListener {
	...
	public AstigmatismCalibrationDialog(ImagePlus imp, List<IFilterUI> filters,
		List<IDetectorUI> detectors, List<IEstimatorUI> estimators, List<DefocusFunction> defocusing)
	{
				...
				previewFuture = previewThreadRunner.submit(new Runnable() {
					...
					public void run() {
						try {
							...
							FloatProcessor filtered = getActiveFilterUI().getThreadLocalImplementation()
								.filterImage(fp);
							new ImagePlus("Filtered frame " + Integer.toString(imp.getSlice()), filtered).show();
							GUI.checkIJEscapePressed();
							List<Point> detections = Point.applyRoiMask(imp.getRoi(), getActiveDetectorUI()
								.getThreadLocalImplementation().detectMoleculeCandidates(filtered));
							GUI.checkIJEscapePressed();
							//
							...
							//
							ImagePlus impPreview = new ImagePlus("Preview for frame " + Integer.toString(imp
								.getSlice()), processor);
							RenderingOverlay.showPointsInImage(impPreview, xCoord, yCoord, Color.red,
