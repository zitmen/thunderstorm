...
public final class Molecule implements Comparable<Molecule>, IMatchable<Molecule> {
	...
	private List<Molecule> detections; // for merging of re-appearing molecules
																			// (post-processing)
	private List<Molecule> neighbors; // for molecule matching (performance
																		// evaluation)
	...
	public double getDist2(Molecule mol, Units distUnits) {
		return sqr(getX(distUnits) - mol.getX(distUnits)) + sqr(getY(distUnits) - mol.getY(distUnits)) +
			sqr(getZ(distUnits) - mol.getZ(distUnits));
	}
	...
	public double getDistAxial(Molecule mol, Units distUnits) {
		return abs(getZ(distUnits) - mol.getZ(distUnits));
	}
	...
	/**
	 * Add a parameter to a specified `column` and shift all the columns with
	 * higher index by one. Note: indexing to the array of `values` is managed
	 * internally and the new value will be also to the last position of the
	 * array, independently of the column.
	 */
	public void insertParamAt(int column, String name, Units units, double value) {
		if (hasParam(name)) { // is the param already present in the table?
			...
			values = realloc(values, pos + 1); // realloc (ineffective, but it's not
																					// supposed to be used often anyway)
			values[pos] = value; // values can be added at the last position since we
														// implement extra indexing for this
		}
	}
	...
	public double getZ(Units unit) {
		if (hasParam(Params.LABEL_Z)) {
			return descriptor.units.get(descriptor.getParamColumn(Params.LABEL_Z)).convertTo(unit,
				getParam(Params.LABEL_Z));
		}
		else {
			return 0.0;
		}
	 ...
	 * For a clone with duplicate data but different references, call
	 * `clone(MoleculeDescriptor)`! Note that `neighbors` are ignored by the
	 * cloning operation!
	 */
	@Override
	...
	/**
	 * Clone the molecule. Caller has to duplicate the descriptor if it is
	 * required! Note that `neighbors` are ignored by the cloning operation!
	 */
	...
	public double dist2xyz(Molecule mol, Units units) {
		return (sqr(mol.getX(units) - getX(units)) + sqr(mol.getY(units) - getY(units)) + sqr(mol.getZ(
			units) - getZ(units)));
	}
	...
	public int compareTo(Molecule mol) {
		...
		double frame = getParam(MoleculeDescriptor.LABEL_FRAME), molFrame = mol.getParam(
			MoleculeDescriptor.LABEL_FRAME);
		if (frame == molFrame) {
			return (int) (getParam(MoleculeDescriptor.LABEL_ID) - mol.getParam(
