...
public class DataGeneratorPlugIn implements PlugIn {
	...
	private void runGenerator() throws InterruptedException {
		...
		// FloatProcessor bkg = new DataGenerator().generateBackground(width,
		// height, drift);
		//
		...
		while (!finished) {
			...
			for (int c = 0; c < numThreads; c++) {
				threads[c].join(wait);
				finished &= !threads[c].isAlive(); // all threads must not be alive to
																						// finish!
			}
		...
		gt.setOriginalState();
		for (int c = 0; c < numThreads; c++) {
			generators[c].fillResults(stack, gt); // and generate stack and table of
																						// ground-truth data
		}
		gt.insertIdColumn();
		...
		cal.setUnit("um");
		cal.pixelWidth = cal.pixelHeight = Units.NANOMETER.convertTo(Units.MICROMETER, CameraSetupPlugIn
			.getPixelSize());
	...
	private class GeneratorWorker implements Runnable {
		...
		public void run() {
			for (int f = frame_start; f <= frame_end; f++) {
				...
				Vector<EmitterModel> molecules = singleFixedMolecule ? datagen.generateSingleFixedMolecule(
					width, height, 0, 0, intensity_range, psf) : datagen.generateMolecules(width, height,
						densityMask, density, intensity_range, psf);
				ShortProcessor slice = datagen.renderFrame(width, height, f, drift, molecules,
					backgroundMeanIntensity);
		...
		private void fillResults(ImageStack stack, IJGroundTruthTable gt) {
			...
			for (int f = frame_start, i = 0; f <= frame_end; f++, i++) {
				...
				for (EmitterModel psf : local_table.elementAt(i)) {
					psf.molecule.insertParamAt(0, MoleculeDescriptor.LABEL_FRAME,
						MoleculeDescriptor.Units.UNITLESS, (double) (f + 1));
					psf.molecule.setParam(PSFModel.Params.LABEL_OFFSET, MoleculeDescriptor.Units.DIGITAL,
						CameraSetupPlugIn.getOffset());
	...
	private boolean getInput() {
		...
		final ParameterKey.Integer widthParam = params.createIntField("width", IntegerValidatorFactory
			.positiveNonZero(), Defaults.WIDTH);
		final ParameterKey.Integer heightParam = params.createIntField("height", IntegerValidatorFactory
			.positiveNonZero(), Defaults.HEIGHT);
		final ParameterKey.Integer framesParam = params.createIntField("frames", IntegerValidatorFactory
			.positiveNonZero(), Defaults.FRAMES);
		final ParameterKey.Double densityParam = params.createDoubleField("density",
			DoubleValidatorFactory.positive(), Defaults.DENSITY);
		final ParameterKey.String psfParam = params.createStringField("psf", new PsfValidator(),
			Defaults.PSF);
		final ParameterKey.Double addPoissonVarParam = params.createDoubleField("addPoisssonVar",
			DoubleValidatorFactory.positive(), Defaults.ADD_POISSON_VAR);
		final ParameterKey.String intensityRangeParam = params.createStringField("intensityRange",
			RangeValidatorFactory.fromTo(), Defaults.INTENSITY_RANGE);
		final ParameterKey.Double driftDistParam = params.createDoubleField("driftDist", null,
			Defaults.DRIFT_DISTANCE);
		final ParameterKey.Double driftAngleParam = params.createDoubleField("driftAngle", null,
			Defaults.DRIFT_ANGLE);
		final ParameterKey.String maskPathParam = params.createStringField("maskPath", null,
			Defaults.MASK_PATH);
		final ParameterKey.String backgroundMaskPathParam = params.createStringField("maskPathBg", null,
			Defaults.BG_MASK_PATH);
		ParameterKey.Boolean singleFixedMoleculeParam = params.createBooleanField("singleFixed", null,
			false);
		...
		if (macroOptions != null) {
			...
			GUI.setLookAndFeel();
			DialogStub dialog = new DialogStub(params, IJ.getInstance(),
				"ThunderSTORM: Data generator (16bit grayscale image sequence)")
			{
				...
				protected void layoutComponents() {
					...
					emittersPanel.add(new JLabel("Intensity range (from:to) [photons]:"), GridBagHelper
						.leftCol());
					...
					JPanel pathPanel = new JPanel(new BorderLayout()) {
						...
						public Dimension getPreferredSize() {
							return densityTextField.getPreferredSize();// same size as fields
																													// above
						}
					};
					...
					noisePanel.add(new JLabel("Background intensity mask (optional):"), GridBagHelper
						.leftCol());
					JPanel bgPathPanel = new JPanel(new BorderLayout()) {

						@Override
						public Dimension getPreferredSize() {
							return noiseVarTextField.getPreferredSize();// same size as fields
																													// above
						}
					};
		...
		drift = new Drift(Units.NANOMETER.convertTo(Units.PIXEL, driftDistParam.getValue()),
			driftAngleParam.getValue(), false, frames);
