...
public class MoleculeMatcher {
	 ...
	 * @param TP [out] List of true-positive pairs &lt;ground-truth,detection&gt;
	 *          (the container must be allocated by caller).
	 * @param FP [out] List of false-positive detections (the container must be
	 *          allocated by caller).
	 * @param FN [out] List of false-negative items from ground-truth (the
	 *          container must be allocated by caller).
	 */
	public void matchMolecules(List<Molecule> det, List<Molecule> gt,
		List<Pair<Molecule, Molecule>> TP, List<Molecule> FP, List<Molecule> FN)
	{
		...
		try {
			for (Molecule mol : gt) {
				try {
					...
					// this might theoretically happen if two molecules are located at the
					// same exact spot; but it is very unlikely
				}
			}
			double dist = sqrt(dist2Thr);
			for (Molecule mol : det) {
				FP.add(mol);
				mol.addNeighbors(tree.range(new double[] { mol.getX(distUnits) - dist, mol.getY(distUnits) -
					dist }, new double[] { mol.getX(distUnits) + dist, mol.getY(distUnits) + dist }), threeD,
					dist2Thr, distUnits);
			}
		}
		...
		//
		// Perform the matching in the neighbourhood (given by dist2Thr) of each
		// molecule
		// - note1: `det` must store the neighbors (`gt`)!
		// - note2: returns <det,gt> KV pairs, thus needs to be swapped for further
		// processing
		...
		for (Molecule gtMol : gt) {
			...
			if (detMol != null) {
				...
				detMol.addParam(LABEL_DISTANCE_TO_GROUND_TRUTH_XY, distUnits, gtMol.getDistLateral(detMol,
					distUnits));
				detMol.addParam(LABEL_DISTANCE_TO_GROUND_TRUTH_Z, distUnits, gtMol.getDistAxial(detMol,
					distUnits));
				detMol.addParam(LABEL_DISTANCE_TO_GROUND_TRUTH_XYZ, distUnits, gtMol.getDist(detMol,
					distUnits));
			}
			else {
				FN.add(gtMol);
