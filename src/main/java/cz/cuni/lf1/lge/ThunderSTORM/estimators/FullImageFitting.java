...
public class FullImageFitting implements IEstimator {
		...
		throws StoppedByUserException
	{
		...
		try {
			...
			SubImage subImage = new SubImage(image.getWidth(), image.getHeight(), xgrid, ygrid,
				getImageData(image), detections.get(maxI).x.doubleValue() - x0, detections.get(maxI).y
					.doubleValue() - y0);
