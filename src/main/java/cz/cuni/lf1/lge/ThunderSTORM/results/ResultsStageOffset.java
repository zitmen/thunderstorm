...
public class ResultsStageOffset extends PostProcessingModule {
	...
	public ResultsStageOffset() {
		framesPerStagePositionParam = params.createIntField("framesPerStagePos", IntegerValidatorFactory
			.positiveNonZero(), 1);
		stagePositionsParam = params.createIntField("stagePositions", IntegerValidatorFactory
			.positiveNonZero(), 1);
	...
	protected JPanel createUIPanel() {
		...
		panel.add(new JLabel("Frames per one Z-stage position:", SwingConstants.RIGHT),
			new GridBagHelper.Builder().gridxy(0, 0).fill(GridBagConstraints.HORIZONTAL).weightx(0.1)
				.build());
		panel.add(framesPerStagePositionTextField, new GridBagHelper.Builder().gridxy(1, 0).fill(
			GridBagConstraints.HORIZONTAL).weightx(0.2).build());
		panel.add(new JLabel("Number of Z-stage positions:", SwingConstants.RIGHT),
			new GridBagHelper.Builder().gridxy(0, 1).fill(GridBagConstraints.HORIZONTAL).weightx(0.1)
				.build());
		panel.add(stagePositionsTextField, new GridBagHelper.Builder().gridxy(1, 1).fill(
			GridBagConstraints.HORIZONTAL).weightx(0.2).build());
		panel.add(new JLabel("Z-stage step [nm]:", SwingConstants.RIGHT), new GridBagHelper.Builder()
			.gridxy(2, 0).insets(new Insets(0, 20, 0, 0)).fill(GridBagConstraints.HORIZONTAL).build());
		panel.add(stageStepTextField, new GridBagHelper.Builder().gridxy(3, 0).fill(
			GridBagConstraints.HORIZONTAL).weightx(0.2).build());
		panel.add(new JLabel("Initial Z-stage offset [nm]:", SwingConstants.RIGHT),
			new GridBagHelper.Builder().gridxy(2, 1).insets(new Insets(0, 20, 0, 0)).fill(
				GridBagConstraints.HORIZONTAL).build());
		panel.add(firstPositionOffsetTextField, new GridBagHelper.Builder().gridxy(3, 1).fill(
			GridBagConstraints.HORIZONTAL).weightx(0.1).build());
		panel.add(Box.createGlue(), new GridBagHelper.Builder().gridxy(4, 1).weightx(0.1).build());
		panel.add(Help.createHelpButton(getClass()), new GridBagHelper.Builder().gridxy(5, 0).anchor(
	 ...
	 */
	void applyToModel(int framesPerStagePosition, int stagePositions, double stageStep,
		double firstPositionOffset)
	{
		...
		for (Molecule molecule : molecules) {
			double z = model.columnExists(PSFModel.Params.LABEL_Z_REL) ? molecule.getParam(
				PSFModel.Params.LABEL_Z_REL) : 0;
			int frame = (int) molecule.getParam(MoleculeDescriptor.LABEL_FRAME);
			double newZ = (((frame - 1) / framesPerStagePosition) % stagePositions) * stageStep +
				firstPositionOffset + z;
			molecule.setZ(newZ);
		}
