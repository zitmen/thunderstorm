...
public class DensityRenderingUI extends AbstractRenderingUI {
	...
	public IncrementalRenderingMethod getMethod() {
		if (threeD.getValue()) {
			...
			return new DensityRendering.Builder().roi(left, left + sizeX, top, top + sizeY).resolution(1 /
				magnification.getValue()).defaultDX(dx.getValue() / CameraSetupPlugIn.getPixelSize())
				.forceDefaultDX(forceDx.getValue()).zRange(r.from, r.to, r.step).colorize(colorize
					.getValue()).colorizationLUT(lutPicker.getLut(colorizationLut.getValue())).defaultDZ(dz
						.getValue()).forceDefaultDZ(forceDz.getValue()).build();
		}
		else {
			return new DensityRendering.Builder().roi(left, left + sizeX, top, top + sizeY).resolution(1 /
				magnification.getValue()).defaultDX(dx.getValue() / CameraSetupPlugIn.getPixelSize())
