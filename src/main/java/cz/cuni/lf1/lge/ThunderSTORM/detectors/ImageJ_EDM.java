...
import ij.process.*;
import ij.gui.*;
import static ij.plugin.filter.EDM.BYTE;
import static ij.plugin.filter.EDM.BYTE_OVERWRITE;
import static ij.plugin.filter.EDM.FLOAT;
import static ij.plugin.filter.EDM.ONE;
import static ij.plugin.filter.EDM.SHORT;
...
import static ij.plugin.filter.PlugInFilter.KEEP_THRESHOLD;
 ...
 *   background pixel (for background pixels, the ImageJ_EDM is 0)
 * - Ultimate Eroded Points  (UEPs) are maxima of the ImageJ_EDM. In the output, the points
 *   are assigned the ImageJ_EDM value, which is equal to the radius of the largest circle
 *   that fits into the particle, with the UEP as the center.
 * - Watershed segmentation of the ImageJ_EDM splits particles at "necks"; starting at
 *   maxima of the ImageJ_EDM.
 ...
 *
 * Watershed, Ultimate Eroded Points and Voronoi are handled by the ImageJ_MaximumFinder 
 * plugin applied to the ImageJ_EDM
 * Note: These functions do not take ROIs into account.
 * Setup is called with argument "" (empty string) for ImageJ_EDM,
 ...
 *
 * The ImageJ_EDM algorithm is similar to the 8SSEDT in
 *   F. Leymarie, M. D. Levine, in: CVGIP Image Understanding, vol. 55 (1992), pp 84-94
 *   http://dx.doi.org/10.1016/1049-9660(92)90008-Q
 *
 * The algorithm provides a fast approximation of the ImageJ_EDM, with the deviation from a
 ...
 *
 * Limitations:
 * Maximum image diagonal for ImageJ_EDM: 46340 pixels (sqrt(2^31)); if the particles are
 * dense enough it also works for width, height <=65534.
 *
 * Version 30-Apr-2008 Michael Schmid:  more accurate ImageJ_EDM algorithm,
...
class ImageJ_EDM implements ExtendedPlugInFilter {
    ...
    private ImageJ_MaximumFinder maxFinder = new ImageJ_MaximumFinder();    //we use only one ImageJ_MaximumFinder (nice progress bar)
    ...
    private boolean background255;      //whether background for ImageJ_EDM is 255, not zero
    ...
    public void run (ImageProcessor ip) {
        ...
        int backgroundValue = (processType==VORONOI) ?
                (background255 ? 0 : (byte)255) : //Voronoi needs ImageJ_EDM of the background
                (background255 ? (byte)255 : 0);  //all others do ImageJ_EDM of the foreground
        ...
        if (USES_MAX_FINDER[processType]) {
            if (processType == VORONOI) floatEdm.multiply(-1); //Voronoi starts from minima of ImageJ_EDM
            int maxOutputType = USES_WATERSHED[processType] ? ImageJ_MaximumFinder.SEGMENTED : ImageJ_MaximumFinder.SINGLE_POINTS;
        ...
        } else switch (outImageType) {          //for all these, output contains the values of the ImageJ_EDM
            case FLOAT:
                outIp = floatEdm;
                break;
            case SHORT:
    ...
    /** Do watershed segmentation based on the ImageJ_EDM of the
     ...
     * its distance from the nearest background (zero) pixel, multiplied by ImageJ_EDM.ONE.
     ...
     * @return                  The ImageJ_EDM, containing the distances to the nearest background pixel.
