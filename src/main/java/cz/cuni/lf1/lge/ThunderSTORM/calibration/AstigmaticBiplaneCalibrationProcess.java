...
public class AstigmaticBiplaneCalibrationProcess extends AbstractCalibrationProcess {
	...
	public AstigmaticBiplaneCalibrationProcess(CalibrationConfig config, IFilterUI selectedFilterUI,
		IDetectorUI selectedDetectorUI, AstigmaticBiplaneCalibrationEstimatorUI calibrationEstimatorUI,
		DefocusFunction defocusModel, double stageStep, double zRangeLimit, ImagePlus imp1,
		ImagePlus imp2, Roi roi1, Roi roi2)
	{
		super(config, selectedFilterUI, selectedDetectorUI, calibrationEstimatorUI, defocusModel,
			stageStep, zRangeLimit);
		this.imp1 = imp1;
		this.imp2 = imp2;
		this.roi1 = roi1;
		this.roi2 = roi2;
	...
	protected double guessZ0(PSFSeparator.Position p) {
		...
		for (int i = 0; i < intensity1AsArray.length; i++) {
			double ratio1 = (Math.max(sigma1AsArray[i], sigma2AsArray[i]) / Math.min(sigma1AsArray[i],
				sigma2AsArray[i])) / intensity1AsArray[i];
			double ratio2 = (Math.max(sigma3AsArray[i], sigma4AsArray[i]) / Math.min(sigma3AsArray[i],
				sigma4AsArray[i])) / intensity2AsArray[i];
	...
	protected Map<PSFSeparator.Position, PSFSeparator.Position> fitPositions() {
		...
		beadFits1 = fitFixedAngle(angle1, imp1, roi1, selectedFilterUI, selectedDetectorUI,
			calibrationEstimatorUI, defocusModel, config.showResultsTable);
		beadFits2 = fitFixedAngle(angle2, imp2, roi2, selectedFilterUI, selectedDetectorUI,
			calibrationEstimatorUI, defocusModel, config.showResultsTable);
		...
		transformationMatrix = Homography.estimateTransform(config.ransacTranslationAndFlip,
			config.ransacHomography, (int) roi1.getFloatWidth(), (int) roi1.getFloatHeight(), fits1,
			(int) roi2.getFloatWidth(), (int) roi2.getFloatHeight(), fits2);
		if (transformationMatrix == null) {
			throw new TransformEstimationFailedException(
				"Could not estimate a transform between the planes!");
		}
		return Homography.mergePositions(transformationMatrix,
			new IBinaryTransform<PSFSeparator.Position>()
			{

				@Override
				public void map(PSFSeparator.Position pos1, PSFSeparator.Position pos2) {
					// map both ways, because both points will be used for curve fitting,
					// thus z0 estimate must be the same for both of them!
					pos1.setFromArray(LABEL_SIGMA3, MoleculeDescriptor.Units.PIXEL, pos2.getAsArray(
						LABEL_SIGMA1, MoleculeDescriptor.Units.PIXEL));
					pos1.setFromArray(LABEL_SIGMA4, MoleculeDescriptor.Units.PIXEL, pos2.getAsArray(
						LABEL_SIGMA2, MoleculeDescriptor.Units.PIXEL));
					pos1.setFromArray(LABEL_INTENSITY2, MoleculeDescriptor.Units.PHOTON, pos2.getAsArray(
						LABEL_INTENSITY, MoleculeDescriptor.Units.PHOTON));

					pos2.setFromArray(LABEL_SIGMA3, MoleculeDescriptor.Units.PIXEL, pos1.getAsArray(
						LABEL_SIGMA1, MoleculeDescriptor.Units.PIXEL));
					pos2.setFromArray(LABEL_SIGMA4, MoleculeDescriptor.Units.PIXEL, pos1.getAsArray(
						LABEL_SIGMA2, MoleculeDescriptor.Units.PIXEL));
					pos2.setFromArray(LABEL_INTENSITY2, MoleculeDescriptor.Units.PHOTON, pos1.getAsArray(
						LABEL_INTENSITY, MoleculeDescriptor.Units.PHOTON));
				}
	...
	public void drawOverlay() {
		...
		// cz.cuni.lf1.lge.ThunderSTORM.calibration.AbstractCalibrationProcess.drawOverlay(AbstractCalibrationProcess.java:321)
		// -- it likely comes from LABEL_FRAME array
		// drawOverlay(imp1, roi1, beadFits1.getAllFits(), usedPositions);
		// drawOverlay(imp2, roi2, beadFits2.getAllFits(),
		// Homography.transformPositions(transformationMatrix,
