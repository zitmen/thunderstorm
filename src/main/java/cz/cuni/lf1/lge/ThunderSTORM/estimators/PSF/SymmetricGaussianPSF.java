...
/**
 * Representation of 2D symmetric Gaussian PSFModel model. <strong>Note that
 * this class will be completely changed in a future relase.</strong>
 */
public class SymmetricGaussianPSF extends PSFModel {
	...
	public double getValue(double[] params, double x, double y) {
		...
		return params[Params.OFFSET] + params[Params.INTENSITY] / (twoSigmaSquared * PI) * exp(-((x -
			params[Params.X]) * (x - params[Params.X]) + (y - params[Params.Y]) * (y -
				params[Params.Y])) / twoSigmaSquared);
	}
		...
		final double[] ygrid)
	{
		return new MultivariateMatrixFunction() {
			...
			// d(b^2 + ((J*J)/2/PI/(s*s)/(s*s)) * e^( -( ((x0-x)^2)/(2*s*s*s*s) +
			// (((y0-y)^2)/(2*s*s*s*s)))))/dx
			public double[][] value(double[] point) throws IllegalArgumentException {
				...
				for (int i = 0; i < xgrid.length; i++) {
					...
					// d()/dx
					retVal[i][Params.X] = transformedPoint[Params.INTENSITY] * xd * expValDivPISigmaPowEight *
						0.5;
					// d()/dy
					retVal[i][Params.Y] = transformedPoint[Params.INTENSITY] * yd * expValDivPISigmaPowEight *
						0.5;
					// d()/dsigma
					retVal[i][Params.SIGMA] = transformedPoint[Params.INTENSITY] * expValDivPISigmaPowEight /
						point[Params.SIGMA] * (xd * xd + yd * yd - 2 * sigmaSquared);
					// d()/dbkg
					retVal[i][Params.OFFSET] = 2 * point[Params.OFFSET];
				}
				// IJ.log("numeric jacobian: " +
				// Arrays.deepToString(SymmetricGaussianPSF.super.getJacobianFunction(xgrid,
				// ygrid).value(point)));
