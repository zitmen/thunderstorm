...
public class ScatterRenderingUI extends AbstractRenderingUI {
	...
	public IncrementalRenderingMethod getMethod() {
		if (threeD.getValue()) {
			...
			return new ScatterRendering.Builder().roi(left, left + sizeX, top, top + sizeY).resolution(1 /
				magnification.getValue()).colorize(colorize.getValue()).colorizationLUT(lutPicker.getLut(
					colorizationLut.getValue())).zRange(zrange.from, zrange.to, zrange.step).build();
		}
		else {
			return new ScatterRendering.Builder().roi(left, left + sizeX, top, top + sizeY).resolution(1 /
				magnification.getValue()).build();
		}
	}
}
