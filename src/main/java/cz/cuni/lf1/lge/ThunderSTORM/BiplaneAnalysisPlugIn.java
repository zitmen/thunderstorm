...
import cz.cuni.lf1.lge.ThunderSTORM.UI.BiplaneAnalysisOptionsDialog;
...
/**
 * ThunderSTORM biplane analysis plugin. Open the options dialog, process an two
 * image stacks simultaneously to recieve a list of localized molecules which
 * will get displayed in the {@code ResultsTable} and previed in a new
 * {@code ImageStack} with detections marked as crosses in {@code Overlay} of
...
public final class BiplaneAnalysisPlugIn implements PlugIn {
	...
	public void run(String arg) {
		GUI.setLookAndFeel();
		//
		if (Utils.getOpenImageTitles(true).length < 3) { // 3 = 2 images + 1 empty
																											// string
		...
		try {
			...
			List<IBiplaneEstimatorUI> allEstimators = ModuleLoader.getUIModules(
				IBiplaneEstimatorUI.class);
			List<IRendererUI> allRenderers = ModuleLoader.getUIModules(IRendererUI.class);

			// Create and show the dialog
			BiplaneAnalysisOptionsDialog dialog = new BiplaneAnalysisOptionsDialog(allFilters,
				allDetectors, allEstimators, allRenderers);
			...
			if (((imp1 = dialog.getFirstPlaneStack()) == null) || ((imp2 = dialog
				.getSecondPlaneStack()) == null))
			{
			...
			roi1 = imp1.getRoi() != null ? imp1.getRoi() : new Roi(0, 0, imp1.getWidth(), imp1
				.getHeight());
			roi2 = imp2.getRoi() != null ? imp2.getRoi() : new Roi(0, 0, imp2.getWidth(), imp2
				.getHeight());

			if (roi1.getFloatWidth() != roi2.getFloatWidth() || roi1.getFloatHeight() != roi2
				.getFloatHeight())
			{
			...
			renderingQueue = new RenderingQueue(method, new RenderingQueue.DefaultRepaintTask(
				renderedImage), selectedRendererUI.getRepaintFrequency());
			...
			try {
				...
				Thresholder.setActiveFilter(dialog.getActiveFilterUIIndex()); // !! must
																																			// be
																																			// called
																																			// before
																																			// any
																																			...
																																			// evaluated
																																			// !!
				Thresholder.parseThreshold(selectedDetectorUI.getThreadLocalImplementation()
					.getThresholdFormula());
			}
			catch (Exception ex) {
			...
			rt.reset();
			rt.setOriginalState();
			rt.setMeasurementProtocol(new MeasurementProtocol(imp1, imp2, selectedFilterUI,
				selectedDetectorUI, selectedEstimatorUI));
			...
			Loop.withIndex(1, stackSize + 1, new Loop.BodyWithIndex() {
				...
				public void run(int frame) {
					try {
						analyzeFrame(frame, getRoiProcessor(imp1, roi1, frame), getRoiProcessor(imp2, roi2,
							frame));
			...
			// this is usually caused by incompatibility of calibration file with an
			// estimator
	...
	private FloatProcessor getRoiProcessor(ImagePlus imp, Roi roi, int index) {
		...
		FloatProcessor fp = subtract(ip.crop().convertToFloatProcessor(), (float) CameraSetupPlugIn
			.getOffset());
	...
	private void analyzeFrame(int frame, FloatProcessor fp1, FloatProcessor fp2) {
		...
		List<Molecule> fits = estimator.estimateParameters(fp1, fp2, Point.applyRoiMask(roi1, det1),
			Point.applyRoiMask(roi2, det2));
