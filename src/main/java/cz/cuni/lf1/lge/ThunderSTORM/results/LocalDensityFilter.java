...
public class LocalDensityFilter extends PostProcessingModule {
	...
	public LocalDensityFilter() {
		distanceRadius = params.createDoubleField("radius", DoubleValidatorFactory.positiveNonZero(),
			50.0);
	...
	protected JPanel createUIPanel() {
		...
		filterPanel.add(distanceLabel, new GridBagHelper.Builder().gridxy(0, 0).weightx(0.3).anchor(
			GridBagConstraints.EAST).build());
		filterPanel.add(distanceTextField, new GridBagHelper.Builder().gridxy(1, 0).weightx(0.3).anchor(
			GridBagConstraints.WEST).build());
		filterPanel.add(twoRadioButton, new GridBagHelper.Builder().gridxy(2, 0).weightx(0.2).anchor(
			GridBagConstraints.WEST).build());
		filterPanel.add(threeRadioButton, new GridBagHelper.Builder().gridxy(3, 0).weightx(0.2).anchor(
			GridBagConstraints.WEST).build());
		filterPanel.add(Help.createHelpButton(getClass()), new GridBagHelper.Builder().gridxy(4, 0)
			.anchor(GridBagConstraints.EAST).build());
		filterPanel.add(neighborsLabel, new GridBagHelper.Builder().gridxy(0, 1).weightx(0.5).anchor(
			GridBagConstraints.EAST).build());
		filterPanel.add(neighborsTextField, new GridBagHelper.Builder().gridxy(1, 1).weightx(0.5)
				...
				public Void doJob() {
					...
					if (is3D()) {
						for (int i = 0; i < coords.length; i++) {
							...
							coords[i] = new double[] { m.getX(MoleculeDescriptor.Units.NANOMETER), m.getY(
								MoleculeDescriptor.Units.NANOMETER), m.getZ(MoleculeDescriptor.Units.NANOMETER) };
						}
					}
					else {
						for (int i = 0; i < coords.length; i++) {
					...
					for (int i = 0; i < coords.length; i++) {
						filter[i] = ((tree.ballQuery(coords[i], radius).size() - 1) >= countThr); // -1
																																											// -->
																																											// do
																																											// not
																																											// count
