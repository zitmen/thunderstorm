...
/**
 * Coordinate based colocalization. <br/>
...
public class CBC {
	...
	public CBC(double[][] firstChannelCoords, double[][] secondChannelCoords, double radiusStep,
		int stepCount)
	{
	...
	public double[] calculateFirstChannelCBC() {
		firstChannelNeighborsInDistance =
			new double[squaredRadiusDomain.length][firstChannelCoords.length];
		firstChannelNearestNeighborDistances = new double[firstChannelCoords.length];
		return calc(firstChannelCoords, firstChannelTree, secondChannelTree,
			firstChannelNeighborsInDistance, firstChannelNearestNeighborDistances);
	}

	public double[] calculateSecondChannelCBC() {
		secondChannelNeighborsInDistance =
			new double[squaredRadiusDomain.length][secondChannelCoords.length];
		secondChannelNearestNeighborDistances = new double[secondChannelCoords.length];
		return calc(secondChannelCoords, secondChannelTree, firstChannelTree,
			secondChannelNeighborsInDistance, secondChannelNearestNeighborDistances);
	}

	/**
	 * If channel1 == channel2 (both res-tables or both gt-tables), avoid
	 * self-counting, i.e., distance to nearest neighbor must not be 0! On the
	 * other hand if comparing res-table with gt-table then self-counting is
	 * allowed even if the data in the tables are the same.
	 */
	private double[] calc(final double[][] mainChannelCoords, final KDTree<double[]> mainChannelTree,
		final KDTree<double[]> otherChannelTree, final double[][] neighborsInDistance,
		final double[] nearestNeighborDistances)
	{
		...
		Loop.withIndex(0, mainChannelCoords.length, new Loop.BodyWithIndex() {
			...
			public void run(int i) {
				try {
					double[] counts = calcNeighborCount(mainChannelCoords[i], mainChannelTree,
						squaredRadiusDomain, (firstChannelCoords == secondChannelCoords));
					for (int j = 0; j < counts.length; j++) {
						counts[j] = counts[j] / counts[lastRadiusIndex] * maxSquaredRadius /
							squaredRadiusDomain[j];
					}

					double[] counts2 = calcNeighborCount(mainChannelCoords[i], otherChannelTree,
						squaredRadiusDomain, (firstChannelCoords == secondChannelCoords));
					nearestNeighborDistances[i] = getDistanceToNearestNeighbor(mainChannelCoords[i],
						otherChannelTree, (firstChannelCoords == secondChannelCoords));
					...
					double result = correlation * MathProxy.exp(-nnDistance / MathProxy.sqrt(
						maxSquaredRadius));
					cbcResults[i] = result;
					if (i % 1024 == 0) {
	...
	private double[] calcNeighborCount(double[] queryPoint, KDTree<double[]> kdtree,
		double[] squaredRadiusValues, boolean checkNotSelf)
	{
		assert queryPoint != null;
		assert squaredRadiusValues != null;
		assert kdtree != null;

		List<KDTree.DistAndValue<double[]>> neighbors = kdtree.ballQuery(queryPoint, Math.sqrt(
			squaredRadiusValues[squaredRadiusValues.length - 1]));
	...
	private void buildKdTrees(double[][] secondChannelCoords) throws RuntimeException {
		// build one tree in separate thread, use Loop's executor so we do not
