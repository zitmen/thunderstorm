...
public class EllipticGaussianWAnglePSF extends PSFModel {
	...
	public double getValue(double[] params, double x, double y) {
		...
		return params[Params.INTENSITY] / (2 * PI * params[Params.SIGMA1] * params[Params.SIGMA2]) *
			exp(-0.5 * (sqr(dx / params[Params.SIGMA1]) + sqr(dy / params[Params.SIGMA2]))) +
	...
	@Override
	public MultivariateMatrixFunction getJacobianFunction(final double[] xgrid,
		final double[] ygrid)
	{
		return new MultivariateMatrixFunction() {
			...
			// d(b^2 + ((J*J)/(2*PI*(s1*s1)*(s2*s2))) * e^( -(
			// (((x0-x)*cos(f)-(y0-y)*sin(f))^2)/(2*s1*s1*s1*s1) +
			// ((((x0-x)*sin(f)+(y0-y)*cos(f))^2)/(2*s2*s2*s2*s2)))))/dJ
			public double[][] value(double[] point) throws IllegalArgumentException {
				...
				for (int i = 0; i < xgrid.length; i++) {
					...
					double oneDivPISS2 = 1 / (PI * transformedPoint[Params.SIGMA1] *
						transformedPoint[Params.SIGMA2]);
					// d()/dx
					double pom = (cosfiXd - cosfi * sinfiYd) / sigma1Squared + (sinfiXd + sinfi * cosfiYd) /
						sigma2Squared;
					retVal[i][Params.X] = oneDivPISS2 * 0.5 * transformedPoint[Params.INTENSITY] * pom *
						expVal;
					// d()/dy
					double pom2 = (cosfi * sinfiXd + cosfiYd) / sigma2Squared - (sinfi * cosfiXd - sinfiYd) /
						sigma1Squared;
					retVal[i][Params.Y] = oneDivPISS2 * 0.5 * transformedPoint[Params.INTENSITY] * pom2 *
						expVal;
					// d()/dIntensity
					retVal[i][Params.INTENSITY] = point[Params.INTENSITY] * expVal * oneDivPISS2;
					// d()/dsigma1
					retVal[i][Params.SIGMA1] = transformedPoint[Params.INTENSITY] * expVal * oneDivPISS2 /
						point[Params.SIGMA1] * (-1 + sqr(first) / sigma1Squared);
					// d()/dsigma2
					retVal[i][Params.SIGMA2] = transformedPoint[Params.INTENSITY] * expVal * oneDivPISS2 /
						point[Params.SIGMA2] * (-1 + sqr(second) / sigma2Squared);
					// d()/dbkg
					retVal[i][Params.OFFSET] = 2 * point[Params.OFFSET];
					// d()/dfi
					double pom3 = -(cosfiXd - sinfiYd) * (-sinfiXd - cosfiYd) / sigma1Squared - (sinfiXd +
						cosfiYd) * (cosfiXd - sinfiYd) / sigma2Squared;
					retVal[i][Params.ANGLE] = 0.5 * transformedPoint[Params.INTENSITY] * pom3 * expVal *
						oneDivPISS2;
				}
...
//          IJ.log("analytic jacobian: " + Arrays.deepToString(retVal));
	...
	public double[] getInitialParams(SubImage subImage) {
		...
		guess[Params.INTENSITY] = (subImage.getMax() - subImage.getMin()) * 2 * PI * defaultSigma *
			defaultSigma;
	...
	{
		Molecule mol = new Molecule(new Params(new int[] { Params.X, Params.Y, Params.SIGMA1,
			Params.SIGMA2, Params.ANGLE, Params.INTENSITY, Params.OFFSET, Params.BACKGROUND }, params,
			true));
