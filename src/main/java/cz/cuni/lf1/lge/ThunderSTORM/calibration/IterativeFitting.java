...
public class IterativeFitting {
	...
	private double[] fit(double[] x, double[] y, ParametricUnivariateFunction function,
		double[] initialParams, int maxIter)
	{
		...
		CurveFitter<ParametricUnivariateFunction> fitter =
			new CurveFitter<ParametricUnivariateFunction>(new LevenbergMarquardtOptimizer(
				new SimplePointChecker(10e-3, 10e-3, maxIter)));
...
//          System.out.println("inliers : " + Arrays.toString(inliers));
	...
	public DefocusFunction fitParams(DefocusFunction defocusModel, double[] x, double[] y,
		int maxIter)
	{
		double min = y[0];
		int minIndex = 0;
		...
		return defocusModel.getNewInstance(defocusModel.transformParams(fit(x, y, defocusModel
			.getFittingFunction(), defocusModel.transformParamsInverse(defocusModel.getInitialParams(
				x[minIndex], min)), maxIter)), false);
	}

	private void computeResiduals(double[] parameters, ParametricUnivariateFunction function,
		double[] x, double[] y, double[] residualArray)
	{
		for (int i = 0; i < x.length; i++) {
