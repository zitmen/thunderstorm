...
public class MultipleLocationsImageFitting implements IEstimator {
		...
		for (int i = 0; i < locations.size(); i++) {
			...
			if (!isCloseToBorder(xInt, yInt)) {
				try {
					...
					// new ImagePlus(String.valueOf(i),new
					// FloatProcessor(2*subimageSize+1, 2*subimageSize+1,
					// subimageData)).show();
					SubImage subImage = new SubImage(2 * subimageSize + 1, 2 * subimageSize + 1, xgrid, ygrid,
						subimageData, locations.get(i).getX().doubleValue() - xInt, locations.get(i).getY()
							.doubleValue() - yInt);
	...
	@Override
	public List<Molecule> estimateParameters(ij.process.FloatProcessor image, List<Point> detections)
	...
	public static void appendGoodnessOfFit(Molecule mol, IOneLocationFitter fitter,
		SubImage subimage)
	{
		LSQFitter lsqfit;
		...
		double chi2 = lsqfit.psfModel.getChiSquared(subimage.xgrid, subimage.ygrid, subimage.values,
	...
	public static void appendCalculatedUncertainty(Molecule mol) {
		try {
			...
			if (protocol != null) {
				mol.addParam(MoleculeDescriptor.Fitting.LABEL_UNCERTAINTY_XY,
					MoleculeDescriptor.Units.NANOMETER, uncertaintyXY(mol));
				if (protocol.is3D()) {
					mol.addParam(MoleculeDescriptor.Fitting.LABEL_UNCERTAINTY_Z,
						MoleculeDescriptor.Units.NANOMETER, uncertaintyZ(mol));
				}
			}
		}
