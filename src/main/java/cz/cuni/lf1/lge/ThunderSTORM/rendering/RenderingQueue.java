...
public class RenderingQueue {
	 ...
	 *          repaintFrequency tasks. Or null if no periodic action should be
	 *          run.
	 * @param repaintFrequency specifies how often to call repaintTask.run().
	 */
	public RenderingQueue(IncrementalRenderingMethod method, Runnable repaintTask,
		int repaintFrequency)
	{
		if (method != null && repaintFrequency > 0) {
			...
			// executor with one thread, that will die after not being used for 30
			// seconds
	...
	class RenTask implements Runnable {
		...
		public void run() {
			...
			taskCounter++; // no need to use atomic counter because this method will
											// be run only from single threaded executor
