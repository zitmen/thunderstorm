...
public class VectorMath {
	 ...
	 *         value to highest value and picking the middle one (eg, the median
	 *         of {3, 5, 9} is 5). If there is an even number of observations,
	 *         then there is no single middle value; the median is then defined to
	...
	public static Number median(Number[] arr) {
		...
		if ((sorted.length % 2) == 1) {
			...
			return new Double((sorted[sorted.length / 2].doubleValue() + sorted[1 + sorted.length / 2]
				.doubleValue()) / 2.0);
		}
	}
	...
	public static Double[] mod(Number[] arr1, Number[] arr2) {
		if (arr1.length != arr2.length) {
			throw new FormulaParserException(
				"When performing modulo operation of two vectors (item-by-item), both must be of the same size!");
		}
	...
	public static Double[] div(Number[] arr1, Number[] arr2) {
		if (arr1.length != arr2.length) {
			throw new FormulaParserException(
				"When dividing two vectors (item-by-item), both must be of the same size!");
		}
		Double[] res = new Double[arr1.length];
		for (int i = 0; i < arr1.length; i++) {
	...
	public static Double[] mul(Number[] arr1, Number[] arr2) {
		if (arr1.length != arr2.length) {
			throw new FormulaParserException(
				"When multiplying two vectors, both must be of the same size!");
		}
		Double[] res = new Double[arr1.length];
		for (int i = 0; i < arr1.length; i++) {
			res[i] = new Double(arr1[i].doubleValue() * arr2[i].doubleValue());
	...
	public static Double[] sub(Number[] arr1, Number[] arr2) {
		if (arr1.length != arr2.length) {
			throw new FormulaParserException(
				"When subtracting two vectors, both must be of the same size!");
		}
		Double[] res = new Double[arr1.length];
		for (int i = 0; i < arr1.length; i++) {
			res[i] = new Double(arr1[i].doubleValue() - arr2[i].doubleValue());
	...
	public static double[] movingAverage(double[] values, int lag) {
		...
		for (int i = 0, j = mavg.length - 1, l = lag / 2; i < mavg.length; i++, j--) {
			double count = (double) Math.min(lag - Math.max(Math.max(0, l - i), Math.max(0, l - j)),
				mavg.length);
			mavg[i] /= count;
		}
		return mavg;
