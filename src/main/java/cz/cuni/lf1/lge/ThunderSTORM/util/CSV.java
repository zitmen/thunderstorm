...
/**
 * Importing CSV files and translating them into internal plugin objects
 * (FloatProcessor, PSFModel, Point).
 */
public class CSV {
	 ...
	 * @throws IOException if the input file specified by {@fname was not found or
	...
	public static FloatProcessor csv2fp(String fname) throws IOException, InvalidObjectException {
		...
		if (lines.size() < 1) throw new InvalidObjectException(
			"CSV data have to be in a full square/rectangle matrix!");
		if (lines.get(0).length < 1) throw new InvalidObjectException(
			"CSV data have to be in a full square/rectangle matrix!");
	...
	/**
	 * Read an input CSV file and interpret the data as a set of PSFs (Point
	 * Spread Functions). The input data are supposed to be in the following
	 * format:
	 * 
	 * <pre>
	 * {@code x,y,sigma,Intensity}
	 * </pre>
	 * 
	 * The background parameter is by default set to zero and it is not expected
	 ...
	 * @see PSFModel
	 */
	public static Vector<Molecule> csv2psf(String fname, int start_row, int start_col)
		throws IOException, InvalidObjectException, Exception
	{
		List<String[]> lines = readCsv(fname);
		...
		int[] params = new int[] { PSFModel.Params.X, PSFModel.Params.Y, PSFModel.Params.INTENSITY,
			PSFModel.Params.SIGMA, PSFModel.Params.BACKGROUND };
		for (int r = start_row, rm = lines.size(); r < rm; r++) {
			loc.add(new Molecule(new Params(params, new double[] { Float.parseFloat(lines.get(
				r)[start_col + 0]), // x
