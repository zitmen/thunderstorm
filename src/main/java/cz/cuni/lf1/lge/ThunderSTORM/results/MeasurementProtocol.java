...
public class MeasurementProtocol {
	...
	public MeasurementProtocol(ImagePlus analyzedImage, IFilterUI filter, IDetectorUI detector,
	...
	public MeasurementProtocol(ImagePlus analyzedPlane1, ImagePlus analyzedPlane2, IFilterUI filter,
		IDetectorUI detector, IBiplaneEstimatorUI biplaneEstimator)
	{
		this.version = "ThunderSTORM (" + ThunderSTORM.INSTANCE.getVERSION() + ")";
		this.imageInfo = getImageInfo(analyzedPlane1); // TODO: append the second
																										// image info!!!
		this.cameraSettings = CameraSetupPlugIn.exportSettings();
		this.analysisFilter = filter;
		this.analysisDetector = detector;
	...
	public boolean is3D() {
		...
		if (!this.isSet3d) {
			this.is3d = ((analysisEstimator != null) &&
				(analysisEstimator instanceof EllipticGaussianEstimatorUI));
			this.isSet3d = true;
		}
		return this.is3d;
	}
	...
	public void exportToFile(String fpath) {
		...
		try {
			...
			this.postProcessing = IJResultsTable.getResultsTable().getOperationHistoryPanel()
				.getHistory(); // update
	...
	private static class FilterAdapter implements JsonDeserializer<IFilterUI> {
		...
		@Override
		public IFilterUI deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
	...
	private static class DetectorAdapter implements JsonDeserializer<IDetectorUI> {
		...
		@Override
		public IDetectorUI deserialize(JsonElement json, Type typeOfT,
	...
	private static class EstimatorAdapter implements JsonDeserializer<IEstimatorUI> {
		...
		@Override
		public IEstimatorUI deserialize(JsonElement json, Type typeOfT,
			JsonDeserializationContext context) throws JsonParseException
		{
			String name = json.getAsJsonObject().get("name").getAsString();
	...
	private static class BiplaneEstimatorAdapter implements JsonDeserializer<IBiplaneEstimatorUI> {
		...
		@Override
		public IBiplaneEstimatorUI deserialize(JsonElement json, Type typeOfT,
			JsonDeserializationContext context) throws JsonParseException
		{
			String name = json.getAsJsonObject().get("name").getAsString();
			for (IBiplaneEstimatorUI biplaneEstimator : ModuleLoader.getUIModules(
	...
	private static class CylindricalLensCalibrationAdapter implements
		JsonDeserializer<DefocusCalibration>
	{

		@Override
		public DefocusCalibration deserialize(JsonElement json, Type typeOfT,
			JsonDeserializationContext context) throws JsonParseException
		{
			String name = json.getAsJsonObject().get("name").getAsString();
