...
public class CrowdedFieldEstimatorUI {
	...
	public CrowdedFieldEstimatorUI() {
		...
		ParameterTracker.Condition enabledAndFixedIntensityCondition =
			new ParameterTracker.Condition()
			{
		...
		PVALUE = params.createDoubleField("pvalue", DoubleValidatorFactory.positive(), 1e-6,
			enabledCondition);
		KEEP_SAME_INTENSITY = params.createBooleanField("keep_same_intensity", null, true,
			enabledCondition);
	...
	public JPanel getOptionsPanel(JPanel panel) {
		...
		final JCheckBox keepSameIntensityCheckBox = new JCheckBox("Same intensity for all molecules",
			false);
		final JCheckBox isFixedIntensityCheckBox = new JCheckBox("Limit intensity range [photons]:",
			true);
		...
		isEnabledCheckbox.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				expectedIntensityTextField.setEnabled(isEnabledCheckbox.isSelected() &&
					isFixedIntensityCheckBox.isSelected());
			}
		});
	...
	MFA_MLEFitter getMLEImplementation(PSFModel psf, double sigma) {
		loadValues();
		return new MFA_MLEFitter(psf, sigma, getMaxMolecules(), getPValue(), isKeepSameIntensity(),
			isFixedIntensity() ? getIntensityRange() : null);
	}
	...
	MFA_LSQFitter getLSQImplementation(PSFModel psf, double sigma) {
		loadValues();
		return new MFA_LSQFitter(psf, sigma, getMaxMolecules(), getPValue(), isKeepSameIntensity(),
			isFixedIntensity() ? getIntensityRange() : null);
	}
