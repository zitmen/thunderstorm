...
public class ModuleLoader {
	 ...
	 * {@link ServiceLoader} for more details. <br/>
	 * The implementation must provide a no-arguments constructor so the module
	 * can be instantiated. If there is an error while loading a module, the error
	 * is logged and the method attempts to continue loading other modules.
	 * Exception is thrown only when no modules are succesfully loaded.
	 *
	...
	public static <T extends IModuleUI> ArrayList<T> getUIModules(Class<T> c) {
		// workaround a bug when service loading does not work after refreshing
		// menus in ImageJ
		...
		try {
			...
			for (Iterator<T> it = loader.iterator(); it.hasNext();) {
				// when something goes wrong while loading modules, log the error and
				// try to continue
				try {
					retval.add(it.next());
				}
	...
	public static List<PostProcessingModule> getPostProcessingModules() {
		...
		if (retval.isEmpty()) {
			// throw exception only when no modules are succesfully loaded
			throw new RuntimeException("No modules of type " + PostProcessingModule.class
