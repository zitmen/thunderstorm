...
public class ASHRenderingUI extends AbstractRenderingUI {
	...
	private void initPars() {
		...
		zShifts = parameters.createIntField("zshifts", IntegerValidatorFactory.positiveNonZero(), 2,
			threeDCondition);
	}
	...
	public IncrementalRenderingMethod getMethod() {
		if (parameters.getBoolean(threeD)) {
			...
			return new ASHRendering.Builder().roi(left, left + sizeX, top, top + sizeY).resolution(1 /
				magnification.getValue()).shifts(shifts.getValue()).zRange(r.from, r.to, r.step).colorize(
					colorize.getValue()).colorizationLUT(lutPicker.getLut(colorizationLut.getValue()))
				.zShifts(zShifts.getValue()).build();
		}
		else {
			return new ASHRendering.Builder().roi(left, left + sizeX, top, top + sizeY).resolution(1 /
				magnification.getValue()).shifts(shifts.getValue()).build();
		}
	}
}
