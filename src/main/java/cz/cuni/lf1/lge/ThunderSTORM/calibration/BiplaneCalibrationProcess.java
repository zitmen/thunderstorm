...
public class BiplaneCalibrationProcess extends AbstractCalibrationProcess {
	...
	public BiplaneCalibrationProcess(CalibrationConfig config, IFilterUI selectedFilterUI,
		IDetectorUI selectedDetectorUI, BiplaneCalibrationEstimatorUI calibrationEstimatorUI,
	...
	public DefocusCalibration getCalibration(DefocusFunction defocusModel) {
		DefocusCalibration cal1 = defocusModel.getCalibration(angle, null, polynomS1Final,
			polynomS1Final);
		DefocusCalibration cal2 = defocusModel.getCalibration(angle, null, polynomS2Final,
			polynomS2Final);
	...
	protected Collection<Position> fitPositions() {
		angle = 0.0;
		List<Position> fits1 = filterPositions(fitFixedAngle(angle, imp1, roi1, selectedFilterUI,
			selectedDetectorUI, calibrationEstimatorUI, defocusModel, config.showResultsTable),
			config.minimumFitsCount);
		List<Position> fits2 = filterPositions(fitFixedAngle(angle, imp2, roi2, selectedFilterUI,
			selectedDetectorUI, calibrationEstimatorUI, defocusModel, config.showResultsTable),
			config.minimumFitsCount);
		...
		return Homography.mergePositions(transformationMatrix, new IBinaryTransform<Position>() {
			...
			fits1, (int) roi2.getFloatWidth(), (int) roi2.getFloatHeight(), fits2).keySet();
	}
}
