...
public class CBCPlugIn implements PlugIn {
	...
	public void run(String arg) {
		...
		try {
			...
			runCBC(dialog.getChannel1Table(), dialog.getChannel2Table(), dialog.is3D(), dialog.radiusStep
				.getValue(), dialog.stepCount.getValue(), dialog.addCBC.getValue(), dialog.addNNDist
					.getValue(), dialog.addNNCount.getValue());
	...
	public void runCBC(GenericTable channel1Table, GenericTable channel2Table, boolean is3D,
		double radiusStep, int stepCount, boolean addCBCToTable, boolean addNNDistToTable,
		boolean addNeighborsInDistToTable)
	{
		...
		if (is3D) {
			for (int i = 0; i < firstCoords.length; i++) {
				...
				firstCoords[i] = new double[] { m.getX(Units.NANOMETER), m.getY(Units.NANOMETER), m.getZ(
					Units.NANOMETER) };
			}
		}
		else {
		...
		if (channel1Table != channel2Table) {
			...
			if (is3D) {
				for (int i = 0; i < secondCoords.length; i++) {
					...
					secondCoords[i] = new double[] { m.getX(Units.NANOMETER), m.getY(Units.NANOMETER), m.getZ(
						Units.NANOMETER) };
				}
			}
			else {
		...
		if (addCBCToTable) addColumnToModel(channel1Table.getModel(), firstChannelCBC, "cbc",
			Units.UNITLESS);
		if (addNNDistToTable) addColumnToModel(channel1Table.getModel(),
			cbc.firstChannelNearestNeighborDistances, "nn_dist", Units.NANOMETER);
		if (addNeighborsInDistToTable) {
			for (int i = 0; i < cbc.squaredRadiusDomain.length; i++) {
				addColumnToModel(channel1Table.getModel(), cbc.firstChannelNeighborsInDistance[i],
					"neighbors_in_dist_" + ((int) ((i + 1) * radiusStep)), Units.UNITLESS);
			}
		}

		double[] x1 = channel1Table.getColumnAsDoubles(PSFModel.Params.LABEL_X,
			MoleculeDescriptor.Units.PIXEL);
		double[] y1 = channel1Table.getColumnAsDoubles(PSFModel.Params.LABEL_Y,
			MoleculeDescriptor.Units.PIXEL);
		double[] x2 = channel2Table.getColumnAsDoubles(PSFModel.Params.LABEL_X,
			MoleculeDescriptor.Units.PIXEL);
		double[] y2 = channel2Table.getColumnAsDoubles(PSFModel.Params.LABEL_Y,
			MoleculeDescriptor.Units.PIXEL);
		...
		RenderingMethod renderer = new ASHRendering.Builder().shifts(2).zShifts(1).colorize(true)
			.zRange(-1, 1, 0.1).roi(0, maxRoiX, 0, maxRoiY).resolution(0.2).build();
		...
		if (channel1Table != channel2Table) {
			...
			if (addCBCToTable) addColumnToModel(channel2Table.getModel(), secondChannelCBC, "cbc",
				Units.UNITLESS);
			if (addNNDistToTable) addColumnToModel(channel2Table.getModel(),
				cbc.secondChannelNearestNeighborDistances, "nn_dist", Units.NANOMETER);
			if (addNeighborsInDistToTable) {
				for (int i = 0; i < cbc.squaredRadiusDomain.length; i++) {
					addColumnToModel(channel2Table.getModel(), cbc.secondChannelNeighborsInDistance[i],
						"neighbors_in_dist_" + ((int) ((i + 1) * radiusStep)), Units.UNITLESS);
				}
			}

			RenderingMethod renderer2 = new ASHRendering.Builder().shifts(2).zShifts(1).colorize(true)
				.zRange(-1, 1, 0.1).roi(0, maxRoiX, 0, maxRoiY).resolution(0.2).build();
	...
	private void addColumnToModel(GenericTableModel model, final double[] data, String title,
		Units units)
	{
	...
	class CBCDialog extends DialogStub {
		...
		public CBCDialog(Window owner) {
			...
			radiusStep = params.createIntField("radiusStep", IntegerValidatorFactory.positiveNonZero(),
				50);
			dimensions = params.createStringField("dimensions", StringValidatorFactory.isMember(dim),
				dim[0]);
			stepCount = params.createIntField("stepCount", IntegerValidatorFactory.rangeInclusive(2,
				10000), 10);
			channel1 = params.createStringField("channel1", StringValidatorFactory.isMember(tables),
				tables[0]);
			channel2 = params.createStringField("channel2", StringValidatorFactory.isMember(tables),
				tables[1]);
		...
		protected void layoutComponents() {
			...
			JCheckBox addNNDistCheckBox = new JCheckBox(
				"Add distance to the nearest neighbor into the results table");
			JCheckBox addNNCountCheckBox = new JCheckBox(
				"Add count of neighbors within the radius into the results table");
			...
			dimPanel.add(twoDimRadioButton, new GridBagHelper.Builder().gridxy(0, 0).weightx(0.2).anchor(
				GridBagConstraints.WEST).build());
			dimPanel.add(threeDimRadioButton, new GridBagHelper.Builder().gridxy(1, 0).weightx(0.2)
				.anchor(GridBagConstraints.WEST).build());
			dimPanel.add(Box.createHorizontalGlue(), new GridBagHelper.Builder().gridxy(2, 0).weightx(0.6)
				.anchor(GridBagConstraints.WEST).build());
			...
			buttons.add(Box.createHorizontalGlue(), new GridBagHelper.Builder().fill(
				GridBagConstraints.HORIZONTAL).weightx(1).build());
		...
		protected GenericTable getChannelTable(ParameterKey.String channel) throws Exception {
			...
			if (!table.columnExists(PSFModel.Params.LABEL_X) || !table.columnExists(
