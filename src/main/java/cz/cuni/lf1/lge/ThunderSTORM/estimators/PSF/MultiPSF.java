...
public class MultiPSF extends PSFModel {
	...
	public double getValue(double[] params, double x, double y) {
		...
		for (int i = 0; i < nmol; i++) {
			double[] tmp = Arrays.copyOfRange(params, i * Params.PARAMS_LENGTH, (i + 1) *
	...
	public double[] transformParameters(double[] params) {
		...
		return transformed; // values of Intensity and offset must be the same for
												// each `multi`-molecule
	}
		...
		final double[] ygrid)
	{
		return new MultivariateMatrixFunction() {
			...
			public double[][] value(double[] point) throws IllegalArgumentException {
				...
				for (int i = 0; i < nmol; i++) {
					double[] tmp = Arrays.copyOfRange(point, i * Params.PARAMS_LENGTH, (i + 1) *
						Params.PARAMS_LENGTH);
	...
	public double[] getInitialParams(SubImage subImage) {
		if (n_1_params == null) {
			...
			double[] residual = subImage.subtract(getValueFunction(subImage.xgrid, subImage.ygrid).value(
				n_1_params));
			...
			SubImage img = new SubImage(subImage.size_x, subImage.size_y, subImage.xgrid, subImage.ygrid,
				residual, subImage.xgrid[max_i], subImage.ygrid[max_i]);
			// get the initial guess for Nth molecule
			System.arraycopy(psf.getInitialParams(img), 0, guess, (nmol - 1) * Params.PARAMS_LENGTH,
				Params.PARAMS_LENGTH);
			// perform push&pull adjustment -- to close to the boundary? push out;
			// else pull in;
	...
	{ // returns one `macro-molecule`
		double[] tmp = Arrays.copyOfRange(params, 0, Params.PARAMS_LENGTH);
		Molecule macroMol = psf.newInstanceFromParams(tmp, subImageUnits, afterFitting); // init...if
																																											// there
																																											// is
																																											// more
																																											// than
	...
	public double getDoF() {
		return psf.getDoF() + (nmol - 1) * (psf.getDoF() - 2); // both intensity and
																														// offset are
																														// estimated for all
																														// molecules as a
																														// single parameter
