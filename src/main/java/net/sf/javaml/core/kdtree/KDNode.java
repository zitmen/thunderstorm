...
class KDNode<T> {
    ...
    protected static <T> KDNode ins(HPoint key, T val, KDNode<T> t, int lev, int K) throws KeyDuplicateException {

        if (t == null) {
            t = new KDNode<T>(key, val);
        }
    ...
    protected static <T> KDNode srch(HPoint key, KDNode<T> t, int K) {
    ...
    protected static <T> void rsearch(HPoint lowk, HPoint uppk, KDNode<T> t, int lev, int K, Vector<KDNode<T>> v) {
    ...
    protected static <T> void nnbr(KDNode<T> kd, HPoint target, HRect hr, double max_dist_sqd, int lev, int K,
        ...
        KDNode<T> nearest = (KDNode<T>) nnl.getHighest();
        double dist_sqd;
        ...
        if (HPoint.eucdist(closest, target) < Math.sqrt(max_dist_sqd)) {
            ...
            KDNode<T> temp_nearest = (KDNode<T>) nnl.getHighest();
