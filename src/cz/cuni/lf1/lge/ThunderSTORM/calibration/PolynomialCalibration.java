...
public class PolynomialCalibration implements CylindricalLensCalibration {
  ...
  /**
   * Z calculation as from the supplement of this paper:
  ...
  public double getZZhuang(final double sigma1, final double sigma2) {
    NonLinearConjugateGradientOptimizer optim = new NonLinearConjugateGradientOptimizer(NonLinearConjugateGradientOptimizer.Formula.POLAK_RIBIERE, new SimplePointChecker<PointValuePair>(1e-10, 1e-10));
    ...
    PointValuePair result = optim.optimize(gradientFunction, distanceFunction, new InitialGuess(new double[]{0}), new MaxEval(3000));
