...
import static cz.cuni.lf1.lge.ThunderSTORM.utils.Math.exp;
import static cz.cuni.lf1.lge.ThunderSTORM.utils.Math.pow;
import static cz.cuni.lf1.lge.ThunderSTORM.utils.Math.PI;
...
public class LeastSquaresEstimator implements IEstimator, IModule {
    ...
    public static class Gaussian extends LMAMultiDimFunction {
        ...
        public double getY(double x[], double[] a) {
            ...
            return a[2]/2.0/PI/sqr(a[3]) * exp(-(sqr(x[0]-a[0]) + sqr(x[1]-a[1])) / 2.0 / sqr(a[3])) + a[4];
        }
        ...
        public double getPartialDerivate(double x[], double[] a, int parameterIndex) {
            ...
            switch (parameterIndex) {
                case 0: return a[2]/2.0/PI/pow(a[3],4) * (x[0]-a[0]) * exp(-arg/2.0/sqr(a[3])); // x0
                case 1: return a[2]/2.0/PI/pow(a[3],4) * (x[1]-a[1]) * exp(-arg/2.0/sqr(a[3])); // y0
                case 2: return exp(-arg/2.0/sqr(a[3])) / 2.0 / PI / sqr(a[3]); // Intensity
                case 3: return a[2]/2.0/PI/pow(a[3],5) * (arg - 2.0 * sqr(a[3])) * exp(-arg/2.0/sqr(a[3])); // sigma
