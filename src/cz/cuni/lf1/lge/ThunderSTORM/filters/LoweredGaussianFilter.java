 ...
 * 
 * Such a Gaussian can be calculated as {@code G(sigma,size) - mean(G(sigma,size))}.
 * 
 * This filter uses the the same trick with the separable kernels as DoG filter.
 * The only difference here is that one of the filters is the Gaussian filter and
 * the other one is an uniform filter.
 * 
