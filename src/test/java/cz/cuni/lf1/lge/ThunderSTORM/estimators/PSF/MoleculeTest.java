...
public class MoleculeTest {
	...
	@Before
	public void setUp() {
		labels = new int[] { PSFModel.Params.X, PSFModel.Params.Y, PSFModel.Params.SIGMA1,
			PSFModel.Params.SIGMA2, PSFModel.Params.INTENSITY, PSFModel.Params.OFFSET,
			PSFModel.Params.BACKGROUND };
	...
	public void testAddParam() {
		...
		Molecule m = mol.clone(mol.descriptor.clone()); // clone so the `mol`
																										// remains the same
	...
	/**
	 * Test of methods hasParam, getParam, getParamAt, and getParamNameAt of class
	 * Molecule.
	 */
	@Test
	...
	public void testSetParamAt() {
		...
		Molecule m1 = mol.clone(mol.descriptor.clone()); // clone so the `mol`
																											// remains the same
		Molecule m2 = mol.clone(mol.descriptor.clone()); // clone so the `mol`
																											// remains the same
		//
		int paramIdx = 4;
	...
	public void testGetSetZ() {
		...
		//
		Molecule m2 = new Molecule(new PSFModel.Params(new int[] { PSFModel.Params.X, PSFModel.Params.Y,
			PSFModel.Params.Z }, new double[] { -0.2, 0.3, 150.0 }, false));
