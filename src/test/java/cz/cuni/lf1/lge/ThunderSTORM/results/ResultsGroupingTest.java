...
public class ResultsGroupingTest {
    ...
    public void testGetMergedMolecules() {
        ...
        //
        assertEquals(5, ResultsGrouping.getMergedMolecules(generateTable(), 10.0, 0, 3, 0).size()); // radius = 10nm, off_frames = 0, max_frames = 3
        assertEquals(3, ResultsGrouping.getMergedMolecules(generateTable(), 10.0, 2, 0, 0).size()); // radius = 10nm, off_frames = 2, max_frames = 0
        assertEquals(2, ResultsGrouping.getMergedMolecules(generateTable(), 50.0, 2, 5, 0).size()); // radius = 10nm, off_frames = 2, max_frames = 5
    }

    private GenericTableModel generateTable() {
        MoleculeDescriptor molDesc = new MoleculeDescriptor(new String[] {LABEL_ID, LABEL_FRAME, LABEL_X, LABEL_Y},
                new Units[] {Units.UNITLESS, Units.UNITLESS, Units.NANOMETER, Units.NANOMETER});
